---
solution: Journey Optimizer
product: Journey Optimizer
title: 開始使用
description: 開始使用 Adobe Journey Optimizer
redpen-status: CREATED_||_2025-08-11_20-05-24
exl-id: fee218bf-8c60-442d-ac41-a3856fc71b70
source-git-commit: 5951889d9cc245488ef89b8a6fc73e515068281e
workflow-type: tm+mt
source-wordcount: '608'
ht-degree: 88%

---

# 開始使用 Adobe Journey Optimizer{#get-started}

Adobe Journey Optimizer可讓您透過單一應用程式，大規模地跨每個管道提供個人化的即時客戶體驗。 如此一來，有意義的品牌體驗就能提升客戶忠誠度和終身價值。

無論您是 Journey Optimizer 的新手還是希望擴展技能的經驗豐富使用者，本節都將為您提供快速有效地入門所需的一切。

## 從基本要點開始 {#essentials}

了解 Adobe Journey Optimizer 的基礎知識，以及它如何實現大規模的資料導向個人化。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/book.svg?lang=zh-Hant)

什麼是 Journey Optimizer？

探索核心功能、使用案例以及 Journey Optimizer 提供個人化客戶體驗的方式。

[了解基本知識](../using/start/get-started.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/code-branch.svg?lang=zh-Hant)

了解其運作原理

了解 Journey Optimizer 如何與 Adobe Experience Platform 整合，以及其主要功能區域如何共同運作。

[探索架構](../using/start/understanding-ajo.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/list.svg?lang=zh-Hant)

重要術語

協助您自信地瀏覽 Journey Optimizer 的基本術語和概念。

[檢視術語](../using/start/terminology.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/sparkle.svg)

AI 與智慧功能

探索 AI 如何協助您建立內容、最佳化傳送時間，以及做出更明智的決策。

[探索 AI 功能](../using/start/ai-features.md)
:::

::::

## 角色快速入門 {#by-role}

開始使用根據您的職責和目標量身打造的角色特定指南。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/user.svg?lang=zh-Hant)

針對行銷人員

設計客戶歷程、建立行銷活動和個人化內容，以吸引您的客群。

[行銷人員快速入門](../using/start/path/marketer.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/database.svg?lang=zh-Hant)

針對資料工程師

設定資料結構、管理結構描述和資料集，以及確保資料品質。

[資料工程師快速入門](../using/start/path/data-engineer.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/gear.svg?lang=zh-Hant)

針對管理員

為您的組織設定環境、管理權限及設定管道。

[管理員快速入門](../using/start/path/administrator.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/code.svg?lang=zh-Hant)

針對開發人員

整合行動應用程式、實作 Web SDK、設定事件及建立自訂整合。

[開發人員快速入門](../using/start/path/developer.md)
:::

::::

[檢視所有角色和職責→](../using/start/quick-start.md)

## 高效工作 {#work-efficiently}

掌握介面並利用智慧功能來加快工作速度。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/globe.svg?lang=zh-Hant)

瀏覽介面

了解如何使用 Journey Optimizer 介面、存取重要功能，並快速找到所需的內容。

[探索 UI](../using/start/user-interface.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/bullseye.svg?lang=zh-Hant)

搜尋與整理

使用搜尋、篩選和標籤來有效管理歷程、行銷活動和內容。

[掌握搜尋功能](../using/start/search-filter-categorize.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/help.svg)

疑難排解常見問題集

尋找常見問題的解答，以及各種管道和功能的常見問題的解決方案。

[檢視疑難排解指南](../using/start/troubleshooting.md)
:::

::::

## 其他資源 {#additional-resources}

### 參考資料與最佳實務

* **[護欄和限制](../using/start/guardrails.md)** - 了解有效使用 Journey Optimizer 功能的限制和建議做法
* **[協助工具功能](../using/start/accessibility.md)** - 了解協助工具功能及 WCAG 合規性，確保對所有使用者的包容性。
* **[疑難排解指南](../using/start/troubleshooting.md)** - 尋找常見問題的解答和常見問題的解決方案

### 學習並取得支援

* **[教學課程影片](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/overview.html?lang=zh-Hant){target="_blank"}** - 涵蓋核心功能的逐步教學課程影片
* **[使用案例教戰手冊](../using/start/ai-features.md#playbooks)** - 針對捨棄購物車復原和歡迎行銷活動等常見案例，預先建立工作流程
* **[監視與疑難排解](troubleshoot-journey-landing-page.md)** - 追蹤績效、識別問題和快速解決問題的工具
* **[最新資訊](../using/rn/release-notes.md)** - 了解最新功能、改進和修正
* **[社群論壇](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer/ct-p/journey-optimizer?profile.language=zh-Hant){target="_blank"}** - 與其他使用者交流、提出問題並分享最佳做法
* **[文件更新](../using/rn/documentation-updates.md)** - 隨時了解文件的最新變更和新增內容

## 準備好開始使用了嗎？

選擇適合您的路徑：

* **剛開始使用 Journey Optimizer？**&#x200B;首先學習[「什麼是 Journey Optimizer？」](../using/start/get-started.md)以了解基本知識
* **知道您的角色嗎？**&#x200B;跳轉到符合您職責的[角色特定指南](../using/start/quick-start.md)
* **想要探索嗎？**&#x200B;瀏覽 [AI 功能](../using/start/ai-features.md)或嘗試[使用案例教戰手冊](../using/start/ai-features.md#playbooks)
* **需要協助嗎？**&#x200B;請檢視[疑難排解指南](../using/start/troubleshooting.md)或造訪[社群論壇](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer/ct-p/journey-optimizer?profile.language=zh-Hant){target="_blank"}

## 快速連結 {#quick-links}

使用這些重要指南立即開始建立：

| 任務 | 指南 |
|------|-------|
| 建立您的第一個歷程 | [歷程建立指南](../using/building-journeys/journey-gs.md) |
| 傳送您的第一個行銷活動 | [行銷活動建立指南](../using/campaigns/get-started-with-campaigns.md) |
| 設計電子郵件 | [電子郵件設計手冊](../using/email/get-started-email-design.md) |
| 建立客群 | [客群建立指南](../using/audience/creating-a-segment-definition.md) |
| 設定管道 | [管道設定指南](../using/configuration/get-started-configuration.md) |
| 建立個人化產品建議 | [決策管理指南](../using/offers/get-started/starting-offer-decisioning.md) |

>[!NOTE]
>
>您的環境可用元件和功能取決於您的[權限](../using/administration/permissions.md)和[授權套件](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"}。如有關於特定設定的問題，請聯絡您的 Adobe 客戶成功經理。
