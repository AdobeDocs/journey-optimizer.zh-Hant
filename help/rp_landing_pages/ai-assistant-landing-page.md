---
solution: Journey Optimizer
product: Journey Optimizer
title: 產生內容
description: 產生內容
redpen-status: CREATED_||_2025-08-12_00-42-48
exl-id: 0fdc67ab-3646-4c6e-884d-604238549c3d
source-git-commit: de2272c6d570047cd386941cd2e38cf82942c029
workflow-type: tm+mt
source-wordcount: '318'
ht-degree: 31%

---

# 產生內容{#section-overview}

Adobe Journey Optimizer的產生內容功能利用產生式人工智慧為電子郵件、簡訊、推播通知、網頁等製作個人化文字和視覺效果，徹底改變了內容建立方式。 無論您是產生完整行銷活動還是修訂特定資產，此功能都能協助您將內容順暢地與品牌指導方針保持一致，同時節省時間。 您將探索如何存取產生內容、瞭解其護欄，以及跨不同管道運用其功能。 每個子主題都提供實用的指引，涵蓋從製作吸引人的電子郵件到實驗內容變化，讓您能夠放心地建立有影響力的符合品牌形象的行銷活動，引起客群的共鳴。

## 產生內容

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/circle-play.svg)

產生內容快速入門

瞭解如何存取及設定產生內容以增強內容建立和個人化。

[開始使用產生內容](../using/content-management/gs-generative.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/envelope.svg)

產生完整內容

探索如何使用AI製作完整的內容，包括電子郵件、推播通知、網頁和登入頁面的文字和影像。

[了解完整內容產生](../using/content-management/generative-full-content.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/message.svg)

僅產生文字

使用AI產生並調整所有頻道的文字內容，包括電子郵件、簡訊、推播通知、網頁和登入頁面。

[探索文字產生](../using/content-management/generative-text.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/image.svg)

僅產生影像

使用AI來建立和自訂影像，以便在電子郵件、推播通知、網頁和登入頁面中使用。

[產生影像](../using/content-management/generative-image.md)
:::

::::


## 其他資源

- **[使用 AI 進行內容實驗](../using/content-management/generative-experimentation.md)** - 使用內容實驗功能測試並測量多個內容變化版本的效能。
- **[產生內容使用案例](../using/content-management/generative-uc.md)** — 探索各種方式，以使用產生內容來產生、細分和自訂。
- **[品牌管理與一致性](brands-landing-page.md)** - 管理品牌方針，並確保在 AI 產生的內容和行銷活動中保持一致。
- **[參加課程：由AI支援的品牌內內容建立](https://experienceleague.adobe.com/en/courses/ajo-ai-powered-on-brand-content-creation-for-marketers){target="_blank"}** — 瞭解如何使用AI支援的工具在Adobe Journey Optimizer中為行銷人員建立品牌內內容。
