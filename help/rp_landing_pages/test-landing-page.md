...
使用特定輪廓進行測試以在發佈前驗證您的歷程，以確保事件、條件和動作如預期般運作。 適用於使用命名空間的草稿歷程。 對於沒有測試設定檔的快速反複專案，請改用[歷程模擬](../using/building-journeys/simulate-journey-gs.md)。
...
不確定要使用哪三種歷程驗證方法？ 檢視[選擇如何測試及驗證您的歷程](../using/building-journeys/choose-validation-method.md)以進行完整比較。
...
1. **使用範例資料測試內容變化版本** - 使用 CSV 或 JSON 檔案中的[範例輸入資料](../using/test-approve/simulate-sample-input.md)、手動輸入或 AI 自動產生來測試最多 30 個個人化案例，而不需要建立許多測試輪廓。
...
**[範例輸入資料](../using/test-approve/simulate-sample-input.md)** - CSV 或 JSON 檔案、手動輸入或 AI 自動產生，以測試個人化。 最多可支援 30 種變化版本。 [如何模擬內容變化版本](../using/test-approve/simulate-sample-input.md)
...
* [模擬內容變化版本](../using/test-approve/simulate-sample-input.md) - 使用 CSV 或 JSON 檔案、手動輸入或 AI 自動產生來測試最多 30 個個人化案例。
...
**搭配使用：**&#x200B;重複設計時的歷程模擬、使用真實測試設定檔進行手動訊息/邏輯測試的測試模式、大規模進行最終邏輯檢查的練習。 檢視[選擇如何測試及驗證您的歷程](../using/building-journeys/choose-validation-method.md)以進行完整比較。
