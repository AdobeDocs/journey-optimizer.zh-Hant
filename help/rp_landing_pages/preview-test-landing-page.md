...
使用範例輸入資料或 AI 產生的變體來模擬內容變化版本，以測試個人化內容並確保準確性。
...
**內容：**&#x200B;此表格會將測試目標對應至 Adobe Journey Optimizer 中的特定工具。
...
| 快速測試 10 種以上的內容變化版本 | [範例輸入資料](../using/test-approve/simulate-sample-input.md) |
| 傳送前先取得利害關係人的核准 | [傳送校樣](../using/content-management/proofs.md) |
| 檢查 Gmail、Outlook、Apple Mail 之間的電子郵件顯示 | [Litmus 轉譯](../using/content-management/rendering.md) |
| 改善收件匣版位 | [垃圾郵件報告](../using/content-management/spam-report.md) |
| 一次預覽所有變化版本 | [預覽模式](../using/content-management/preview.md) |

## 測試工作流程檢查清單

**內容：**&#x200B;適用於所有管道 (電子郵件、簡訊、推播、網頁、應用程式內) 的建議五步測試順序。

請依照下列順序進行完整驗證：

1. **預覽** - 使用測試輪廓以正確檢查個人化轉譯
2. **測試變化版本** - 上傳範例資料 CSV/JSON 以驗證多個案例
3. **檢查傳遞能力** (電子郵件) - 執行垃圾郵件報告和轉譯測試
4. **傳送校樣** - 與利害關係人共用以進行檢閱和核准
5. **最終檢查** - 驗證所有連結、影像和 CTA 是否正常運作

**專業提示：**&#x200B;使用至少 3 個代表不同客戶區段 (高值、新增、非使用中) 的輪廓進行測試，以捕捉邊緣案例。
...
- **測試輪廓**&#x200B;對於預覽個人化內容至關重要；請使用範例輸入資料來有效測試 10 個以上的變化版本
- **電子郵件特定工具**&#x200B;包含轉譯測試 (Litmus)、垃圾郵件報告和校樣
- **建議的順序：**&#x200B;預覽 → 測試變化版本 → 檢查傳遞能力 → 傳送校樣 → 最終檢查
- **省時：**&#x200B;上傳具有客戶屬性的 CSV/JSON，而非建立個別的測試輪廓
