...
**在此頁面上：**&#x200B;探索程式碼型管道如何讓您在應用程式和網頁上，將個人化內容傳送到精細位置，以及何時使用它來取代其他管道。
...
當您的使用案例超出視覺編輯器所能支援的範圍，且具備可投入建置與維護實作的開發資源時，程式碼型管道的運作成效最佳。

| 優點 | 原因 | 使用案例範例 |
| --- | --- | --- |
| 深度個人化 | 支援視覺編輯器無法提供的複雜邏輯和即時資料 | 由即時庫存或使用者內容驅動的個人化 |
| 與外部系統整合 | 可以使用外部系統的資料來撰寫內容 | 以天氣為基礎的促銷活動、由即時庫存驅動的優惠 |
| 進階、多步驟條件式工作流程 | 不受視覺化管道預先定義動作的限制 | 跨接觸點的多步驟決策邏輯 |
| 超越平台限制 | 可讓開發人員建置自訂互動式元素 | 現成管道不支援的自訂 UI 元件 |
| 提升行銷活動的彈性 | 內容和邏輯皆可依您的實施自行定義 | 無頭式、API 型或非瀏覽器數位資源 |

## 不適用情境 {#when-not-to-use}

由於程式碼式管道需要投入開發工作，因此並非適用於所有情境。在下列情況下，建議考慮採用其他管道：

* 您的行銷活動簡單且可快速完成，而且可以透過 Web 或應用程式內等無程式碼管道建置，無需任何開發工作
* 沒有可用的開發資源或測試環境來建置及驗證自訂s實施
* 自訂開發的時間表或預算有限，因為程式碼型體驗需要更多前期規劃
* 標準傳送訊息功能已符合內建管道的功能，因此無需進行自訂開發
* 擔心自訂程式碼的長期維護性，因為相關介面與實施需要持續的開發人員支援
