...
1. 在「電子郵件設計器」首頁中，從以下選項中選擇設計電子郵件的方式：
   ...
   * 選擇 **[!UICONTROL Design from scratch]** 使用電子郵件設計器功能建立電子郵件內容。 [進一步了解](#design-scratch)
...
1. 如果需要，您可以切換到代碼編輯器，通過按一下 **[!UICONTROL Switch to code editor]** 的子菜單。 有關代碼編輯器的詳細資訊，請參閱 [頁](existing-content.md#import-raw-html-code)。
...
1. 按一下 **[!UICONTROL Show preview]** 檢查您的電子郵件呈現。 您可以選擇案頭或移動視圖。
   ...
   有關如何預覽電子郵件的詳細資訊，請參閱 [預覽和test郵件](preview.md)。
...
>title="使用前標"
>abstract="前標允許您配置簡短的摘要文本，以幫助您更好地跟蹤和自定義電子郵件。"

>[!NOTE]
>
>請注意，預標頭與所有電子郵件客戶端不相容。 不支援時，不顯示前標。

前標是一個簡短的摘要文本，在查看來自電子郵件客戶端的電子郵件時，它跟在主題行後。 前標可以幫助您更好地跟蹤和自定義電子郵件。
