...
1. 使用[片段](../content-management/fragments.md)將多個內容新增至決定專案 — 例如，如果您想要為多個行動裝置型號顯示不同的內容。 然後，您就可以在您的決定政策中利用這些片段。 [了解作法](fragments-decision-policies.md)

   >[!AVAILABILITY]
   >
   >此功能在&#x200B;**程式碼型體驗**&#x200B;和&#x200B;**電子郵件**&#x200B;管道的「有限可用性」中提供。 如欲請求存取權，請和您的 Adobe 代表聯絡。

   在您的決定政策中使用這些片段時，您可以將片段設為選用，如果Edge上暫時無法使用，則會略過該片段。 [了解作法](fragments-decision-policies.md#optional-fragments)
