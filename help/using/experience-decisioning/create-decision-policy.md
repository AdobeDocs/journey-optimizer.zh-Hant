...
若要向客戶呈現最佳的動態優惠和體驗，請將決定原則新增至行銷活動或歷程中的內容，然後設定要傳回的專案及要使用的選擇策略。 若要執行此操作，請遵循下列步驟。
...
1. 開啟個人化編輯器以建立決定原則。
   ...
   對於電子郵件，您也可以在電子郵件設計工具中使用專用功能表來建立決定原則。 展開下列各節以探索這兩種方法。
...
1. 選取要傳回的專案數。 例如，如果您選取 2，則目前設定將顯示最佳的 2 個符合資格的產品建議。
   ...
   若要在電子郵件中傳回多個專案，您必須使用&#x200B;**[!UICONTROL 重複網格]**&#x200B;內容元件。 展開下列區段以取得詳細資訊：
