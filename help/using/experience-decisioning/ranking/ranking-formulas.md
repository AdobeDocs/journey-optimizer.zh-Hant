...
**在此頁面上：**&#x200B;使用AI公式產生器建立排名公式，該公式會合併AI模型分數、優惠優先順序、設定檔屬性和內容訊號，因此您可以控制哪個優惠會最先出現，並依據您的業務目標和客戶需求做出決策。
...
>title="建立排名公式"
>abstract="排名公式可讓您定義規則，以決定應先呈現的決策項目，而非考量項目的優先順序分數。 建立排名公式後，即可將其指派至選取策略。"
