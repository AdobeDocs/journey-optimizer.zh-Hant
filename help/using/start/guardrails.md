...
權利、產品限制和效能護欄列於 [Adobe Journey Optimizer 產品說明頁面](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"} {target=&quot;_blank&quot;}。

在開始之前，您還需要注意即時客戶設定檔資料](https://experienceleague.adobe.com/docs/experience-platform/profile/guardrails.html?lang=zh-Hant){target="_blank"}{target=&quot;_blank&quot;}的[護欄。
...
每個套件的授權量度都列在[Journey Optimizer產品說明](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"}{target=&quot;_blank&quot;}頁面上。

### 程式碼型管道護欄 {#code-based-guardrails}

若要在[!DNL Journey Optimizer]中使用程式碼型體驗動作，並傳遞您的應用程式可以使用的程式碼內容承載，請依照[此頁面](../code-based/code-based-prerequisites.md)上詳述的先決條件執行。
...
在[此頁面](../configuration/delegate-subdomain.md)上進一步瞭解網域委派。
...
在[此頁面](../audience/get-started-audience-orchestration.md)上進一步瞭解對象組合。
...
* 內建的&#x200B;**反應**&#x200B;事件可讓您對開箱即用的動作做出反應。 在[此頁面](../building-journeys/reaction-events.md)瞭解更多資訊。 如果要對透過自訂動作傳送的訊息做出反應，則需設定專用事件。
* 您無法同時進行兩個動作，必須逐一新增。
...
特定護欄套用於&#x200B;**[!UICONTROL 更新輪廓]**&#x200B;活動。它們列在[此頁面](../building-journeys/update-profiles.md)上。
...
* 歷程只能有一個&#x200B;**讀取客群**&#x200B;活動。
* 另請參閱關於如何使用[此頁面](../building-journeys/read-audience.md)上的&#x200B;**讀取對象**&#x200B;活動的建議。
...
下列護欄適用於&#x200B;**[!UICONTROL 應用程式內訊息]**&#x200B;動作。在[此頁面](../in-app/create-in-app.md)上進一步瞭解應用程式內訊息。
...
特定護欄適用於&#x200B;**[!UICONTROL 跳轉]**&#x200B;活動。它們列在[此頁面](../building-journeys/jump.md#jump-limitations)上。
