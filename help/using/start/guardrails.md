...
**在此頁面上：**&#x200B;檢閱Adobe Journey Optimizer的系統、歷程、對象、管道和內容限制，以便規劃可擴充的部署，而不會發生失敗。
...
>[!TIP]
>
>**這對您來說意味著什麼：** **50活動限制**&#x200B;和&#x200B;**使用中歷程限制**&#x200B;是大多數團隊在縮放時首先遇到的兩個護欄。 規劃提早分割歷程，並將讀取對象開始時間隔開至少5至10分鐘，以避免沙箱輸送量競爭。
...
* 針對所有自訂動作、每個主機和每個沙箱，已定義一分鐘內&#x200B;**300,000次呼叫的上限限制**。 此上限會針對回應時間少於 0.75 秒的端點，強制作為每個沙箱和每個端點的滑動視窗。 對於回應時間超過0.75秒的端點，則套用每30秒&#x200B;**150,000次呼叫的個別限制** （也是滑動視窗）。
* 自訂動作 URL 不支援動態參數。
* 支援POST、PUT和GET呼叫方法。
* 查詢參數或標題的名稱不得以「.」開頭 或「$」。
* URL中不允許使用IP位址。 請使用主機名稱。
* 內部 Adobe 地址 (`.adobe.*`) 不允許在 URL 及 API 中使用。
* 無法移除內建自訂動作。
* 自訂動作僅在使用請求或回應裝載時支援 JSON 格式。 請參閱[此頁面](../action/about-custom-action-configuration.md#custom-actions-limitations)。
* 自訂動作目標的任何端點都必須支援至少&#x200B;**200 TPS**。 請留意，節流設定不可低於 200 TPS。 根據您的預期輸送量，高回應時間可能會影響實際輸送量。

>[!TIP]
>
>**這對您有何意義：**&#x200B;預設的300,000個呼叫/分鐘上限可保護您的外部端點，避免因歷程輸送量而受限。 如果您的端點可以處理更多負載，您可以使用[上限API](../configuration/capping.md)或[節流API](../configuration/throttling.md)來提高此限制。 如需Journey Optimizer如何連線至外部系統的更廣泛總覽，請參閱[此頁面](../configuration/external-systems.md)。 如果您需要更高的組織限制，請聯絡您的Adobe代表。
...
* 每個組織在所有沙箱和歷程中最多可同時執行&#x200B;**五個** **讀取對象**&#x200B;執行個體（已排程或已觸發業務事件）。 避免同時開始超過五個具有&#x200B;**讀取客群**&#x200B;的歷程；讓它們相隔 5 到 10 分鐘。 在[本節](../building-journeys/entry-management.md#journey-processing-rate)中深入了解歷程處理速率。
* 沙箱輸送量：系統管理每個沙箱的處理，每秒最多&#x200B;**20,000個設定檔**，在所有&#x200B;**讀取對象**&#x200B;活動中共用。 個別活動可設定為每秒&#x200B;**500到20,000個設定檔**。 如果達到沙箱限制，工作可能會排入佇列。
* 工作處理逾時： **無法於** 12小時&#x200B;**內處理的讀取對象**&#x200B;工作會自動清除，且不會執行。
* 擷取匯出作業時，預設會對受眾觸發的歷程套用重試。 如果在建立匯出工作期間發生錯誤，將每隔10分鐘進行重試，最多為&#x200B;**1小時**。 之後，該歷程會被視為失敗，因此最多可在排程時間後的1小時內執行。
* 對於使用補充ID的歷程，每個歷程執行個體的&#x200B;**讀取對象**&#x200B;活動的讀取率限製為每秒&#x200B;**500個設定檔上限**。
