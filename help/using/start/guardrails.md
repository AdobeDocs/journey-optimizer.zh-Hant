...
在下方，您會找到使用[!DNL Adobe Journey Optimizer]時的額外護欄和限制。
...
* 擷取資料時，電子郵件會區分大小寫。 這表示可能會建立重複的設定檔(例如，John.Greene@luma.com的一個設定檔、john.greene@luma.com的另一個設定檔)，並在您的[!DNL Journey Optimizer]歷程和行銷活動中鎖定對應的收件者時使用。
