...
* 對於使用&#x200B;**讀取對象**&#x200B;活動的歷程，可以同時開始的歷程次數有上限。 系統將執行重試，但請避免在完全相同的時間，開始超過五個歷程（具有&#x200B;**讀取對象**、排程或「儘快」），方法是將其分散在一段時間內，例如相隔5到10分鐘。
* **讀取對象**&#x200B;活動無法與Adobe Campaign活動搭配使用。
* **讀取對象**&#x200B;活動只能用作歷程中的第一個活動，或商務事件活動之後的第一個活動。
* 歷程只能有一個&#x200B;**讀取對象**&#x200B;活動。
* 另請參閱關於如何在[此頁面](../building-journeys/read-audience.md)中使用&#x200B;**讀取對象**&#x200B;活動的建議。
