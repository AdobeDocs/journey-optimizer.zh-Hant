...
* 內建的&#x200B;**反應**&#x200B;事件可讓您對開箱即用的動作做出反應。 在[本頁](../building-journeys/reaction-events.md)中瞭解更多。如果您想要對透過自訂動作傳送的訊息做出反應，則必須設定專用事件。
* 您無法同時進行兩個動作，必須逐一新增。
...
* 您無法建立使用增量讀取方式的新讀取對象歷程版本。 您必須複製歷程。

### 自訂動作 {#custom-actions-g}

* 為每個主機和每個沙箱的所有自訂動作，定義 1 分鐘內 300,000 次呼叫的上限。請參見[此頁面](../action/about-custom-action-configuration.md)。此限制是根據客戶使用情況來設定，可保護自訂動作鎖定為目標的外部端點。您必須透過定義適當的讀取率（使用自訂動作時為5000個設定檔/秒），在以對象為基礎的歷程中考量這一點。 如有需要，您可以透過上限/節流 API 定義較高的上限或節流限制來覆寫此設定。 請參閱[此頁面](../configuration/external-systems.md)。
* 自訂動作 URL 不支援動態參數。
...
* 使用自訂動作選擇要作為目標的端點時，請確定：
   ...
   * 此端點可使用來自[節流 API](../configuration/throttling.md) 或[設定 API 上限](../configuration/capping.md)的設定來支援歷程的輸送量，藉此加以限制。 請留意，節流設定不可低於 200 TPS。任何目標端點都必須支援至少200個TPS。
...
* Journey Optimizer 需將事件串流至資料收集核心服務 (DCCS)，才能觸發歷程。 批次收錄的事件，或來自內部 Journey Optimizer 資料集的事件 (訊息意見回饋、電子郵件追蹤等等) 無法用來觸發歷程。 若是無法取得串流事件的使用案例，您必須根據這些事件建立對象，並改用&#x200B;**讀取對象**&#x200B;活動。 技術上可使用對象資格，但不建議使用，因為它可能會根據使用的動作導致下游挑戰。
...
特定護欄適用於&#x200B;**[!UICONTROL 更新設定檔]**&#x200B;活動。 它們列在[此頁面](../building-journeys/update-profiles.md)中。
...
* 「讀取對象」活動無法搭配Adobe Campaign活動使用。
* 讀取對象活動只能作為歷程中的第一個活動，或商業事件活動後的第一個活動。
* 歷程只能有一個讀取對象活動。
* 另請參閱[此頁面](../building-journeys/read-audience.md)中關於如何使用讀取對象活動的建議。
...
* 對象資格活動無法與Adobe Campaign活動搭配使用。


### 運算式編輯器 {#expression-editor}

* 從讀取對象、對象資格或業務事件活動開始的歷程中，無法使用體驗事件欄位群組。 您必須建立新對象，並在歷程中使用對象不足條件。
...
* 應用程式內活動無法搭配Adobe Campaign活動使用。

* 應用程式內顯示會繫結至歷程期限，這表示當設定檔的歷程結束時，該歷程中的所有應用程式內訊息將不再顯示該設定檔。  因此，無法直接從歷程活動停止應用程式內訊息。 您必須改為結束整個歷程，才能停止將應用程式內訊息顯示給設定檔。
...
特定護欄適用於&#x200B;**[!UICONTROL 跳轉]**&#x200B;活動。 它們列在[此頁面](../building-journeys/jump.md#jump-limitations)中。

### 行銷活動 {#ac-g}

下列護欄適用於&#x200B;**[!UICONTROL Campaign v7/v8]**&#x200B;和&#x200B;**[!UICONTROL Campaign Standard]**&#x200B;活動：

* Adobe Campaign活動不能與讀取對象或對象資格活動搭配使用。
* 這些活動無法與應用程式內活動搭配使用。
...
|---------|----------|
| 決策 API 請求 | 每秒 500 個 |
| Edge Decisioning API請求與Edge分段 | 每秒 1500 個 |
| Edge Decisioning API請求不含Edge分段 | 每秒 5000 個 |
