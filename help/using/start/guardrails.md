...
當使用 [!DNL Adobe Journey Optimizer] 時，您將找到下列其他護欄和限制。
...
>[!NOTE]
>
>這些限制不適用於具有&#x200B;**Select**&#x200B;套件的讀取對象或業務事件歷程。 如果您需要具有多個動作、條件或等待活動的更複雜歷程邏輯，請考慮升級您的授權套件或在適用的情況下使用讀取對象歷程。

對於使用&#x200B;**Select**&#x200B;授權套件的客戶，下列額外限制尤其適用於單一歷程、以事件開始的歷程或對象資格：

* **SELECT套件：單一歷程中只允許一個動作(ERR_PKG_SELECT_8)**：單一歷程只能包含一個動作活動。 您無法在同一歷程中新增多個電子郵件、推播、簡訊或其他動作活動。

* **SELECT套件：單一歷程中不允許條件(ERR_PKG_SELECT_7)**：單一歷程中不能使用條件活動。 歷程必須遵循單一線性路徑，不含分支邏輯。

* **SELECT封裝：單一歷程(ERR_PKG_SELECT_6)中不允許等待**：等待活動無法新增至單一歷程。 動作必須立即執行且無延遲。

* **SELECT套件：從節點轉換逾時/錯誤必須僅指向結束節點(ERR_PKG_SELECT_2)**：如果您設定動作（例如電子郵件動作）的逾時或錯誤轉換，這些路徑必須直接指向結束節點。 他們無法連線到歷程中的其他活動或動作。
...
* 在歷程專案中，設定檔會使用批次對象快照中的屬性值。 但是，當設定檔達到&#x200B;**等待**&#x200B;活動時，歷程會透過從整合設定檔服務(UPS)擷取最新資料來自動重新整理設定檔屬性。 這表示在歷程執行期間，設定檔屬性可能會變更。
