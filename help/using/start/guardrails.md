...
#### 歷程裝載大小驗證 {#journey-payload-size}

當您儲存或發佈歷程時，Journey Optimizer會驗證歷程裝載總大小，以保留穩定性和效能。

**預設組態**

* **預設要求大小上限**： 2 MB （2,000,000位元組）。 某些組織可能有Adobe設定的自訂限制。
* **警告臨界值**：最大限制的90%。
* **錯誤臨界值**：最大限制的100%。 儲存或發佈被封鎖，要求傳回&#x200B;**HTTP 413要求實體太大**。

**使用者體驗案例**

* **承載&lt;限制的90%**：歷程已成功儲存和發佈。 不會顯示警告或錯誤。
* **承載90-99%的限制**：歷程儲存並發佈成功，並出現警告以最佳化。 警告訊息： **警告**：歷程裝載大小接近限制。 最大節點： &#39;[NodeName]&#39; （型別： &#39;[NodeType]&#39;，大小： [N]位元組）。
* **承載>= 100%限制**：歷程儲存或發佈因錯誤而封鎖。 錯誤訊息： **錯誤**：歷程承載大小超過限制。 最大節點： &#39;[NodeName]&#39; （型別： &#39;[NodeType]&#39;，大小： [N]位元組）。

**錯誤回應詳細資料**

如果要求超過允許的大小上限，回應會包含&#x200B;**要求實體太大**。 歷程承載超過允許的大小上限。 檢閱錯誤詳細資料並最佳化您的歷程。

**疑難排解與建議**

* 檢閱警告或錯誤中反白顯示的最大節點。
* 簡化條件、減少資料對應，以及移除不必要的步驟或引數。
* 如有需要，請考慮將歷程分割為較小的歷程。
* 如果您認為您的組織需要更高的限制，請聯絡您的Adobe代表。
...
* **[!UICONTROL Campaign Standard]**&#x200B;活動無法搭配其他頻道活動使用：卡片、程式碼型體驗、電子郵件、推播、簡訊、應用程式內訊息、網頁。
* **[!UICONTROL 促銷活動v7/v8]**&#x200B;活動可與相同歷程中的原生頻道活動搭配使用。
...
* 應用程式內活動無法與&#x200B;**[!UICONTROL Campaign Standard]**&#x200B;活動搭配使用。
