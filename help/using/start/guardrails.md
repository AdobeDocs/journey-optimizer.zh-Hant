...
* 以「 」開始的歷程 **對象資格** v1中的活動必須一律以 **對象資格** 在後續版本中。
* 在中選擇的對象和名稱空間 **對象資格** （第一個節點）無法在新版本中變更。
* 所有歷程版本中的重新進入規則必須相同。
* 以「 」開始的歷程 **讀取對象** 在後續版本中無法以其他事件開頭。
* 您無法建立具有增量讀取的新版本讀取對象歷程。 您需要複製歷程。
...
* Journey Optimizer 需將事件串流至資料收集核心服務 (DCCS)，才能觸發歷程。 批次收錄的事件，或來自內部 Journey Optimizer 資料集的事件 (訊息意見回饋、電子郵件追蹤等等) 無法用來觸發歷程。 若是無法取得串流事件的使用案例，請根據這些事件建立受眾，並使用 **讀取對象** 活動。 技術上可使用對象資格，但可能會根據使用的動作造成下游挑戰。
...
* 體驗事件欄位群組不能用於以讀取對象、對象資格或業務事件活動開始的歷程。 您需要建立新受眾，並在歷程中使用受眾條件。
