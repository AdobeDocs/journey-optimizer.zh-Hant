...
* 生產沙箱中的即時、已關閉、已暫停和試執行歷程數量上限為&#x200B;**200**，開發沙箱中的數目上限為&#x200B;**100**。 此限制會在您發佈歷程時強制執行。 目前的歷程數目會顯示在歷程畫布上方。

  當您發佈歷程時，我們會自動縮放和調整，以確保輸送量與穩定性達到最高。 已關閉的歷程只有在轉出此護欄後建立時才計算。

>[!NOTE]
>
>對於發佈時間護欄，在引入護欄時已超過限制的組織會收到例外狀況。 現有歷程不受影響。

* 在歷程中使用客群資格篩選時，該客群資格篩選活動最多可能需要 **10 分鐘**，才會啟用和接聽進入或退出客群的輪廓。

* 輪廓的歷程執行個體大小上限為 **1 MB**。 在歷程執行過程中收集的所有資料都會儲存在該歷程執行個體中。 因此，來自傳入事件的資料、從 Adobe Experience Platform 擷取的輪廓資訊、自訂動作回應等會儲存在該歷程執行個體中，並影響歷程大小。 當歷程以事件開始時，建議限制該事件承載的大小上限 (例如：低於 **800 KB**)，以避免在歷程執行中經過幾項活動後達到該限制。 當達到該限制時，輪廓處於錯誤狀態並將從歷程中排除。
...
>[!TIP]
>
>**這對您來說意味著什麼：** **50活動限制**&#x200B;和&#x200B;**使用中歷程限制**&#x200B;是大多數團隊在縮放時首先遇到的兩個護欄。 及早規劃歷程分割，並將讀取客群開始時間隔開至少 5 至 10 分鐘，以避免沙箱輸送量競爭。
...
| 承載為限制的 90-99% | 警告 (軟式) | 歷程儲存並發佈警告：**警告**：歷程承載大小接近限制。 最大節點：&#39;[NodeName]&#39; (類型：&#39;[NodeType]&#39;，大小：[N] 位元組)。 |
| 裝載 ≥ 限制的100% | **錯誤 (硬式)** | 已封鎖儲存或發佈。 傳回 **HTTP 413 請求實體太大**。 錯誤：歷程承載大小超過限制。 最大節點：&#39;[NodeName]&#39; (類型：&#39;[NodeType]&#39;，大小：[N] 位元組)。 |

**預設設定**

* **預設請求大小上限**：**2 MB** (2,000,000 位元組)。 某些組織可能有 Adobe 設定的自訂限制。
* **警告臨界值**：上限的 90%。
* **錯誤臨界值**：上限的 100%。
...
* 在&#x200B;**對象資格** （第一個活動）中選擇的對象和名稱空間在新版本中無法變更。
* 所有歷程版本中的重新進入規則必須相同。
...
* Journey Optimizer支援的尖峰流量為每秒&#x200B;**5,000個傳入歷程事件**&#x200B;適用於單一事件，以及所有沙箱中每秒&#x200B;**5,000個傳入歷程事件**&#x200B;適用於讀取對象型歷程事件。 請[在此頁面](../event/about-events.md#event-thoughput)深入了解此限制。
...
* 在任何一次，在全部即時、已關閉、已暫停、測試模式和試執行歷程中，最多&#x200B;**25**&#x200B;個歷程可參考單一事件。 當達到此限制時，將封鎖使用該事件的任何其他發佈歷程。
* 單一XDM結構描述一次最多可由所有即時、已關閉、已暫停、測試模式和試執行歷程中的&#x200B;**100**&#x200B;個事件參考。 當達到此限制時，會封鎖任何包含參照該結構描述的事件節點的發佈歷程。
* 單一歷程 (從事件或客群資格篩選開始) 包含可防止同一事件多次錯誤觸發歷程的護欄。 在預設情況下，輪廓重新進入時會暫時封鎖 **5 分鐘**。 例如，如果某個事件在12:01觸發特定設定檔的歷程，而另一個事件在12:03達到（無論是相同事件還是不同事件觸發相同歷程），則此設定檔的歷程將不會再次開始。
...
* 客群資格鑑定歷程不支援補充識別碼。
* 沙箱最多可包含所有即時、已關閉、已暫停、測試模式和試執行歷程中的&#x200B;**300**&#x200B;個對象資格活動。 此限制也適用於當作退出條件使用的對象資格活動。 當達到此限制時，會封鎖具有其他對象資格活動的發佈歷程。
...
>[!CAUTION]
>
>如果最終處理的訊息內容超過 **2 MB**，歷程發佈將會失敗。 請將您編寫的訊息內容保持在 2 MB 以下 (最好在 **1 MB** 以下)，以便保留 300-400KB 的緩衝區用於後端處理額外負荷。
