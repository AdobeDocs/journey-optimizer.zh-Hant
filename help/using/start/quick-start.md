...
Journey Optimizer提供兩種主要方法來觸及和吸引客戶：

* **歷程** — 即時、一對一的協調流程，讓每位客戶根據自己的步調行動，並由行為或事件觸發。 最適合入門序列、購物車放棄和生命週期參與。
* **行銷活動** — 根據您的使用案例，提供三種傳送模式的受眾型傳訊：
   * **動作行銷活動** — 排程或週期性訊息一次傳送給定義的對象。 最適合於電子報、促銷公告和產品推出。
   * **API觸發的行銷活動** — 由外部系統透過API觸發的隨選訊息。 最適合用於訂單確認、送貨警示和帳戶通知等交易式訊息。
   * **協調的行銷活動** — 具有多實體細分和畫布式執行的複雜批次工作流程。 最適合季節性促銷、多步驟批次方案，以及需要確切預先傳送計數的行銷活動。
...
成功的實施從準備開始。 在設定Journey Optimizer之前，請根據下列事項調整您的團隊：

* **先定義您的使用案例** — 識別您要處理的客戶案例，並排定其優先順序。 這會指引每個設定決定，從[資料管理](../data/gs-data.md)到[頻道設定](../configuration/get-started-configuration.md)。
* **讓所有接觸客戶體驗的團隊參與** — Journey Optimizer實作通常橫跨行銷、IT、資料和作業。 跨團隊的早期對齊可防止重工。
* **建立共用的客戶識別碼** — 同意所有資料來源的共同識別碼（例如CRM ID或電子郵件地址）。 這是[統一客戶設定檔](../audience/get-started-profiles.md)的基礎。
* **驗證資料隱私權合規性** — 確保您計畫連線的所有資料來源在內嵌前都符合適用的[隱私權法規](../privacy/get-started-privacy.md)。
* **在上線前規劃測試** — 驗證[事件觸發程式、歷程條件以及管道動作](../building-journeys/journey-gs.md)在開發或中繼沙箱中的行為是否如預期般運作。
* **準備您的品牌內容和資產庫** — 識別您的團隊將在歷程和行銷活動中使用的數位資產、範本和品牌准則。 在Launch啟動之前，先將訊息載入Journey Optimizer的[內建資產庫](../integrations/assets.md)，可加快訊息建立速度，並確保品牌從第一天起就保持一致。
