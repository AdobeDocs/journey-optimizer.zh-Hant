...
* **設定URL追蹤引數** — 在表面層級設定追蹤引數，以自動附加行銷活動識別碼（utm_campaign、utm_source等） 至所有電子郵件連結。 這可讓您在整個數位生態系統中進行歸因追蹤。 [了解更多](../email/url-tracking.md)
...
* **自動連結追蹤** — 使用URL協助程式函式，將任何URL新增至您的SMS內容。 Journey Optimizer會自動縮短連結，並追蹤點按，不需要額外設定。 若要縮短URL長度，您必須先設定SMS子網域。 [了解更多](../mobile/mobile-subdomains.md)

* **傳入郵件追蹤** — 會自動擷取收件者的回覆，讓您監視雙向交談和回應模式。 [了解更多](../mobile/mobile-opt-out.md#sms-native-keywords)
