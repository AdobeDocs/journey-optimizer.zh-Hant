...
description: 比較歷程、動作行銷活動、API觸發的行銷活動，以及協調的行銷活動，以選擇適合您在Adobe Journey Optimizer行銷需求的方法。
...
>[!TIP]
>
>**快速經驗法則：**&#x200B;需要每個客戶以自己的速度使用即時邏輯嗎？ 使用&#x200B;**歷程**。 要依排程傳送訊息給對象嗎？ 使用&#x200B;**動作行銷活動**。 透過API從外部系統觸發單一訊息？ 使用&#x200B;**API觸發的行銷活動** — 或是&#x200B;**單一事件歷程** （如果您在API傳送事件後需要多步驟協調）。 需要多實體資料、確切計數或批次畫布？ 使用&#x200B;**協調的行銷活動**。

## 詳細比較 {#detailed-comparison}

使用這份全方位的表格來瞭解主要差異：

| 功能 | 歷程 | 動作行銷活動 | API 觸發的行銷活動 | 協調的行銷活動 |
|---------|----------|------------------|------------------------|----------------------|
...
| 頻道 | 歷程 | 動作行銷活動 | API 觸發的行銷活動 | 協調的行銷活動 |
|---------|:--------:|:----------------:|:-----------------------:|:---------------------:|
| 電子郵件 | ✅ | ✅ | ✅ | ✅ |
| 推播 | ✅ | ✅ | ✅ | ✅ |
| 簡訊 | ✅ | ✅ | ✅ | ✅ |
...
動作行銷活動通常是最簡單的（向對象傳送單一訊息），其次是API觸發的行銷活動、歷程（使用多步驟邏輯更複雜）以及協調的行銷活動（由於畫布工作流程和多實體功能而最複雜）。
...
* **協調的行銷活動**&#x200B;在包含大型資料集和多實體資料的複雜細分方面表現出色。
