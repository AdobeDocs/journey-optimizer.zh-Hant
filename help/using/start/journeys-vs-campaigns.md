...
| 方針 | 最適合 | 執行樣式 |
|----------|----------|-----------------|
| **個歷程** | 具有條件式邏輯的多步驟、即時客戶體驗 | 1:1協調流程 — 每個設定檔都有各自的步調 |
...
| **分段** | 預先建立的對象+即時條件 | Experience Platform的預先建立對象 | 裝載導向鎖定目標（無排程對象） | 在包含精確計數的畫布中建立的隨選受眾 |
| **設定檔處理中** | 個人即時（當事件發生時） | 批次，一次完成 | 每個API呼叫，裝載導向 | 批次，以及多實體支援 |
| **Personalization** | 即時內容資料+設定檔屬性 | 設定檔屬性 | 裝載資料+可選的設定檔屬性 | 精準定位的多實體資料 |
...
是。 在[!DNL Adobe Experience Platform]中建立的對象可用於歷程、動作行銷活動和協調的行銷活動（其中也可以在畫布中隨選建立對象邏輯）。 API觸發的行銷活動是裝載導向，不使用相同方式的預先建立對象。
