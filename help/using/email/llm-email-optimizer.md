...
[!DNL Adobe Journey Optimizer]隨附電子郵件通道功能，可協助您建構訊息的[文字版本](text-version-email.md)以改善AI輔助收件匣體驗，例如[!DNL Apple Intelligence]中的[!DNL Google Gemini]和[!DNL Gmail]，讓他們能更準確地回答問題，並根據您的內容摘要郵件，獲得更好的結果。

>[!NOTE]
>
>此功能只會變更純文字，不會變更電子郵件內容的HTML版本。

您可以使用此功能來調整純文字，讓AI輔助收件匣體驗更容易呈現您想要的選件、行動號召和詳細資訊，而不是精簡的自動產生文字或不相關的內容。
...
1. 選取&#x200B;**[!UICONTROL 純文字]**&#x200B;圖示以開啟電子郵件的文字版本。 [了解更多](text-version-email.md)
...
1. 此時會顯示電子郵件的文字版本。 按一下&#x200B;**[!UICONTROL 「針對AI收件匣最佳化」]**&#x200B;按鈕，以產生改良的純文字版本，其中強調重要資訊，以供AI輔助閱讀和摘要之用。
   ...
   >[!NOTE]
   >
   >按一下&#x200B;**[!UICONTROL 針對AI收件匣最佳化]**&#x200B;按鈕後，**[!UICONTROL 與HTML同步]**&#x200B;選項會自動停用。 [了解更多](text-version-email.md#plain-text-custom)
...
1. 隨即顯示產生的文字。 檢閱變更，視需要編輯，然後如常儲存電子郵件。
   ...
   >[!NOTE]
   >
   >**最佳化AI收件匣的電子郵件文字**&#x200B;只會更新純文字內文。 這不會變更您的HTML設計、版面或影像。
...
* **品牌與法律曝光度** — 不正確或不完整的AI摘要仍可能會導致客戶對促銷活動、條款或選擇退出語言產生混淆或爭議。 **最佳化AI收件匣的電子郵件文字**&#x200B;可改善您提供的文字圖層；它無法確保協力廠商的模型會忠實地重製文字。

* **[!UICONTROL 針對]**&#x200B;中的AI收件匣最佳化[!DNL Journey Optimizer] — 電子郵件Designer中的製作時間控制項與一般使用者收件匣助理不同。 傳送前，請務必檢閱產生的純文字。

## 相關主題 {#related-topics}

* [管理電子郵件的文字版本](text-version-email.md)
