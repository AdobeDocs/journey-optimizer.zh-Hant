...
影像到HTML轉換器是&#x200B;**內容範本**&#x200B;功能表中提供的AI支援創新功能，可將靜態影像設計轉換為完全可自訂、模組化的HTML電子郵件內容範本，大幅加快電子郵件的建立速度。 此無程式碼工具可讓行銷人員將視覺設計從圖形設計工具或設計工具轉換為回應式、可編輯的電子郵件範本，這些範本可儲存至內容範本資料庫，然後跨多個歷程及行銷活動重複使用，無需專業技術。

>[!AVAILABILITY]
>
>此功能為「有限可用性」的狀態。請聯絡您的 Adobe 代表以取得存取權。

運用創作AI技術，將影像轉換為HTML轉換器可分析影像中的版面、印刷樣式、顏色和視覺元素，並產生簡潔的模組化HTML程式碼，以維持設計的精確度，同時確保完整可編輯性並與Email Designer相容。 如此可免除將模型轉換為HTML的手動耗時流程，並減少對於開發資源的依賴。

**主要優點：**

* **幾分鐘即可將設計轉換為HTML**：由AI支援、無程式碼轉換，可在幾分鐘內將靜態影像轉換為可編輯的模組化範本，大幅縮短範本建立時間
* **增強非技術使用者的能力**：減少對設計或開發團隊的依賴 — 行銷人員能夠以最少的技術知識產生和自訂範本
* **簡化的平台移轉**：輕鬆地將外部範本或其他平台的設計轉換為AJO就緒的HTML
* **建立可重複使用的範本程式庫**：建立可擴充、品牌一致的範本程式庫，可用於多個歷程和行銷活動
* **設計逼真度**：建立完全可編輯的內容時，維持視覺與原始設計的一致性
...
* **平台移轉**：要從其他電子郵件行銷平台移轉嗎？ 將您現有的電子郵件設計轉換為可在AJO中使用的HTML範本，無需從頭重新建置
...
1. 在右側區段中，在&#x200B;**[!UICONTROL 將影像轉換為範本]**&#x200B;區段中，按一下&#x200B;**[!UICONTROL 上傳影像]**&#x200B;按鈕以選取您的影像檔案。
...
1. 選擇影像並按一下&#x200B;**[!UICONTROL 開啟]**&#x200B;以開始AI支援的轉換程式。

   >[!NOTE]
   >
   >根據影像設計的複雜性和大小，產生程式最多可能需要5分鐘。 AI處理會在背景進行，因此您可以在轉換過程中離開此畫面處理其他任務。 您不需要讓熒幕保持開啟。
...
建立並儲存內容範本後，您就可以在歷程或行銷活動中設計電子郵件時使用該範本：

在歷程或行銷活動中建立電子郵件時，請從&#x200B;**[!UICONTROL 編輯內容]**&#x200B;畫面存取電子郵件Designer 。
...
是！使用影像轉換為HTML轉換器建立的範本會自動儲存至內容範本程式庫。 您可以在歷程及行銷活動中的任何電子郵件中存取及重複使用它們。 [了解更多](../content-management/content-templates.md)
