...
不可以。 核准政策是有條件的。 只有當您想要強制審查特定行銷活動或歷程集（例如，特定團隊建立的所有已排程行銷活動）時，才需要建立原則。 如果行銷活動或歷程未套用任何原則，則建立者無需請求核准即可直接發佈。

+++

+++如果核准者不在身邊，會發生什麼情況？

請求會保持「稽核中」狀態，直到核准者對其採取行動為止。 您可以取消請求（將專案傳回「草稿」），並在有正確的核准者可用時重新提交。 管理員也可以更新核准原則以新增其他核准者。

+++

+++我可以在行銷活動或歷程處於未決核準時編輯它嗎？

不可以。 提交核准後，行銷活動或歷程即處於鎖定的「稽核中」狀態。 若要進行變更，建立者或核准者必須先取消請求。 專案會傳回「草稿」，且可在重新提交前進行編輯。
