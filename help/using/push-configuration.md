...
1. 按一下 **[!UICONTROL Create App Surfaces]** 來建立新配置。
...
在Adobe Experience Platform和中設定您的行動應用程式後 [!DNL Adobe Experience Platform Data Collection]，您現在可以在傳送推播通知至您的設定檔之前先測試它。 在此使用案例中，我們將建立歷程來定位行動應用程式，並設定會觸發推播通知的事件。
...
在 [!DNL Adobe Experience Platform Data Collection]，您需要建立訊息預設集，才能傳送推播通知 **[!DNL Journey Optimizer]**.
