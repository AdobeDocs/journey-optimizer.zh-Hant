...
**在此頁面上：**&#x200B;瞭解如何從屬性和事件建立區段定義，以建立對象，並選擇串流、批次或邊緣評估方法，以便在Adobe Journey Optimizer中鎖定目標。
...
1. 將所需欄位從左窗格拖放至中央工作區，然後視需要加以設定。
   ...
   * **屬性**：在1980年後出生的亞特蘭大、舊金山或西雅圖的設定檔。
     ...
     >[!NOTE]
     >
     >`frequencyMap`屬性不支援在區段定義中使用，且無法當作對象細分條件的一部分使用。 對於以頻率為基礎的鎖定目標，請考慮根據商業規則使用頻率限定規則。
