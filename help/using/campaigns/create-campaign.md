...
>title="行銷活動類型"
>abstract="**排程的行銷活動** 會立即執行或在指定日期執行，且旨在傳送行銷型別訊息。 **API觸發** 行銷活動是使用API呼叫執行的。 它們旨在傳送行銷訊息或交易式訊息，也就是在個人執行動作後傳送的訊息：密碼重設、購物車放棄等。"

>[!CONTEXTUALHELP]
>id="ajo_campaigns_campaign_category"
>title="行銷活動類別"
>abstract="如果您要建立排程的行銷活動， **行銷** 型別會自動選取。 針對API觸發的行銷活動，選擇是否要傳送 **行銷** 或 **異動** 訊息，即在個人執行動作後傳送的訊息：密碼重設、購物車放棄等。"
...
1. 如果您要建立排程的行銷活動， **行銷** 型別會自動選取。 針對API觸發的行銷活動，選擇是否要傳送 **行銷** 或 **異動** 訊息。」
