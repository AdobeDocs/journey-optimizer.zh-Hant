...
此頁面說明如何使用Handlebars `{{#each}}`語法在訊息個人化中，從內容來源（事件、自訂動作回應、資料集查詢和技術屬性）重複執行陣列，以及在設定歷程活動時，如何在歷程運算式語法中使用陣列。

**個意圖**

* 使用`{{#each}}`在訊息個人化中反複處理事件陣列資料（例如購物車專案、訂單專案）
* 反複處理訊息中的自訂動作回應陣列（例如產品建議）
* 在訊息中反複處理資料集查詢結果陣列
* 在單一個人化訊息中合併來自多個內容來源的資料
* 使用歷程運算式語法，將陣列值傳遞至自訂動作引數
* 在資料集查詢活動中使用陣列作為查詢索引鍵
* 套用空白陣列遞補、變數命名、效能和運算式片段範圍設定的最佳實務

>[!TAB 字彙]

* **Handlebars**： Journey Optimizer訊息個人化中使用的範本語言，適用於反複專案(`{{#each}}`)和條件式轉譯(`{{#if}}`)。 *（產品特定）*
* **`{{#each}}`協助程式**：反複運算陣列的Handlebars語法；每個反複運算會透過具名變數（例如`|product|`）公開目前專案。 *（產品特定）*
* **內容資料**：訊息傳送時可用的資料來自歷程來源（事件、自訂動作回應、資料集查詢和歷程技術屬性），而不是靜態設定檔屬性。 *（產品特定）*
* **`currentEventField`**：歷程運算式（非Handlebars）中使用的參照，用於在篩選或對應作業期間參照事件陣列中的每個專案。
* **`currentActionField`**：在歷程運算式中使用，以參照自訂動作回應集合中的每個專案。
* **`currentDataPackField`**：在歷程運算式中使用，以參考資料來源集合中的每個專案。
* **`serializeList`**：歷程運算式函式，可將值清單轉換為分隔字串（例如以逗號分隔），以作為查詢引數。
* **補充識別碼**：歷程層級的識別碼，可區分由相同設定檔觸發的並行歷程執行個體；用來篩選陣列以取得與目前執行個體相關的專案。

>[!TAB 術語]

* **正式名稱：** Handlebars反複專案 — 變體： `{{#each}}`回圈，每個回圈，陣列反複專案
* **請勿混淆：** Handlebars `{{#each}}`語法（用於反複運算及顯示的訊息內容）≠歷程運算式語法（用於歷程活動設定 — 使用類似`first`、`all`、`serializeList`的函式）
* **請勿混淆：** `currentEventField` （事件陣列上的歷程運算式） ≠ `currentActionField` （自訂動作回應集合） ≠ `currentDataPackField` （資料來源集合）
* **請勿混淆：** `@index` / `@first` / `@last` （Handlebars特殊變數，僅可在訊息內容中的`{{#each}}`回圈中使用）≠ `first` / `head`函式（擷取單一專案的歷程運算式函式，用於歷程活動設定）

>[!TAB 護欄與限制]

* 歷程無法建立動態回圈，因為每個陣列專案會有一個動作節點執行多次 — 這是為了避免效能問題而設計。 改為將整個陣列或序列化清單傳遞至單一自訂動作。
* 將事件裝載量維持在總共50KB以下。
* 自訂動作回應裝載應小於100KB。
* 限制資料集查詢索引鍵和傳回實體的數目以提高效能。
* 運算式片段無法接收回圈範圍的變數（例如，目前的`{{#each}}`反複專案）做為引數 — 這是已知的限制。 請改用全域變數或內嵌邏輯。
* 數值事件ID必須在運算式路徑中以反引號括住（例如`` context.journey.events.`1697323153`.fieldName ``）；如果沒有反引號，PQL剖析器會產生語法錯誤。

>[!TAB 常見問題集]

**問：使用陣列時，Handlebars語法和歷程運算式語法有何差異？**

Handlebars `{{#each}}`已用於訊息內容中，以供反複專案及顯示。 歷程運算式語法（使用如`first`、`all`和`serializeList`等函式）用於歷程活動設定（例如自訂動作引數、條件）。 它們是不同內容中所使用的不同語法。

**問：我可以回圈歷程動作節點，讓每個陣列專案執行一次嗎？**

沒有。 歷程無法建立動態回圈，因為每個專案都會多次執行動作節點。 請改為將整個陣列或序列化清單傳遞至處理所有專案的單一自訂動作，或使用外部彙總。

**問：我可以將目前的回圈專案傳遞給`{{#each}}`回圈內的運算式片段嗎？**

沒有。 運算式片段無法接收回圈範圍的變數做為引數。 使用在回圈外定義的全域變數，或直接在回圈內包含個人化邏輯，而不是使用片段。

**問：陣列為空時如何顯示遞補內容？**

使用`{{#each}}`區塊中的`{{else}}`子句。 當陣列沒有專案時，`{{else}}`內的內容會呈現。

**問：`{{#each}}`回圈內的`@index`、`@first`和`@last`代表什麼？**

這些是特殊的Handlebars變數，只能在訊息內容的`{{#each}}`回圈中使用： `@index`是以0為基礎的目前反複專案索引，`@first`是第一個反複專案的true，而`@last`是最後一個反複專案的true。
