...
>[!IMPORTANT]
>
>並非所有ISP都提供傳統FBL，例如Gmail。 Gmail不提供個人層級的意見回饋，並且無法用於追蹤個別收件者的垃圾郵件投訴，而是專注於其Google Postmaster工具中的彙總層級報告。 [了解更多](#providers-no-fbl)


所有Adobe客戶都會自動註冊下列傳統ISP的FBL。
...
Adobe會執行定期稽核，以確保新增最新可用的FBL。

### 沒有每個收件者FBL的提供者 {#providers-no-fbl}

並非所有ISP都提供傳統FBL。 幾個主要信箱提供者沒有傳回每個收件者的垃圾郵件投訴，這表示使用這些提供者的收件者應該會在隱藏清單中缺少專案。

| 信箱提供者 | 每位收件者的投訴FBL？ |
|---|---|
| **Gmail / Google Workspace** | 否 — 僅透過Google Postmaster工具彙總報表（意見回饋ID；不是每個收件者）。 [了解更多](https://support.google.com/a/answer/6254652?hl=en){target="_blank"} |
| **Apple iCloud** (icloud.com， me.com， mac.com) | 否 — [深入瞭解](https://support.apple.com/en-us/102322){target="_blank"} |
| **公司Microsoft 365 / Exchange Online** | 否 — 垃圾郵件報告程式(JMRP)僅涵蓋消費者Outlook.com網路；來自公司M365環境的租使用者垃圾郵件報告不會傳回給寄件者。 [了解更多](https://learn.microsoft.com/en-us/answers/questions/5787589/does-the-junk-mail-reporting-program-(jmrp)-send-c){target="_blank"} |

這些提供者共同代表大部分的消費者和企業收件匣。 由於這些服務都沒有傳回每位收件者的申訴，因此使用這些服務將郵件標籤為垃圾郵件的收件者&#x200B;**不會**&#x200B;自動出現在隱藏清單中。
