...
title: 存取及訂閱系統警示
description: 瞭解如何在Adobe Journey Optimizer中存取、訂閱及管理系統警示。 使用主動警報通知來監視歷程和行銷活動生命週期、自訂動作錯誤、設定檔問題和電子郵件傳遞能力。
...
**在此頁面上：**&#x200B;瞭解如何存取、訂閱及管理Adobe Journey Optimizer中的系統警示，以主動監控歷程與行銷活動生命週期事件、自訂動作錯誤、設定檔問題以及電子郵件傳遞能力。
...
警報會透過電子郵件和/或Journey Optimizer通知中心（右上角的鈴鐺圖示）的應用程式內通知傳送。 在[Adobe Experience Cloud偏好設定](../start/user-interface.md#in-product-uc)中設定您偏好的傳遞管道。

![](assets/alert-pulse.png)

Journey Optimizer提供一次性警報（資訊性事件，例如「歷程已發佈」）和重複警報（監視臨界值）。 重複警報會持續評估和通知，直到條件解決為止。

為避免通知疲勞使值波動，即使條件持續存在，警報也會在1小時後自動解決。 這可防止在量度暫留在臨界值附近時持續收到通知。
...
系統警示可從&#x200B;**[!UICONTROL 管理]**&#x200B;下的&#x200B;**[!UICONTROL 警示]**&#x200B;功能表進行管理。 Adobe Experience Platform提供數個您可啟用的預先定義警報規則，包括歷程和通道設定的[!DNL Adobe Journey Optimizer]特定警報。
...
## 可用警示 {#available-alerts}

Journey Optimizer提供預先設定的警報規則，可監控歷程、行銷活動和頻道設定的特定方面。 您不需要建立這些警報，這些警報是現成可用的，可透過訂閱啟用。
...
瀏覽以下索引標籤以檢閱歷程、行銷活動和頻道設定警報。 在索引標籤中選取警報名稱，以展開其完整說明。

>[!BEGINTABS]

>[!TAB 歷程警示]

此標籤會列出使用者介面中可用的所有歷程通知。 選取警報名稱以展開其完整說明和指引。

➡️ [進一步瞭解警示傳遞](#subscribe-alerts) | [瞭解如何管理警示訂閱](#subscribe-alerts) | [瞭解如何監視和解決歷程問題](../building-journeys/troubleshooting-execution.md)
...
➡️ [設定讀取對象](../building-journeys/read-audience.md)

➡️ [定義和使用對象](../audience/about-audiences.md)

+++

+++ 超過設定檔捨棄率

如果過去5分鐘內捨棄的設定檔與輸入的設定檔之比超過臨界值，此警報會發出警告。 預設臨界值設為20%，但您可以定義自訂臨界值。
...
➡️ [疑難排解歷程問題](../building-journeys/troubleshooting.md)

➡️ [定義自訂警示臨界值](#custom-threshold)

+++

+++ 超出自訂動作錯誤率

如果自訂動作錯誤與過去5分鐘成功HTTP呼叫的比率超過臨界值，此警報會警告您。 預設臨界值設為20%，但您可以定義自訂臨界值。
...
➡️ [在測試模式中驗證](../building-journeys/testing-the-journey.md)

➡️ [檢查歷程即時報告](../reports/journey-live-report.md)

➡️ [設定自訂動作](../action/about-custom-action-configuration.md)
...
如果過去5分鐘內錯誤設定檔與輸入設定檔的比率超過臨界值，此警報會警告您。 預設臨界值設為20%，但您可以定義自訂臨界值。

按一下警示的名稱以檢查警示詳細資訊和組態。

若要疑難排解設定檔錯誤，您可以查詢步驟事件中的資料，以瞭解設定檔在歷程中失敗的位置和原因。

➡️ [使用歷程步驟事件](../reports/journey-step-events-overview.md)
...
➡️ [發佈歷程](../building-journeys/publish-journey.md)
...
此警報會在歷程完成後通知您。 「已完成」的定義會因歷程型別而異。

這是資訊性警報，可幫助您追蹤歷程完成。 沒有解決標準，因為這是一次性通知。

➡️ [瞭解歷程何時完成](../building-journeys/end-journey.md#journey-finished-definition)
...
觸發上限時，這表示在定義的時段內已達到API呼叫的最大數量，且有更多的呼叫正在節流或排入佇列。
...
在&#x200B;**動作**&#x200B;和&#x200B;**API觸發的**&#x200B;行銷活動中發生重要生命週期或傳遞事件時，系統警示會通知您。 在下方選取警報名稱以展開其說明。

➡️ [進一步瞭解警示傳遞](#subscribe-alerts) | [瞭解如何管理警示訂閱](#subscribe-alerts) | [瞭解如何監視和解決行銷活動問題](../campaigns/manage-campaigns.md)

+++ 行銷活動已啟用

當行銷活動已成功&#x200B;**啟動** （發佈/啟動已完成）時通知您。

➡️ [檢閱並啟用動作行銷活動](../campaigns/review-activate-campaign.md)

➡️ [檢閱並啟用API觸發的行銷活動](../campaigns/review-activate-api-triggered-campaign.md)

+++

+++ 行銷活動啟用失敗

在&#x200B;**啟用**&#x200B;促銷活動&#x200B;**失敗**&#x200B;時通知您。 使用此警報來及早偵測設定或技術問題，並在客戶受影響之前重試或修正行銷活動。

➡️ [檢閱並啟用動作行銷活動](../campaigns/review-activate-campaign.md)

➡️ [檢閱並啟用API觸發的行銷活動](../campaigns/review-activate-api-triggered-campaign.md)

➡️ [檢閱必要條件和行銷活動設定](../campaigns/get-started-with-campaigns.md)

+++

+++ 行銷活動已停止

當行銷活動已成功&#x200B;**停止**&#x200B;時通知您（例如，在手動停止後，或當執行根據您的工作流程完成時）。

➡️ [瞭解行銷活動狀態](../campaigns/manage-campaigns.md#statuses)

➡️ [停止動作行銷活動](../campaigns/manage-campaigns.md#stop)

+++

+++ 行銷活動停止失敗

當&#x200B;**停止**&#x200B;作業&#x200B;**失敗**&#x200B;時通知您。 在重試之前，請調查行銷活動狀態和UI中顯示的任何錯誤。

➡️ [瞭解行銷活動狀態](../campaigns/manage-campaigns.md#statuses)

➡️ [解譯錯誤指標](../campaigns/manage-campaigns.md#error-indicators)
...
當行銷活動的&#x200B;**訊息傳遞**&#x200B;已&#x200B;**開始**&#x200B;時通知您（執行已移至傳遞階段）。

➡️ [檢閱行銷活動報告(CJA)](../reports/campaign-global-report-cja.md)

➡️ [管理行銷活動](../campaigns/manage-campaigns.md)

+++

+++ 行銷活動傳遞已完成

當行銷活動的&#x200B;**訊息傳遞**&#x200B;已成功&#x200B;**完成**&#x200B;時通知您。
...
當行銷活動&#x200B;**的**&#x200B;訊息傳遞&#x200B;**失敗**&#x200B;時通知您。 檢閱行銷活動報告、執行記錄和管道設定以進行疑難排解。

➡️ [檢閱行銷活動報告(CJA)](../reports/campaign-global-report-cja.md)

➡️ [解譯錯誤指標](../campaigns/manage-campaigns.md#error-indicators)

➡️ [設定頻道傳遞](../configuration/channel-surfaces.md)

+++

>[!TAB 協調的行銷活動通知]

系統警報會通知您有關已協調行銷活動中的重要事件。 在下方選取警報名稱以展開其說明。

➡️ [進一步瞭解警示傳遞](#subscribe-alerts) | [瞭解如何管理警示訂閱](#subscribe-alerts) | [瞭解如何監視和解決協調的行銷活動問題](../orchestrated/start-monitor-campaigns.md#logs-tasks)

+++ 協調的行銷活動錯誤

當協調的行銷活動在執行期間遇到錯誤時，通知您。
...
當來自已協調行銷活動的傳遞成功完成時通知您。

+++

+++ 協調的行銷活動開始

每次開始執行協調的行銷活動時都會通知您。 對於循環協調的行銷活動，每次循環都會觸發此警報。 此警報與發佈警報不同：已排程的協調行銷活動無需啟動即可發佈。
...
當協調的行銷活動完成時，在所有遞回完成後通知您。

+++

>[!TAB 頻道設定警示]

此標籤中會列出使用者介面中可用的通道組態監視警示。 選取警示名稱以展開修正步驟與附註。

➡️ [進一步瞭解警示傳遞](#subscribe-alerts) | [瞭解如何管理警示訂閱](#subscribe-alerts) | [瞭解如何監視和解決通道設定問題](../configuration/channel-surfaces.md)
...
1. 按一下警示以導向介面[!DNL Journey Optimizer]中受影響的[子網域](../configuration/delegate-subdomain.md)。
...
>[!IMPORTANT]
>
>此警示僅適用於使用[自訂子網域](../configuration/delegate-custom-subdomain.md)委派型別的&#x200B;**電子郵件**&#x200B;通道設定。<!--Other channel types (such as SMS, push, or in-app) are not covered by this alert.-->

如果系統稽核偵測到電子郵件通道設定問題，則會觸發此警報。 這些問題可能包括設定錯誤的通道設定、無效的DNS設定、隱藏清單問題、IP不一致或任何其他可能影響電子郵件傳送的錯誤。

如果您收到這類警示，以下列出解決步驟。

1. 按一下警示以導向至[!DNL Journey Optimizer]介面中受影響的[電子郵件通道設定](../email/get-started-email-config.md)。
...
1. 按一下警示以在[!DNL Journey Optimizer]中開啟受影響的[子網域](../configuration/delegate-subdomain.md)。
...
1. 從可用選項中選取要啟用的警示：
   * [超出輪廓捨棄率](#available-alerts)
   * [超出自訂動作錯誤率](#available-alerts)
   * [超出輪廓錯誤率](#available-alerts)
   * [已發佈歷程](#available-alerts)
   * [歷程已完成](#available-alerts)
   * [自訂動作上限已觸發](#available-alerts)
...
您可以設定[歷程警示](#available-alerts)的臨界值。 超過預設值的臨界值警報為20%。
