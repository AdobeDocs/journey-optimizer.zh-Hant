...
**id 、 instanceid和profileid之間的區別**

* id:對於所有步驟事件條目唯一。 兩個不同的步驟事件不能具有相同的ID。
* 實例ID:對於行程執行中與配置檔案關聯的所有步驟事件，實例ID相同。 如果配置檔案重新進入行程，將使用其他instanceId。 對於重新輸入實例的所有步驟事件（從開始到結束），此新實例Id將相同。
* 配置檔案ID:配置檔案的標識與journey命名空間相對應。

## 基本使用案例/常見查詢 {#common-queries}

**在特定時間範圍內輸入行程的配置檔案數**

此查詢提供在給定時間範圍內輸入給定行程的不同配置檔案數。
...
**在特定時間範圍內的特定行程中對特定配置檔案的操作**

_資料湖查詢_

此查詢按時間順序返回指定時間的給定配置檔案和行程的所有步驟事件和服務事件。
...
例如，這些查詢可用於估計在等待活動中花費的時間。 這允許您確保正確配置等待活動。
...
Journey Step Events資料集包含所有stepEvents和serviceEvents。 stepEvents用於報告，因為它們與活動（事件、操作等）相關 在旅途中的檔案。 serviceEvents儲存在同一資料集中，它們指示用於調試的附加資訊，例如實驗事件丟棄的原因。

下面是查詢示例，用於檢查serviceEvent的詳細資訊：
