...
例如，這些查詢可用於估計等待活動所花費的時間。 這可讓您確定等候活動已正確設定。
...
Journey Step Events資料集包含所有stepEvents和serviceEvents。 stepEvents用於報告，因為它們與活動（事件、動作等）相關 歷程中的設定檔。 serviceEvents會儲存在相同的資料集中，且會指出除錯用途的其他資訊，例如體驗事件捨棄的原因。

以下是檢查serviceEvent詳細資訊的查詢範例：
