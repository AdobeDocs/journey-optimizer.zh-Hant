...
* **maxInstanceStackEventsHadped**：歷程執行階段已達到指定歷程版本每個設定檔事件棧疊的內部10個事件限制。
  ...
  **疑難排解**：減少可能經常重新觸發的路徑上長期執行的步驟、中斷或重複上游事件，並將長期案例分割成多個歷程。 這是安全護欄，且無法設定限制；會捨棄其他事件，直到棧疊排清為止。 如需詳細指引，請參閱[由於封鎖歷程執行個體而捨棄的事件](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)。
...
* [查詢範例 — 商業規則查詢](query-examples.md#business-rules-queries)。
* [內建方案字典](https://experienceleague.adobe.com/tools/ajo-schemas/schema-dictionary.html?lang=zh-Hant)
