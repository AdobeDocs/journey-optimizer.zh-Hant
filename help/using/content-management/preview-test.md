...
一旦定義內容，您就可以在傳送訊息之前，先預覽內容。 這是確保內容和個人化設定都準確無誤的重要步驟。

您也可以測驗電子郵件訊息給特定收件者或訂閱者的傳遞，以便進行測試和驗證，並檢查郵件在熱門桌面、行動裝置和網頁型用戶端中的轉譯效果。 此外，您也可以評估一般內容品質方面，例如可讀性和有效性。 [了解更多關於內容品質驗證的資訊](brands-score.md#validate-quality)
...
* **使用測試設定檔資料測試內容**
  ...
  使用測試設定檔，預覽內容，傳送電子郵件校樣，同時檢查電子郵件呈現。 如果您已新增個人化欄位，就可以使用測試輪廓資料，檢查這些欄位的顯示方式。 如需詳細資訊，請參閱以下區段：
...
* **使用內容資料執行個人化** - 當預覽訊息或傳送校樣時，只會顯示出設定檔個人化資料。 根據內容資料 (例如事件資訊) 進行的個人化，只能在歷程情境中測試。 可到[此使用案例](../personalization/personalization-use-case.md)，了解使用方法。
...
* **映象頁面限制** — 映象頁面連結在校訂中無效，包括從測試設定檔或透過範例輸入資料測試的內容變數傳送的校訂。 校樣可讓您驗證內容、轉譯和個人化，但映象頁面連結則不行，因為此連結只會在最終訊息中啟動。 [了解更多](../email/message-tracking.md#mirror-page)。

## 作法影片 {#video-preview}

了解如何使用測試輪廓來測試各個收件匣裡的電子郵件呈現，根據測試輪廓預覽您的個人化電子郵件，同時傳送校樣。
