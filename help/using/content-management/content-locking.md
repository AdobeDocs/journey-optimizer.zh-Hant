...
Journey Optimizer允許您通過鎖定整個模板或特定結構和元件來鎖定電子郵件模板中的內容。 這可以防止無意中進行編輯或刪除，從而讓您能夠更好地控制模板定制，並提高電子郵件活動的效率和可靠性。

>[!IMPORTANT]
>
>內容鎖定是作者的編輯器級功能，不保證在通過API導入或建立時內容將保持未編輯狀態。

可以在&#x200B;**結構**&#x200B;級別或在&#x200B;**元件**&#x200B;級別應用內容鎖定。 以下是在將內容鎖定到模板中時在結構和元件級別應用的主要原則：
...
1. 從&#x200B;**[!UICONTROL 模式]**&#x200B;下拉式清單中，選取範本所需的鎖定模式：
   ...
   * **[!UICONTROL 內容鎖定]**：鎖定範本中內容的特定區段。 預設情況下，所有結構和元件都可編輯。 然後，可以有選擇地鎖定單個元素。
...
1. 選取鎖定模式後，您可以定義哪些結構和/或元件要鎖定（如果您已選取&#x200B;**[!UICONTROL 內容鎖定]**&#x200B;模式）：
   ...
   * [瞭解如何鎖定元件](#lock-components)
   ...
   如果選擇&#x200B;**[!UICONTROL 只讀]**&#x200B;模式，請繼續完成並像往常一樣保存模板。

通過選擇模板主體，您可以隨時調整&#x200B;**[!UICONTROL Governance]**&#x200B;設定。 要執行此操作，請按一下位於右側窗格頂部的導航欄中的&#x200B;**[!UICONTROL Body]**&#x200B;連結。
...
1. 在&#x200B;**[!UICONTROL 鎖定類型]**&#x200B;下拉清單中，選擇&#x200B;**[!UICONTROL 鎖定]**。
...
在鎖定結構後，不能複製或在其中添加其他內容元件或片段。 依照預設，鎖定結構內的所有元件也會鎖定。 要使元件在鎖定的結構中可編輯，請執行以下操作：
...
要輕鬆識別模板中鎖定的結構和元件，請使用左側菜單中的&#x200B;**[!UICONTROL 導航樹]**。 此菜單提供所有模板元素的可視概覽，突出顯示帶鎖定表徵圖的鎖定項目和帶鉛筆表徵圖的可編輯項目。

在下面的示例中，為模板正文啟用了governance。 *結構2*&#x200B;已鎖定，*元件1*&#x200B;可編輯，而&#x200B;*結構3*&#x200B;已完全鎖定。
...
例如，在下面的模板中，除了已鎖定的頂部影像外，所有區域都是可編輯的，這意味著您不能編輯或刪除它。

![](assets/template-lock-highlight.png)

以下各節提供了可應用的不同類型鎖定的詳細資訊：
