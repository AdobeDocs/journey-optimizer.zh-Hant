...
>title="治理"
>abstract="通過鎖定整個範本或特定結構和元件來切換治理以鎖定範本中的內容。 這樣做讓您可以防止無意間不小心編輯內容，或將內容刪除，讓您更能掌控範本自訂，進而提高電子郵件行銷活動的效率和可靠性。"
...
>title="啟用內容添加部分"
>abstract="開啟此選項以進一步定義使用者與範本的互動方式。 選擇“ **允許結構內容添加** ”以允許使用者在現有結構之間添加結構，並在可編輯結構中添加內容元件或片段。 **允許添加內容僅** 允許使用者在可编辑結構中添加內容元件或片段，而不能添加或重複結構。"
...
Journey Optimizer 允許您通過鎖定整個範本或特定結構和元件來鎖定電子郵件範本中的內容。 這樣做讓您可以防止無意間不小心編輯內容，或將內容刪除，讓您更能掌控範本自訂，進而提高電子郵件行銷活動的效率和可靠性。

>[!AVAILABILITY]
>
>有權建立內容範本的用戶可以啟用內容鎖定。

內容鎖定可以在結構&#x200B;**級別或**&#x200B;元件&#x200B;**級別應用**。以下是在範本中鎖定內容時在結構和元件級別應用的主要原則。

* 當結構被鎖定時：

   * 默認情況下，該結構中的所有內容也會被鎖定。
   * 無法新增任何內容至結構。
...
>title="請在元件中使用特定鎖定機制"
>abstract="若要鎖定範本中的元件，請開啟&#x200B;**使用特定鎖定**&#x200B;選項。 從「 **[!UICONTROL 鎖定類型]** 」下拉清單中清單，選擇首選鎖定選項：「 **可編輯內容鎖定僅** 允許您鎖定元件的樣式，但允許內容編輯，而 **」已鎖定** “則完全鎖定元件的內容和樣式。"
...
1. 默認情況下，使用者無法刪除鎖定的元件。 您可以使用允許 **[!UICONTROL 刪除]** 選項來啟用刪除。

### 識別鎖定的內容 {#identify}

要輕鬆識別範本內的鎖定結構和元件，請使用 **[!UICONTROL 左側功能表中的導航樹]** 。 此功能表提供所有範本元素的可視化概述，使用鎖定圖示突出顯示鎖定的專案，用鉛筆圖示突出顯示可編輯的專案。

在下面的示例中，為範本正文啟用了治理。 *結構 2* 已鎖定 *，元件 1* 可編輯，而 *結構 3* 完全鎖定。
...
>title="強調可編輯區域"
>abstract="根據應用於範本的鎖定類型，您可以對範本的結構和元件執行不同的操作。 要快速識別範本中的所有可編輯區域，請打開 **[!UICONTROL 高亮顯示可編輯區域]** 選項。"

使用具有鎖定內容的範本時， **[!UICONTROL “已啟用治理”]** 消息將显示在右窗格中。
