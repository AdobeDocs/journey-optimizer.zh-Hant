...
自訂動作是技術使用者定義的其他動作，可供行銷人員使用。 設定後，它們會顯示在歷程的左側浮動視窗中，位於 **[!UICONTROL Action]** 類別。 深入了解 [本頁](../building-journeys/about-journey-activities.md#action-activities).
...
1. 設定 **[!UICONTROL Authentication]** 區段。 此設定與資料來源的設定相同。  請參閱 [本節](../datasource/external-data-sources.md#custom-authentication-mode).
...
1. 在 **[!UICONTROL URL]** 欄位，指定外部服務的URL:
   ...
   * 如果URL包含動態路徑，則僅輸入URL的靜態部分，即配置、主機、埠，以及路徑的靜態部分（可選）。
      ...
      將自訂動作新增至歷程時，您會指定URL的動態路徑。 [深入了解](../building-journeys/using-custom-actions.md).
   >[!NOTE]
   >
   >基於安全考量，強烈建議您為URL使用HTTPS配置。 不允許使用非公開的Adobe位址和IP位址。
   >
...
1. 選取呼叫 **[!UICONTROL Method]**:它可以是 **[!UICONTROL POST]** 或 **[!UICONTROL PUT]**.
   ...
   >[!NOTE]
   >
   > 此 **刪除** 方法不受支援。 如果您需要更新現有資源，請選取 **PUT** 方法。
...
1. 在 **[!UICONTROL Headers]** 一節，定義要傳送至外部服務的要求訊息的HTTP標題：
   ...
   1. 如果您已選取 **[!UICONTROL Constant]**，然後輸入常數值。
      ...
      如果您已選取 **[!UICONTROL Variable]**，則會在將自訂動作新增至歷程時指定此變數。 [深入了解](../building-journeys/using-custom-actions.md).
...
* 變數表示參數的值會有所不同。 行銷人員在歷程中使用此自訂動作時，可免費傳遞所需值，或指定要擷取此參數值的位置（例如從事件、Adobe Experience Platform等）。 在此情況下，切換常數/變數右側的欄位，就是行銷人員在歷程中看到的為此參數命名的標籤。
