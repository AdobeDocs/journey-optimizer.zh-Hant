...
**在此頁面上：**&#x200B;瞭解如何在「最佳化」活動中使用條件，以根據資料來源、時間、日期、百分比分割、設定檔上限或對象會籍來建立多個歷程路徑。
...
>title="關於簡單運算式編輯器"
>abstract="簡單運算式編輯器模式可讓您根據欄位組合執行簡單查詢。 所有可用的欄位都會顯示在螢幕的左側。 欄位可拖放至主要區域中。 若要合併不同的元素，則需要將它們互相連接以建立不同的群組和/或群組層級。 然後，邏輯運算子便會合併同一層級上的元素。"
...
我們來看看兩個路徑條件：「這個人是VIP」和「這個人是男性」。 如果人員符合兩個條件，系統會選擇第一個路徑，因為該路徑高於第二個路徑。 若要變更此優先順序，請將您的活動移至不同的垂直順序。
...
1. 在左側，展開&#x200B;**[!UICONTROL 對象]**&#x200B;節點。 拖放您要用於條件的對象。 依預設，對象的條件為true。
   ...
   運算式編輯器中的![對象節點，用於選取[!DNL Adobe Experience Platform]對象](assets/segment4.png){width=80%}

   >[!NOTE]
   >
   >請注意，只有具有&#x200B;**已實現**&#x200B;對象參與率的個人
   >狀態將被視為對象的成員。 如需如何操作的詳細資訊
   >評估對象，請參閱[Segmentation Service檔案](https://experienceleague.adobe.com/docs/experience-platform/segmentation/tutorials/evaluate-a-segment.html?lang=zh-Hant#interpret-segment-results){target="_blank"}。
   >
