...
* 對於週期性歷程：如果設定檔處於對象預期狀態，則會在每次週期性進入歷程。 如果他們仍然在上一個循環的歷程中，他們將從頭重新開始。

在商業事件歷程中，從 **讀取對象** 活動：瞭解此歷程是根據接收業務事件，如果設定檔符合預期對象中的資格，他們會針對收到的每個業務事件進入歷程，這表示此設定檔可能同時於同一歷程中多次，但位於不同業務事件的內容中。
