...
1. **設定動作**:Journey Optimizer訊息功能是內建的，您只需要設計內容並發佈訊息即可。 請參閱[本節](../get-started-content.md)。如果您使用協力廠商系統來傳送訊息，則可建立自訂動作。 了解更多資訊 [節](../action/action.md). 此步驟由&#x200B;**技術使用者**&#x200B;執行。
...
1. 拖放個人將遵循的後續步驟。 例如，您可以新增條件，後接訊息。 若要進一步了解活動，請參閱 [本節](using-the-journey-designer.md).
...
歷程「結束」時，其狀態為 **[!UICONTROL Closed]**. 歷程將停止讓新人進入歷程。 已在歷程中的人員會正常完成歷程。
...
若要啟用「突發」模式，請開啟您的歷程，然後按一下右上角的鉛筆圖示，以存取歷程的屬性。 然後，啟用 **啟用突發模式** 切換。
...
歷程關閉時（基於上述任何原因），其狀態都會是 **[!UICONTROL Closed]**. 歷程將停止讓新人進入歷程。 已在歷程中的人員會正常完成歷程。 在30天的預設全域逾時後，歷程會切換至 **已完成** 狀態。 看這個 [節](../building-journeys/journey-gs.md#global_timeout).
...
關閉時，歷程會具有狀態 **[!UICONTROL Closed]**. 在30天的預設全域逾時後，歷程會切換至 **已完成** 狀態。 看這個 [節](../building-journeys/journey-gs.md#global_timeout).
