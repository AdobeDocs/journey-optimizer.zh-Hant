...
您必須發佈歷程才能啟用它，並讓新設定檔可以進入它。 發佈歷程之前，請確認其有效且沒有錯誤。 您無法發佈含有錯誤的歷程。 Journey Optimizer也會在儲存和發佈時間驗證裝載大小，以保留穩定性和效能。 在此[區段](publish-journey.md)中進一步瞭解歷程發佈，並參閱[歷程裝載大小驗證](../start/guardrails.md#journey-payload-size)。
