...
* **TL；DR：**&#x200B;此頁面說明如何發佈Adobe Journey Optimizer歷程、管理歷程版本，以及瞭解歷程上線後套用的限制。
...
**字彙表：**
* **歷程版本**：歷程的編號反複專案；建立新版本以修改即時歷程，而不會中斷進行中的設定檔&#x200B;*（產品特定）*
* **已關閉狀態**：發佈新版本時，先前歷程版本會自動進入的狀態；新設定檔無法進入已關閉歷程&#x200B;*（產品特定）*
* **核准原則**：選擇性治理工作流程，需要明確核准才能發佈歷程&#x200B;*（產品專屬）*

**護欄：**
* 含有錯誤的歷程無法發佈。
* Journey Optimizer會在儲存和發佈時間驗證歷程裝載總大小；如果超過限制，可能會封鎖發佈。
* 發佈後，歷程處於唯讀模式；只能編輯標籤、說明和歷程名稱。
* 新版本只能從歷程的最新版本建立。
* 歷程停止時會永久停止；必須重複歷程才能再次執行。
* 自首次發佈後，730天內皆可存取Assets和已傳送內容中的影像，到期後須重新發佈。
* 如果歷程訊息中使用的優惠決定變更，必須取消發佈並重新發佈歷程。
* 特定護欄適用於歷程版本設定（請參閱護欄頁面）。
