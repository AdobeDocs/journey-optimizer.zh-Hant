...
* **名稱空間需求：** **[!UICONTROL 跳轉]**&#x200B;活動僅適用於使用名稱空間的歷程。 您只能跳至使用與來源歷程相同名稱空間的歷程。

* **目標歷程限制：**&#x200B;您無法跳至以&#x200B;**對象資格**&#x200B;事件或&#x200B;**讀取對象**&#x200B;開始的歷程。

* **對象資格支援（漸進式轉出）：**&#x200B;如果歷程從對象資格開始，且跳轉重新導向以事件為基礎的開始歷程，則您可在相同歷程中擁有&#x200B;**[!UICONTROL 跳轉]**&#x200B;活動和&#x200B;**對象資格**&#x200B;事件。 此功能正在逐步推出。 如果您的環境中沒有看到此內容，可能是因為您仍在對象資格中使用批次對象。

* **讀取對象限制：**&#x200B;您無法在同一歷程中擁有&#x200B;**[!UICONTROL 跳轉]**&#x200B;活動和&#x200B;**讀取對象**&#x200B;事件。

* **多重跳躍：**&#x200B;您可以視需要在歷程中包含不限數量的&#x200B;**[!UICONTROL 跳躍]**&#x200B;活動。 在&#x200B;**[!UICONTROL 跳轉]**&#x200B;之後，您可以新增任何需要的活動。 您可以視需要儘量使用跳躍層級（例如，歷程A跳至歷程B，然後跳至歷程C）。 目標歷程也可以包含所需數量的跳轉&#x200B;**[!UICONTROL 跳轉]**&#x200B;活動。

* **防止回圈：**&#x200B;不支援回圈模式。 無法將兩個或多個歷程連結在一起，因為這會產生無限回圈。 **[!UICONTROL 跳轉]**&#x200B;活動設定畫面會防止您執行此動作。
...
1. 在&#x200B;**目標歷程**欄位內按一下。
清單會顯示草稿、即時或測試模式中的所有歷程版本。 使用不同名稱空間或以**讀取對象**&#x200B;事件開始的歷程無法使用。 以&#x200B;**對象資格**&#x200B;事件開始的歷程僅適用於您的來源歷程也以「對象資格」開始的歷程（而且貴組織中新的「對象資格」節點已棄用批次對象）。 也會篩選掉會建立回圈模式的目標歷程。
...
* 無法在同一歷程中使用跳轉活動和讀取對象事件
* 僅當歷程以對象資格開始並跳至事件型歷程（取決於功能標幟轉出）時，才能在同一歷程中使用跳轉活動和對象資格事件
...
* **問：我可以跳至讀取對象歷程嗎？**  — 否；您無法跳至以「讀取對象」事件開始的歷程。 您也無法跳至以對象資格事件開始的歷程。 不過，如果您的來源歷程從對象資格事件開始，您可以跳至事件型歷程（此功能正在逐步推出，可能尚未在所有組織中提供）。
