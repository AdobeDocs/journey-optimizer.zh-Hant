...
1. 使用相關購物車詳細資料設定「購物車放棄」事件
2. 將事件新增為歷程進入點
3. 立即進行電子郵件動作
4. 設計包含個人化產品詳細資料的購物車復原電子郵件
5. 發佈歷程

每當收到購物車放棄事件時，歷程就會自動觸發，並即時傳送復原電子郵件。
...
使用具有逾時的&#x200B;**[!UICONTROL 回應]**&#x200B;事件建立事件觸發歷程：

1. **設定「購物車已放棄」事件**：已新增專案但未在時間範圍內完成結帳時觸發
2. **傳送初始訊息** （選擇性）：電子郵件確認購物車專案
3. **在頻道動作之後立即新增[!UICONTROL 回應]事件**：設定它以接聽購買事件
4. **設定逾時期間**：在&#x200B;**[!UICONTROL 回應]**&#x200B;事件上定義逾時（例如1-2小時），讓客戶有時間自然完成
5. **建立兩個路徑**：
...
請參閱[選擇驗證方法](choose-validation-method.md)以取得完整比較，或深入瞭解[模擬](simulate-journey-gs.md)、[測試模式](testing-the-journey.md)及[試用](journey-dry-run.md)。
...
深入瞭解[條件](conditions.md)、[訊息動作](journey-action.md)和[頻道選擇](../channels/gs-channels.md)。
