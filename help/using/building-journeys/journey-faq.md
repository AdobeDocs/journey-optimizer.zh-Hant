...
Adobe Journey Optimizer支援三種受眾型別，各有不同的特性和護欄：
...
* **讀取對象歷程**：支援所有三種對象型別；當歷程執行時會發生批次匯出
* **對象資格歷程**：建議串流對象；批次對象延遲資格偵測；不支援上傳對象
...
深入瞭解[對象](../audience/about-audiences.md)、[建立區段](../audience/creating-a-segment-definition.md)和[自訂上傳對象](../audience/custom-upload.md)。
