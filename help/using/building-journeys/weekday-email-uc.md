...
增強您的工作流程，並處理更複雜的業務需求：

* **複雜營業時間**：將公式擴充至基本工作日檢查之外的假日、時區或特定營業時間。
* **自訂傳送時間**：調整等待公式中的小時引數(H)，以符合您的最佳傳送時間。 例如，如果上午10點顯示較好的參與率，請將公式變更為使用小時10。
* **多重時區支援**：為不同的地理區域建立個別歷程，以確保每一位收件者的當地時區週一傳遞。

### 時區管理

`now()`函式和歷程執行使用在歷程層級設定的時區。 請考量下列要點：

* **歷程時區**：發佈前，請在歷程屬性中設定此專案，確保歷程時區符合您的需求。 [進一步瞭解時區管理](timezone-management.md)
...
* **讀取對象歷程**： [排程讀取對象](read-audience.md#schedule)以在對象認為合適的時間觸發。 清晨執行（例如，上午6:00）在商務通訊中很常見。
...
[進一步瞭解測試歷程](testing-the-journey.md)

### 重新進入和頻率

對於週期性行銷活動，請仔細管理設定檔重新輸入：

* **設定重新進入**：適當地設定&#x200B;**[!UICONTROL 重新進入]**&#x200B;設定。 [進一步瞭解重新進入設定](entry-management.md)
* **一致的行為**：如果設定檔可以重新進入歷程，則每次都會受到星期幾檢查，確保週末專案一律排入星期一的佇列。
* **頻率上限**：請考慮新增[頻率上限規則](../conflict-prioritization/journey-capping.md)，以避免在設定檔可以頻繁地重新輸入時過度傳訊。

## 進階變數

### 特定日目標定位

只在特定日期（例如，星期二和星期四）傳送電子郵件：
...
2. **新增等待活動**，用於計算到下個星期二或星期四之前的天數。

### 不同天數的不同傳送時間

使用不同的「等待」公式建立多個路徑，以彈性排程：
...
1. **調整等待公式中的小時引數**。 例如，對於在下午2點而非上午9點的傳送：

   ```javascript
   setHours(nowWithDelta(1, "days"), 14)
   ```

2. **新增時間檢查** （選擇性）：在等待之後新增第二個條件，以確認目前時間在傳送前的營業小時內。
...
2. **如果條件符合假日，請新增「等待」活動**，以延遲至下一個營業日。

[深入瞭解日期比較函式](functions/date-functions.md)
