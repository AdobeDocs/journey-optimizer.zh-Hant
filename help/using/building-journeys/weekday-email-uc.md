...
* 使用條件活動來檢查當天是星期六還是星期日
* 使用週末專案的自訂公式實作等待活動
* 將星期一傳遞的週末電子郵件在特定小時排入佇列
* 立即傳送平日專案的電子郵件（星期一至星期五）
...
* 作用中的Adobe Journey Optimizer執行個體
* 已設定的[電子郵件通道表面](../configuration/channel-surfaces.md)
* 要觸發歷程的[對象](../audience/about-audiences.md)或[事件](../event/about-events.md)
* 基本瞭解[歷程條件](condition-activity.md)和[運算式](expression/expressionadvanced.md)
...
* **複雜營業時間**：將公式擴充至基本工作日檢查之外的假日、時區或特定營業時間。

* **自訂傳送時間**：調整等待公式中的小時引數(H)，以符合您的最佳傳送時間。 例如，如果上午10點顯示較好的參與率，請將公式變更為使用小時10。

* **多重時區支援**：請考慮針對不同地理區域建立個別歷程，以確保每個收件者的當地時區週一傳遞。

### 時區管理

`now()`函式和歷程執行使用在歷程層級設定的時區。 請考量下列事項：

* **歷程時區**：確保歷程時區符合您的需求。 發佈前在歷程屬性中設定此專案。 [進一步瞭解時區管理](timezone-management.md)。

* **全域對象**：如果您的對象跨越多個時區，星期幾檢查會在歷程設定的時區中進行，而不是在收件者的當地時區中進行。

* **當地語系化排程**：針對特定時區的傳遞，針對不同區域建立個別歷程，或在讀取對象活動中使用時區設定。

### 歷程進入與時間

* **讀取對象歷程**：對於批次歷程，[排程讀取對象](read-audience.md#schedule)以在對象認為合適的時間觸發。 清晨執行（例如，上午6:00）在商務通訊中很常見。

* **事件型歷程**：收到事件時，會立即評估條件。 週末輸入的設定檔會自動等到星期一。 [進一步瞭解事件](../event/about-events.md)

* **等待逾時考量事項**：請確定您的[歷程逾時設定](journey-properties.md#timeout)符合最長等待時間（從星期六到星期一，最多2天）。

### 測試是必要的

如實施指南中強調的，請一律測試您的歷程邏輯，以確認一切都如預期般運作：

* 使用&#x200B;**測試模式**&#x200B;來模擬不同的輸入情境，而不傳送真正的電子郵件
* 測試所有三個路徑：星期六專案、星期日專案和工作日專案
* 驗證等待時間計算是否正確
* 確認星期一的傳遞在指定的時間發生
* 檢查歷程視覺效果，以確保路徑路由正確
...
* 對於週期性行銷活動，請正確設定&#x200B;**[!UICONTROL 重新進入]**&#x200B;設定。 [進一步瞭解重新進入設定](entry-management.md)

* 如果設定檔可以重新進入歷程，則每次都將受到星期幾檢查，確保週末專案一律排入星期一的佇列。

* 請考慮新增[頻率上限規則](../conflict-prioritization/journey-capping.md)，以避免設定檔可以頻繁地重新輸入時過度傳訊。
...
您可以針對不同的週末行為，使用不同的等待公式來建立多個路徑：

* **星期六→星期三傳遞**：使用`nowWithDelta(4, "days")`
* **星期日→星期二傳遞**：使用`nowWithDelta(2, "days")`
