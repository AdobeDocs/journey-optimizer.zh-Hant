...
* 在等待活動中使用傳送時間最佳化，以在繼續任何下游活動之前決定最佳時間
...
* **傳送時間最佳化等候**：一種等待型別，使用Adobe的傳送時間最佳化AI模型來選取最佳時間，以繼續下一個活動，與任何訊息傳送&#x200B;*（產品特定）*&#x200B;分離
...
* 等待活動中的傳送時間最佳化看不到無訊息時間規則；如果下游管道動作受到無訊息時間規則保護，該規則設定為捨棄訊息，則設定檔可以從訊息傳送中移除並從歷程中退出。
...
* **問：傳送時間最佳化等待活動是否知道安靜時間？**  — 否。 安靜時間僅在訊息動作時評估，因此「等待」活動可以在安靜時間視窗中挑選時間。 根據無訊息時數規則，訊息會排入佇列直到無訊息時數結束或捨棄，這也會從歷程中退出設定檔。
