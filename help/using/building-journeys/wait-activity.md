...
選取&#x200B;**[!UICONTROL 傳送時間最佳化]**&#x200B;型別，讓Adobe的AI根據每個設定檔的預測參與行為，決定繼續路徑中下一個活動的最佳時間。 這會使用與電子郵件和推播動作相同的[傳送時間最佳化](send-time-optimization.md)模型，但將等待與傳送本身分離。 等待後的活動可以是任何活動（例如自訂動作），而不是僅繫結至電子郵件或推播動作。

[進一步瞭解傳送時間最佳化的運作方式，以及如何為您的組織啟用它](send-time-optimization.md#how-send-time)。

>[!IMPORTANT]
>
>傳送時間最佳化無法顯示[無訊息時數](../conflict-prioritization/quiet-hours.md)規則。 只有當設定檔達到&#x200B;**訊息**&#x200B;動作時，才會評估無訊息時間，因此「傳送時間最佳化等待」活動可以選取位於下游頻道動作無訊息時間視窗中的最佳時間。 衝突只會在稍後傳送訊息時出現。
