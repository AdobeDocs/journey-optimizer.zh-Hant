...
**在此頁面上：**&#x200B;瞭解Adobe Journey Optimizer中歷程的基礎知識，包括歷程型別、設計工作流程、重要功能，以及建立個人化客戶體驗的最佳實務。
...
* **測試模式**：歷程驗證模式，使用測試設定檔模擬執行，以在上線前攔截問題。 *（產品特定）*
* **練習**：模擬執行的歷程預覽模式，不會影響實際的設定檔資料或傳送訊息。 *（產品特定）*
...
* 請勿混淆：「試執行」≠「測試模式」 — 試執行預覽執行，而不接觸真實資料；測試模式會使用實際的測試設定檔模擬歷程
...
* **問：如何驗證歷程而不傳送真正的訊息？**  — 使用練習模式，這種模式可預覽歷程執行，而不會影響真實資料或傳送訊息。
