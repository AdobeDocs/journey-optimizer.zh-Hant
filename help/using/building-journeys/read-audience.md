...
**在此頁面上：**&#x200B;瞭解如何設定讀取對象活動，以隨選或依排程將Adobe Experience Platform對象的設定檔帶入歷程，並個人化每個設定檔的路徑。
...
>
>**真實世界的範例**
>* **每週電子報**→讀取對象。 您的對象是每日批次快照。 您排程每個星期一上午9點的歷程。 所有合格的設定檔會一起輸入。
>* **忠誠度層級升級**→對象資格。 設定檔一旦在串流對象中達到金級狀態，就會立即進入歷程並收到祝賀電子郵件。
>* **重新參與系列**→讀取對象。 您每30天執行一次週期性歷程，鎖定長達90天以上未使用的設定檔。
...
例如，在[組建對象](../audience/about-audiences.md)使用案例中建立的`Luma app opening and checkout`對象可以當做進入點。 所有合格的設定檔都會使用條件、計時器、事件和動作，透過個人化路徑進入歷程和進度。
...
您將設定： **對象** （必要）、**名稱空間** （必要）、**讀取率** （必要，預設5,000/s）和&#x200B;**排程** （當歷程執行時）。 選擇性地新增&#x200B;**標籤**&#x200B;和&#x200B;**補充識別碼**。 下列步驟會逐步引導您完成每個設定。

### 新增活動並選取客群 {#add-activity-and-select-audience}

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_label"
>title="標籤"
>abstract="在報告和測試模式記錄中用來識別此活動的選用標籤。"
...
1. 將&#x200B;**[!UICONTROL 標籤]**&#x200B;新增至活動（選擇性）。 選用標籤可協助您在報告和測試模式記錄中識別活動。

1. 在&#x200B;**[!UICONTROL 對象]**&#x200B;欄位中，選擇將進入歷程的[!DNL Adobe Experience Platform]對象，然後按一下&#x200B;**[!UICONTROL 儲存]**。 您可以選取使用[區段定義](../audience/creating-a-segment-definition.md)產生的任何[!DNL Adobe Experience Platform]個對象。
   ...
   ![對象選擇介面顯示可用的[!DNL Adobe Experience Platform]個對象](assets/read-segment-selection.png)
...
### 補充識別碼 {#read-audience-supplemental-id}
...
**下一步：**&#x200B;設定[讀取率](#profile-entry-and-reading-rate)和[排程](#schedule)，然後[測試並發佈](#testing-publishing)。

### 輪廓輸入與讀取率 {#profile-entry-and-reading-rate}

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_reading_rate"
>title="讀取率"
>abstract="每秒進入歷程的輪廓數量上限 (500 至 20,000)。 預設為 5,000。"

設定&#x200B;**[!UICONTROL 讀取率]** （必要）。 這是每秒可以進入歷程的設定檔數目上限。 此費率僅適用於此活動，不適用於歷程中的其他活動。 例如，如果您想定義自訂動作的節流率，則需使用節流應用程式開發介面。 請參見此[頁面](../configuration/throttling.md)。
...
>title="增量讀取"
>abstract="首次執行後，只有新增到客群的新輪廓會進入歷程。"
...
>title="新客群評估的等待時間"
>abstract="歷程等待新客群資料的時長 (1 至 6 小時，以分鐘或小時為單位)。"
...
>**歷程狀態和91天全域逾時：**
>
>* **非循環**&#x200B;讀取對象歷程會在最後一個作用中設定檔結束後自動轉換成&#x200B;**已停止**&#x200B;狀態，除非歷程包含會導致等待時間的節點（等待節點、回應節點或事件觸發的轉換），在此情況下，適用標準[91天全域逾時](journey-properties.md#global_timeout)。 [了解更多](end-journey.md#auto-stop-non-recurring)
...
使用此選項時，系統會從[!DNL Adobe Experience Platform]的區段服務執行的上次對象評估工作開始，回顧&#x200B;**24小時**。
...
## 歷程中的受眾目標定位 {#audience-targeting-in-journeys}

以對象為基礎的歷程一律以&#x200B;**讀取對象**&#x200B;活動開始，以擷取屬於[!DNL Adobe Experience Platform]對象的個人。 這些設定檔會讀取一次，或按照週期性排程讀取。
...
* **`inAudience()`條件時間：**&#x200B;在讀取對象歷程的條件節點中使用`inAudience()`時，會從設定檔的批次投影讀取區段會籍。 此投影中的資料在內嵌後&#x200B;**2小時**&#x200B;內已重新整理。 如需傳輸計時情況的完整詳細資訊，請參閱[inAudience函式檔案](functions/functioninaudience.md#propagation-timing)。
...
* **TL；DR：**&#x200B;此頁面說明如何設定並使用Adobe Journey Optimizer中的「讀取對象」活動，以一次或循環排程將來自Adobe Experience Platform對象的設定檔新增至歷程中，並提供排程、輸送量、疑難排解和最佳實務的相關指引。

**意圖：**
* 設定讀取對象活動作為歷程的進入點
* 選取歷程的Adobe Experience Platform對象和身分名稱空間
* 設定讀取率，以控制每秒輸入的設定檔數
* 排程歷程，以每日、每週或自訂週期執行一次
* 啟用增量讀取，在循環執行時僅處理新的對象成員
* 疑難排解受眾規模不相符、零個人資料執行和延遲專案
* 根據批次與即時需求，在讀取對象和對象資格之間做出決定

**字彙表：**
* **讀取對象活動**：從選取的Adobe Experience Platform對象中讀取所有合格設定檔並將其新增至歷程&#x200B;*（產品特定）*&#x200B;的歷程進入點活動
* **讀取率**：每秒可進入歷程的設定檔數目上限（500-20,000；預設5,000） *（產品特定）*
* **增量讀取**：週期性歷程選項，僅處理自上次歷程執行&#x200B;*（產品特定）*&#x200B;以來新增到對象中的設定檔
* **重複時強制重新進入**：排程選項，會在每次新執行前移除所有作用中的歷程參與者，讓設定檔可以重新輸入新的&#x200B;*（產品特定）*
* **批次對象評估後觸發**：排程選項會延遲歷程執行，直到有新的批次對象快照可用為止（最多6小時） *（產品特定）*
* **補充識別碼**：次要識別碼（例如，訂單ID），當識別碼不同&#x200B;*（產品特定）*&#x200B;時，允許相同設定檔多次進入歷程

**護欄：**
* 每個歷程只允許一個讀取對象活動，且必須是第一個活動。
* 每個「讀取對象」活動只能選取一個對象。
* 每個組織最多可同時執行5個讀取對象。
* 最大讀取率是每個沙箱每秒20,000個設定檔（所有同時讀取對象活動的總和）。
* 使用補充識別碼時，讀取率限製為每秒500個設定檔。
* 只有具有「已實現對象參與率」狀態的設定檔才會進入歷程。
* 只有以人物為基礎的身分名稱空間才可使用；沒有所選名稱空間的設定檔無法進入。
* 12小時作業逾時適用於讀取對象匯出作業。
* 每10分鐘會重試失敗的匯出作業，最多重試1小時。
* 對於已啟用增量讀取的自訂上傳對象，設定檔僅在第一次重複時擷取（這些對象是固定的）。
* 縮放成功者不適用於讀取對象歷程（路徑實驗）。

**術語：**
* 正式名稱：讀取對象 — 縮寫：none — 變體：區段觸發器、對象型歷程專案、讀取區段（舊版API名稱）
* 同義字：「讀取對象」=「區段觸發器」=「對象觸發的歷程」
* 請勿混淆：「讀取對象」≠「對象資格」（讀取對象為批次/已排程；對象資格為即時串流）

**常見問題集：**
* **問：何時應使用「讀取對象」而非「對象資格」？**  — 將讀取對象用於批次、排程的使用案例（例如每週電子報、重新參與行銷活動）。 當設定檔必須在即時符合資格時立即進入歷程時，請使用對象資格。
* **問：進入歷程的設定檔為何少於對象人數？**  — 常見原因包括設定檔沒有選取的名稱空間、在執行歷程之前尚未完成批次分段工作，或設定檔未處於「已實現」狀態。 啟用「批次對象評估後觸發」並檢查名稱空間設定。
* **問：增量讀取在第一次執行時做什麼？**  — 在第一次執行時，所有對象設定檔都會進入。 在後續執行中，只會處理自上次執行以來新新增到對象的設定檔。
* **問：如果匯出作業失敗會發生什麼情況？**  — 系統每10分鐘重試一次，最多可重試1小時。 失敗會在「警示」中回報。 1小時未成功後，該執行會被視為失敗。
* **問：相同的設定檔可以多次進入讀取對象歷程嗎？**  — 是，如果設定了補充識別碼且專案之間不同，或是啟用重複時強制重新進入。 若沒有這些專案，設定檔就無法同時出現多次。
* **問：單次讀取對象歷程會維持多久時間？**  — 它會在最後一個設定檔退出時自動停止到「已停止」 ，除非歷程包含「等待」 、 「反應」或「事件觸發的轉變」 — 在這種情況下，將套用91天全域逾時。 依預設，在91天完成之前不會維持即時狀態。
