...
本頁面說明如何設定及管理歷程的所有全域設定，包括進入規則、時區、開始/結束日期、逾時行為、退出條件、裝載大小和衝突管理。

**個意圖**

* 設定設定檔的歷程進入和重新進入規則
* 設定開始和結束日期，以控制設定檔何時可以進入或退出歷程
* 定義退出條件，以在符合業務條件時自動移除設定檔
* 使用物件層級的存取控制標籤管理對歷程的存取
* 監視歷程裝載大小以防止發佈失敗
* 解決歷程與行銷活動之間的衝突並指派優先順序分數

>[!TAB 字彙]

* **歷程屬性**：全域設定面板（右側邊欄），可控制歷程的名稱、進入規則、時區、日期、逾時、裝載大小和衝突管理。 *（產品特定）*
* **重新進入等待期間**：設定檔在允許重新進入單一歷程之前必須等待的最短時間；最長為90天。 *（產品特定）*
* **全域歷程逾時(TTL)**：設定檔在歷程中可保持作用中狀態的最長持續時間 — 目前為91天，過了這段時間，設定檔會退出並刪除其資料。 *（產品特定）*
* **退出條件**：在歷程層級定義的規則，會在指定事件發生或符合對象條件時，自動從歷程中移除設定檔。 *（產品特定）*
* **以設定檔屬性為基礎的退出條件**：以設定檔屬性（例如，位置、狀態）為基礎的退出規則，這些規則會在動作步驟中評估，而且只有在歷程暫停時才能編輯。 *（產品特定）*
* **合併原則**： Adobe Experience Platform用來合併來自多個來源之設定檔資料的規則集；在整個歷程中一致套用。 *（產品特定）*
* **衝突管理**：歷程屬性中的工具，用於指派優先順序分數、套用規則集及識別重疊的歷程或行銷活動。 *（產品特定）*
* **歷程承載大小**：與設定的限制相較之下，歷程定義承載的目前大小；超過限制會封鎖發佈。 *（產品特定）*
* **OLAC （物件層級存取控制）**：使用資料使用標籤來限制存取個別歷程的許可權模型。
* **保留群組（歷程層級） （有限可用性）**：使用歷程屬性的保留群組&#x200B;**區段設定在**&#x200B;測量歷程提升度；可讓您從歷程中排除對象的百分比，以測量其遞增有效性。 *（產品特定）*
* **保留群組**：排除進入歷程的目標對象百分比，用作測量歷程遞增成效的基準。 保留群組中的設定檔會透過排除事件進行追蹤。 *（產品特定）*
* **遞增提升度**：進入歷程的設定檔（作用中群組）與未進入歷程的設定檔（保留群組）之間結果的可測量差異，用於量化歷程的真正影響。

>[!TAB 術語]

* **正式名稱：**&#x200B;歷程屬性 — 首字母縮寫： none — 變體：歷程設定，歷程設定面板
* **同義字：** &quot;global journey timeout&quot; = &quot;TTL&quot; = &quot;Time-to-Live&quot;
* **請勿混淆：** 「全域歷程逾時（91天）」≠「報告期間（~91天）」 — 逾時會限制歷程中的個別設定檔持續時間；報告期間是分析資料的UI顯示限制

>[!TAB 護欄與限制]

* 重新進入等待期間的上限為90天
* 全域歷程逾時為91天；在此期間後，會刪除設定檔資料並退出設定檔
* 歷程裝載預設限製為4 MB；超過此限制以防止發佈 — 請聯絡Adobe客戶服務以取得更高限制
* 退出條件只能在草稿狀態（事件/對象型別）中設定；設定檔屬性退出條件只能在歷程暫停時編輯
* 每個歷程只允許一個設定檔屬性退出條件規則
* 設定檔屬性退出條件僅在動作步驟進行評估，而非全域
* 更新對象合併原則時，必須重新發佈參考該對象的任何作用中歷程
* 歷程區塊發佈中的合併原則不一致；訊息個人化中的不一致不會引發警報
* 對於即時歷程，屬性面板只會顯示出版日期和發佈者名稱
* 保留：建議用於統計顯著性的受眾和1,000個設定檔中的至少5%
* 保留：開啟和點選量度沒有意義 — 請改用funnel底部轉換量度
* 保留樣本：每個歷程只有一個成功量度；這裡不支援跨歷程保留樣本
* 新歷程版本中的保留百分比變更僅套用至新加入者

>[!TAB 常見問題集]

**問：設定檔可以在歷程中停留多久？**

最多91天（全域歷程逾時）；在此期間後，設定檔會自動退出並刪除其資料。

**問：我可以在歷程即時時編輯歷程屬性嗎？**

對於即時歷程，屬性面板只會顯示發佈日期和發佈者名稱；結構變更需要新版本。

**問：設定多個退出條件時會發生什麼事？**

在歷程的每個步驟中，都會使用OR邏輯從上到下評估設定檔；只要符合任何一個條件，設定檔就會退出。

**問：如何防止設定檔重新進入歷程？**

在歷程屬性中取消勾選「允許重新進入」選項；這適用於單次體驗，例如贈品選件。

**問：歷程逾時和結束日期之間有何差異？**

結束日期會停止所有新登入並在該特定日期自動退出作用中設定檔；從設定檔進入的那一刻起，91天全域逾時便會套用在每個設定檔，無論歷程的結束日期為何。

**問：歷程的合併原則是如何決定的？**

這取決於歷程型別：讀取對象和對象資格歷程使用對象的合併原則；單一事件歷程使用預設合併原則；業務事件歷程在後續讀取對象活動中使用來自目標對象的合併原則。
