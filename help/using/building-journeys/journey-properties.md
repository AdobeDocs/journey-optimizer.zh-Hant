...
**在此頁面上：**&#x200B;瞭解如何在編寫期間從右側邊欄設定歷程的全域屬性 — 包括其名稱、進入規則、時區、開始和結束日期、逾時、退出條件、衝突管理和保留組態（可用性限制）。
...
* 選取您的歷程成功量度。 [瞭解如何設定及追蹤您的歷程成功量度](success-metrics.md)
...
* [以保留群組測量歷程提升度（可用性限制）](#performance-management)以量化您歷程的遞增效益
...
任何已設定的開始和結束日期會直接顯示在&#x200B;**歷程標題**&#x200B;中（位於狀態徽章旁）。 這讓您不需要開啟屬性面板，就能一目瞭然地檢視歷程排程。 顯示的標籤會根據每個日期的狀態而調整：

* **開始日期即將到來** — 標題會顯示設定的開始日期。
* **已超過開始日期** — 標題指出歷程已開始。
* **結束日期即將到來** — 標題會顯示設定的結束日期。
* **結束日期已通過** — 標題指示歷程已結束。

如果未定義開始或結束日期，則標題中不會出現日期資訊。
...
此全域逾時會在個人進入歷程 **91 天**&#x200B;後停止其進度。 這表示個人的歷程不能持續超過91天。 在此逾時期間後，個人的資料會被刪除。 在逾時期間結束時仍在歷程中流動的個人將會停止，且將不會在報表中將其列入考量。 因此，您可能會看到更多人進入歷程而不是退出。

>[!NOTE]
>
>將歷程視為「已完成」時的確切定義因歷程型別而異。 [檢視詳細條件](end-journey.md#journey-finished-definition)。
...
### 存留時間(TTL)和資料保留常見問題集 {#timeout-faq}
...
您可以新增多個退出條件。 退出條件現在已啟用，並將在歷程的每個步驟進行評估。

![顯示歷程終止對象條件的退出條件面板](assets/exitcriteria-sample.png){width="40%"}
...
>[!AVAILABILITY]
>
>此功能目前處於「有限可用性」。 若想取得存取權，請聯絡您的 Adobe 代表。 如需發行週期與可用性階段的完整詳細資訊，請參閱 [Journey Optimizer 發行週期](../rn/releases.md)。
...
保留是隨機選取的目標對象百分比，會排除進入歷程且不會收到任何通訊。 將保留設定檔的結果與進入歷程的設定檔結果進行比較，即可測量遞增提升度，即歷程對受眾的實際影響。

### 設定保留樣本 {#configure-holdout}
...
1. 輸入&#x200B;**[!UICONTROL 保留百分比]**：將排除在進入歷程之外的對象比例。
   ...
   *在此範例中，50%的對象將會指派給保留群組，而且不會進入歷程。 對於其餘對象，歷程將正常執行，而Customer Journey Analytics將使用電子郵件開啟次數(AJO)量度來測量提升度。*
...
屬性位於歷程畫布的右側邊欄中。 建立新歷程時，預設會顯示這些專案。 若為現有歷程，按一下歷程名稱旁的鉛筆圖示以開啟。 對於即時歷程，面板僅顯示發佈日期和發佈歷程的使用者名稱。 檢視[存取歷程](#access-properties)的屬性。

**我可以變更即時歷程上的屬性嗎？**

歷程上線後，大部分屬性都是唯讀的。 若要修改它們，請建立新的歷程版本或複製歷程，在草稿中進行變更，然後重新[發佈](publish-journey.md)。

**重新進入設定與重新進入等待期間之間有何差異？**

**[允許重新進入](#allow-reentrance)**&#x200B;控制設定檔是否可以進入歷程一次以上。 **[重新進入等待期間](#reentrance-wait)** （僅在允許重新進入時顯示）會定義相同設定檔可重新進入單一歷程之前的等待時間。 預設值為5分鐘，最大值為90天。 如需詳細資訊，請參閱[設定檔入口管理](entry-management.md)。

**設定檔在歷程中可以停留多久？**

[全域歷程逾時](#global_timeout)會在設定檔&#x200B;**91天**&#x200B;之後停止 — 個人的歷程不能超過此時間。 此逾時未顯示在介面中，且無法變更。 因為設定檔資料會在91天後移除，所以無法保證重新進入封鎖會超過該期限。 另請參閱[歷程如何結束](end-journey.md#journey-finished-definition)。
...
這取決於歷程型別： [讀取對象](read-audience.md)和[對象資格](audience-qualification-events.md)歷程使用對象的合併原則，[單一事件](../event/about-events.md)歷程使用預設合併原則，而[商業事件](../event/about-creating-business.md)歷程在以下讀取對象活動中使用目標對象的合併原則。 相同的合併原則適用於整個歷程。 如果對象合併原則已更新，任何參考該對象的作用中歷程都必須重新發佈或複製。 請參閱[合併原則](#merge-policies)。

**91天歷程逾時和91天報告期間之間有何差異？**

這些是獨立的概念。 **[歷程全域逾時](#global_timeout)** （91天）是個別設定檔在歷程中保持作用中狀態的最長時間，過了這段時間，設定檔會退出並刪除其資料。 **報告期間** （大約91天）是UI顯示限制：不再顯示早於~91天的效能資料，但歷程會持續執行，而新設定檔會持續進入。 如需TTL和資料保留詳細資料，請參閱[存留時間(TTL)和資料保留常見問題集](#timeout-faq)。

## 相關主題 {#related-topics}

* [設定檔入口管理](entry-management.md) — 設定設定檔如何進入及重新進入歷程
* [歷程登入與退出條件指南](entry-exit-criteria-guide.md) — 包含真實世界範例和最佳實務的完整指南
* [歷程結束的方式](end-journey.md) — 瞭解自然歷程完成和設定檔退出
* [暫停歷程](journey-pause.md) — 暫停並繼續具有設定檔屬性退出條件的歷程
* [時區管理](timezone-management.md) — 設定歷程和設定檔時區
* [衝突管理與優先順序](../conflict-prioritization/conflicts.md) — 識別並解決歷程與行銷活動之間的衝突
