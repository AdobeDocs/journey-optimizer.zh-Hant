...
* **TL；DR：**&#x200B;此頁面說明如何設定及管理歷程的所有全域設定，包括進入規則、時區、開始/結束日期、逾時行為、退出條件、裝載大小及衝突管理。
...
* **問：設定檔可以在歷程中停留多久？**  — 最多91天（全域歷程逾時）；在此期間後，設定檔會自動退出並刪除其資料。
* **問：我可以在歷程即時時編輯歷程屬性嗎？**  — 對於即時歷程，屬性面板只會顯示出版日期和發行者名稱；結構變更需要新版本。
* **問：設定多個退出條件時會發生什麼事？**  — 在歷程的每個步驟中，會使用OR邏輯從上到下評估設定檔；只要符合任一條件，就會退出設定檔。
* **問：如何防止設定檔重新進入歷程？**  — 在歷程屬性中取消勾選「允許重新進入」選項；這適用於單次體驗，例如贈品選件。
* **問：歷程逾時和結束日期之間有何差異？**  — 結束日期會停止所有新登入並在該特定日期自動退出作用中設定檔；從設定檔進入的那一刻起，91天全域逾時便會套用至每個設定檔，無論歷程的結束日期為何。
* **問：歷程的合併原則是如何決定的？**  — 這取決於歷程型別：讀取對象和對象資格歷程使用對象的合併原則；單一事件歷程使用預設合併原則；業務事件歷程在後續讀取對象活動中使用來自目標對象的合併原則。
