...
**在此頁面上：**&#x200B;瞭解如何結合事件、協調流程和動作活動，以建立多步驟、跨管道歷程，以及標籤活動、管理引數和疑難排解的最佳實務。
...
在此頁面[&#128279;](troubleshooting.md)上瞭解如何疑難排解活動及歷程中的錯誤。
...
* **TL；DR：**&#x200B;此頁面介紹歷程活動的三個類別（事件、協調流程和動作），並說明在Adobe Journey Optimizer歷程中標示、管理引數及處理錯誤的最佳作法。

**意圖：**
* 識別並區分歷程中的事件、協調流程和動作活動
* 將標籤和說明新增至歷程活動，以便輕鬆識別和報告
* 設定處理歷程活動逾時或錯誤的替代路徑
* 覆寫特定歷程活動的進階引數
* 結合多種活動型別以建立跨管道歷程情境
* 在發佈歷程之前疑難排解活動設定錯誤
...
**字彙表：**
* **事件活動**：由傳入事件（例如，購買、對象資格）觸發的歷程活動，它會透過歷程&#x200B;*（產品專屬）*&#x200B;開始或推進設定檔
* **協調活動**：控制歷程&#x200B;*（產品特定）*&#x200B;的流程與分支邏輯的歷程活動（例如，最佳化、讀取對象、等待）
* **動作活動**：作為觸發程式&#x200B;*（產品特定）*&#x200B;的結果，傳遞通訊或呼叫外部系統的歷程活動
* **自訂動作**：將Journey Optimizer連線至協力廠商系統以傳送訊息或資料的使用者設定動作&#x200B;*（產品特定）*
* **替代路徑**：已將遞補分支新增至活動，因此即使發生逾時或錯誤，歷程仍會繼續進行&#x200B;*（產品專屬）*

**護欄：**
* 如果在任何活動中仍偵測到設定錯誤，則無法執行測試和發佈
* 大部分活動上的進階/技術引數是唯讀的，且必須使用「引數覆寫」功能才能修改

**術語：**
* 正式名稱：歷程活動 — 縮寫：無 — 變體：活動、節點、步驟
* 同義字： &quot;action activity&quot; = &quot;channel action&quot; = &quot;message action&quot;
* 請勿混淆：「協調活動」≠「動作活動」（協調控制流程；動作傳遞通訊）

**常見問題集：**
* **問：事件、協調流程和動作活動之間有何差異？**  — 事件活動會觸發歷程進入或進展；協調活動會控制分支和流程邏輯；動作活動會傳送訊息或呼叫外部系統。
* **問：如何將標籤新增至歷程活動？**  — 開啟活動屬性窗格並填入「標籤」欄位；標籤會以尾碼的形式顯示在畫布上的活動節點下。
* **問：動作或條件活動發生錯誤時會發生什麼事？**  — 除非您勾選該活動上的「在逾時或錯誤的情況下新增替代路徑」選項，否則設定檔的歷程會停止。
* **問：我可以使用Adobe Campaign從歷程傳送訊息嗎？**  — 是，Journey Optimizer支援與Adobe Campaign v7/v8、Campaign Standard和Marketo Engage整合，以便透過自訂動作活動傳送訊息。
* **問：如何在活動上覆寫唯讀進階引數？**  — 按一下引數欄位右側的「啟用引數覆寫」圖示，以強制自訂值。
