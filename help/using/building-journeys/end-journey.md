...
* 沒有長時間執行的等待、反應或事件觸發的節點的非循環讀取對象歷程，在其排程執行後約96小時（~4天）自動轉換為已停止。 在此緩衝期間，歷程保持即時狀態。 波段型歷程和使用傳送時間最佳化的歷程，會從此自動停止中排除，且除非手動關閉或停止，否則會維持91天的全域逾時規範。
...
* **問：為什麼非週期性歷程在其執行後會持續幾天處於即時狀態？**  — 這是預期中的情形。 AJO套用約96小時（約4天）的安全緩衝時間：24小時讓飛行中的傳送完成，加上72小時讓無訊息時間延遲。 歷程在緩衝過後不久轉換為「已停止」。
* **問：以波段為基礎的歷程會在~96小時後自動停止嗎？**  — 否。 這個自動停止會排除以波段為基礎的歷程，以及使用「傳送時間最佳化」的歷程，讓他們可以在所有已排程的波段中保持作用中。 除非已關閉或手動停止，否則會遵循標準91天歷程逾時。
* **問：讀取對象歷程何時達到完成狀態？**  — 對於非循環讀取對象歷程：它會在排程執行後約96小時（約4天）自動停止至已停止（安全緩衝區：24小時閒置時段+ 72小時無訊息時間允許）。 在此緩衝期間，歷程保持即時狀態。 如果等待、回應或事件節點保持設定檔作用中，則套用標準91天全域逾時。 當已關閉歷程點選91天全域逾時時，或在finished-definition表格中每個循環歷程規則時，會達到Finished 。
