...
1. 當&#x200B;**最後一個使用中的設定檔結束**&#x200B;時，歷程會進入安全緩衝期間，並維持在&#x200B;**[!UICONTROL 即時]**&#x200B;狀態。
1. 經過安全緩衝區後（在歷程排程的執行時間後約96小時），歷程會在下一個掃描器行程中自動轉換為&#x200B;**[!UICONTROL 已停止]**&#x200B;狀態。

此行為僅適用於&#x200B;**非循環讀取對象歷程**。 循環歷程不受影響。

* **自動停止時間：**&#x200B;安全緩衝區會佔用兩個視窗：**24小時閒置視窗**&#x200B;允許任何飛行中傳送完成，以及&#x200B;**72小時無訊息時間允許** （無訊息時間最多可延遲傳送72小時，掃描器無法看見）。 在歷程排程的執行時間之後，總緩衝區約為&#x200B;**96小時（~4天）**。 在此期間，歷程保持在&#x200B;**[!UICONTROL 即時]**&#x200B;狀態。 這是預期行為，並不表示有問題。

* **排除以波段為基礎的歷程：**&#x200B;此自動停止行為不適用於以波段為基礎的歷程，包括使用傳送時間最佳化的歷程。 除非手動關閉或停止，否則這些歷程會在所有排定的波段中保持作用中，且僅由標準[91天全域逾時](../building-journeys/journey-properties.md#global_timeout)停止。
...
|--------------|------------|---------------|--------------------------|
| 讀取客群 | 無 | 不適用 | 上次設定檔退出後~96h （自動停止緩衝區） |
| 讀取客群 | 是 | 無 | 上次發生開始後91天 |
| 讀取客群 | 是 | 是 | 達到結束日期時 |
...
* **TL；DR：**&#x200B;此頁面說明即時歷程的不同結束方式（包括全域91天逾時、手動關閉新入口，以及緊急停止），及其對進行中設定檔的影響。

**意圖：**

* 關閉前往新入口的即時歷程，同時允許目前的設定檔完成歷程
* 立即停止歷程，以停止所有進行中的設定檔
* 瞭解「已關閉」、「已停止」和「已完成」歷程狀態之間的差異
* 根據歷程的型別和設定判斷歷程何時視為「已完成」
* 歷程達到「已完成」狀態後即將其刪除

**字彙表：**

* **結束標籤**：在編寫期間，每個歷程路徑的結尾會顯示自動產生的、無法移除的節點；其標籤可以變更&#x200B;*（產品特定）*
* **靠近新入口**：手動動作，可防止新設定檔進入歷程，同時允許現有設定檔完成其路徑&#x200B;*（產品特定）*
* **全域歷程逾時**：歷程會自動切換為已完成狀態並移除所有設定檔資料的最長91天期間&#x200B;*（產品專屬）*
* **已停止狀態**：所有進行中設定檔都會立即停止的歷程狀態；僅用於緊急情況&#x200B;*（產品特定）*

**護欄：**

* 無法重新啟動或刪除已關閉和已停止的歷程；只能建立新版本或重複專案。
* 只能刪除處於已完成狀態的歷程。
* 停止歷程需要管理歷程許可權；具有內嵌行銷活動或訊息節點的歷程也需要行銷活動>發佈行銷活動許可權。
* 91天全域逾時後，所有設定檔歷程資料都會移除，而剩餘的設定檔會自動退出。
* 沒有長時間執行的等待、回應或事件觸發的節點的非循環讀取對象歷程，會在最後一個設定檔退出後約96小時（約4天）自動轉換為已停止。 在此緩衝期間，歷程保持即時狀態。 波段型歷程（包括傳送時間最佳化使用案例）會從此自動停止中排除，且除非手動關閉或停止，否則會維持91天全域逾時的規範。

**術語：**

* 正式名稱：靠近新入口 — 首字母縮寫：n/a — 變體：關閉歷程，手動關閉
* 同義字：「已停止」歷程≠「已關閉」歷程 — 已立即停止所有設定檔；已關閉僅封鎖新入口
* 請勿混淆：「結束標籤」≠「結束活動」 — 「結束」標籤是自動產生且無法移除；「結束」活動是可放置的畫布節點

**常見問題集：**

* **問：關閉和停止歷程有何不同？**  — 關閉會封鎖新的入口，但會讓現有的設定檔完成；立即停止其追蹤中的所有設定檔。
* **問：為什麼非週期性歷程在其執行後會持續幾天處於即時狀態？**  — 這是預期中的情形。 在最後一個設定檔退出後，AJO會套用約96小時（約4天）的安全緩衝時間：24小時讓飛行中的傳送完成，另加72小時讓無訊息時間延遲。 歷程在緩衝經過後的下一個掃描器行程中轉換為「已停止」。
* **問：以波段為基礎的歷程會在~96小時後自動停止嗎？**  — 否。 這個自動停止會排除以波段為基礎的歷程，包括使用傳送時間最佳化的歷程，以便他們可以在所有已排程的波段中保持作用中。 除非已關閉或手動停止，否則會遵循標準91天歷程逾時。
* **問：讀取對象歷程何時達到完成狀態？**  — 對於非循環讀取對象歷程：它會在最後一個設定檔退出後約96小時（約4天）自動停止至已停止（安全緩衝： 24小時閒置時段+ 72小時無訊息允許）。 在此緩衝期間，歷程保持即時狀態。 如果等待、回應或事件節點保持設定檔作用中，則套用標準91天全域逾時。 當已關閉歷程點選91天全域逾時時，或在finished-definition表格中每個循環歷程規則時，會達到Finished 。
* **問：我可以刪除已關閉的歷程嗎？**  — 否，只能刪除已完成的歷程。
* **問：當91天逾時點選時，仍在歷程中的設定檔會發生什麼事？**  — 屆時會自動從歷程中結束。
* **問：我需要特殊許可權才能停止歷程嗎？**  — 是，需要管理歷程許可權，如果歷程包含內嵌行銷活動或訊息節點，則加上行銷活動>發佈行銷活動。
