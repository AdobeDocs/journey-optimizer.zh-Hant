...
* 沒有長時間執行的等待、回應或事件觸發的節點的非循環讀取對象歷程，會在最後一個設定檔退出時自動轉換為已停止。 除非手動關閉，否則使用這些節點的歷程仍受91天全域逾時的限制。
...
* **問：讀取對象歷程何時達到完成狀態？**  — 對於非循環讀取對象歷程：當最後一個設定檔退出時（或如果等待、回應或事件節點保持設定檔作用中，則在91天後），它自動停止到「已停止」。 當已關閉歷程點選91天全域逾時時，或在finished-definition表格中每個循環歷程規則時，會達到Finished 。
