...
* 非循環讀取對象歷程&#x200B;**在其排定的執行之後，在安全緩衝區之後自動停止**。 [了解更多](#auto-stop-non-recurring)
* 在最後一次發生循環對象歷程後。
...
**非循環讀取對象歷程**&#x200B;在其排定的執行之後，在安全緩衝區之後自動轉換為&#x200B;**[!UICONTROL 已停止]**&#x200B;狀態。 如此可免除先前非循環讀取對象歷程一直保持&#x200B;**即時**&#x200B;狀態直到91天全域逾時過期的行為，即使沒有設定檔在積極地流過。

**運作方式：**

1. 歷程會執行，並處理對象中的所有設定檔。
1. 當每個設定檔達到歷程結尾時，就會正常結束。
1. 排定的執行之後，在安全緩衝期間，歷程保持在&#x200B;**[!UICONTROL 即時]**&#x200B;狀態。
1. 經過安全緩衝後（在歷程排程的執行時間後約96小時），歷程會在之後不久自動轉換成&#x200B;**[!UICONTROL 已停止]**&#x200B;狀態。

此行為僅適用於&#x200B;**非循環讀取對象歷程**。 循環歷程不受影響。

* **自動停止時間：**&#x200B;安全緩衝會佔用兩個視窗：**24小時閒置視窗**&#x200B;允許任何飛行中傳送完成，以及&#x200B;**72小時無訊息時間允許** （無訊息時間最多可延遲72小時傳送）。 在歷程排程的執行時間之後，總緩衝區約為&#x200B;**96小時（~4天）**。 在此期間，歷程保持在&#x200B;**[!UICONTROL 即時]**&#x200B;狀態。 這是預期行為，並不表示有問題。

* **排除以波段為基礎的歷程：**&#x200B;此自動停止行為不適用於以波段為基礎的歷程，以及使用「傳送時間最佳化」的歷程。 除非手動關閉或停止，否則這些歷程會在所有排定的波段中保持作用中，且僅由標準[91天全域逾時](../building-journeys/journey-properties.md#global_timeout)停止。

* 此自動停止行為&#x200B;**不**&#x200B;適用於包含造成等候期間之節點的非週期性歷程，例如&#x200B;**等待**&#x200B;節點（以計時器為基礎）、**回應**&#x200B;節點（等候電子郵件開啟或點按之類的事件），或事件觸發的轉變。 這些歷程仍受標準[91天全域逾時](../building-journeys/journey-properties.md#global_timeout)限制。

* 您仍然可以使用[**[!UICONTROL 關閉新入口]**](#close-to-new-entrances)選項，隨時手動關閉非循環讀取對象歷程。 自動停止行為可確保歷程在不再需要時自動停止，無需手動干預。
