...
>[!NOTE]
>
>如果您使用新的畫布體驗，請參閱[改用工具列圖示](#use-the-toolbar-icons)。
...
![新的歷程畫布體驗](assets/journey-new-canvas.png)

新建立的歷程會使用新的畫布體驗。 對於在此變更之前建立的歷程，您可以按一下歷程畫布中的&#x200B;**[!UICONTROL 新增體驗]**&#x200B;按鈕以切換至新體驗。 若要回覆成舊體驗，請按一下&#x200B;**[!UICONTROL 舊體驗]**&#x200B;按鈕。

![在舊歷程畫布體驗和新歷程畫布體驗之間切換](assets/journey-new-experience-switch.png)

觀看此[操作說明影片](#video)，以完整逐步解說新畫布體驗。

### 將活動新增至您的歷程

若要將活動新增至畫布，請按一下兩個活動之間的加號(+)圖示，然後從清單中選取一個活動，將其新增至您的歷程。

![使用加號圖示從畫布新增活動](assets/journey-canvas-add-activity.png)

清單會自動進行篩選，以僅顯示可新增至歷程中該特定位置的活動，而非浮動視窗中可用的每個活動。 如果您想要檢視每個活動，無論位置為何，請按一下&#x200B;**[!UICONTROL 篩選專案]**&#x200B;圖示，並清除&#x200B;**[!UICONTROL 僅顯示可用專案]**&#x200B;選項。

### 使用工具列圖示

畫布右上角的工具列可讓您快速存取基本功能。 使用這些工具來自訂您的畫布檢視和管理複雜的歷程配置。

![畫布工具列](assets/journey-canvas-toolbar.png)

| 工具列圖示 | 說明 |
|--- |--- |
| 下載畫布影像 | 將目前的畫布檢視擷取為影像，並儲存在電腦上。 |
| 分享意見回饋 | 開啟意見回饋表單，告知我們您是否有新畫布上的遺漏專案，或是否有改善專案的想法。 |
| 多選 | 切換到選擇模式以選擇多個活動。 |
| 旋轉 | 變更畫布配置的方向。 |
| 符合熒幕大小 | 自動調整縮放等級以符合檢視的整個歷程。 |
| 縮小 | 降低縮放等級。 |
| 放大 | 增加縮放等級。 |

### 選取多個活動以進行大量動作

您可以一次選取多個活動並對其執行大量動作。

若要選取多個活動，請使用下列其中一種方法：

* **Shift + drag** — 按住&#x200B;**Shift**，並在您要選取的活動上拖曳滑鼠。
* **Ctrl/Cmd +按一下** — 選取第一個活動，然後按住&#x200B;**Ctrl** （或Mac上的&#x200B;**Command**），然後按一下其他活動以個別選取它們。
* **工具列** — 按一下工具列中的&#x200B;**多重選取**&#x200B;圖示，然後按一下您要選取的每個活動。

選取後，您可以執行下列大量動作：

* **複製** — 複製所有選取的活動以用於同一個歷程或貼到另一個歷程。
* **刪除** — 一次移除所有選取的活動。
* **另存為歷程片段** — 將選取的活動轉換為可重複使用的歷程片段，以用於其他歷程。

![選取多個活動以執行大量動作](assets/journey-canvas-bulk.png)
...
1. 選取您要複製的活動。
1. 選取&#x200B;**複製**&#x200B;或按&#x200B;**Ctrl+C** （或Mac上的Command+C）以複製。

   ![](assets/journey-canvas-copy.png)

1. 導覽至您的沙箱中的目的地，可以是相同歷程或不同歷程。
1. 按一下加號&#x200B;**(+)**&#x200B;圖示，然後從下拉式清單中選取&#x200B;**[!UICONTROL 貼上]**。

   ![](assets/journey-canvas-paste.png)

您可以依照活動複製的相同順序貼上活動，且活動會維持其設定。 如需複製/貼上限制和最佳實務的詳細資訊，請參閱[複製和貼上活動](#copy-paste)。

若要複製單一活動，請選取該活動，然後按一下右側活動設定窗格中的&#x200B;**[!UICONTROL 複製]**&#x200B;圖示，或按下&#x200B;**Ctrl+C** （或Mac上的Command+C）進行複製。

![](assets/journey-canvas-copy-single.png)

若要貼上，請按一下加號&#x200B;**(+)**&#x200B;圖示，然後從下拉式清單中選取&#x200B;**[!UICONTROL 貼上]**。


### 加入和分離分支

您可以分離分支以將其與歷程的其餘部分中斷連線而不將其刪除，並稍後加入以在不同點重新連線。

若要分離分支，請使用下列其中一種方法：

* 直接按一下邊緣以進入您要分離的活動，然後選取[分離]邊緣&#x200B;]**。**[!UICONTROL 

  ![按一下邊緣以將其分離](assets/journey-canvas-detach.png)

* 按一下該邊緣上的加號&#x200B;**(+)**&#x200B;圖示，然後選取&#x200B;**[!UICONTROL 分離邊緣]**。

  ![從加號圖示功能表分離邊緣](assets/journey-canvas-detach2.png)

任一方法都會將邊緣重新連線到新的&#x200B;**[!UICONTROL End]**&#x200B;活動，將鏈結限定在分離點上方。 不會刪除分離的活動及其下游的任何活動。 它會成為畫布上其自身的已中斷連線分支，以便您稍後重複使用。

在畫布上![已分離的分支](assets/journey-canvas-detached-branch.png)

>[!NOTE]
>
>**[!UICONTROL 分離邊緣]**&#x200B;只會出現在分離有意義的地方。 例如，目標為多個分支共用的&#x200B;**[!UICONTROL End]**&#x200B;活動的邊緣無法使用此功能。

若要加入分支，請在您要繼續連線的邊緣按一下加號&#x200B;**(+)**&#x200B;圖示，然後在下拉式清單中開啟&#x200B;**[!UICONTROL 加入]**&#x200B;區段。 從那裡：

* 從&#x200B;**[!UICONTROL 已中斷連線]**&#x200B;清單（歷程中任何位置尚未連線的分支）或&#x200B;**[!UICONTROL 分支]**&#x200B;清單（歷程中其他位置已使用的分支）中選取分支。 如果您有大量活動，請使用搜尋欄位。
* 選擇&#x200B;**[!UICONTROL 從畫布選取]**&#x200B;以反白畫布上所有符合條件的活動，然後按一下您要連線的活動。

![從畫布選取分支以加入分支](assets/journey-canvas-join.png)
...
>[!NOTE]
>
>如果您使用新的畫布體驗，請改為參閱[加入和分離分支](#join-and-detach-branches)。
...
>[!NOTE]
>
>如果您使用新的畫布體驗，請改為參閱[複製並貼上活動](#copy-and-paste-activities)。
