...
title: 從對象資格歷程移轉批次對象
description: 瞭解如何在實施日期前移轉使用對象資格節點中批次對象的歷程。
...
# 從對象資格歷程和退出條件移轉批次對象{#aq-batch-migration}

自2026年9月起，Journey Optimizer將封鎖在「對象資格」節點或「退出條件」中使用批次對象之歷程的發佈。 在下方識別您的使用案例，並遵循建議的移轉路徑。

>[!CAUTION]
>
>**強制日期： 2026年9月。** 在此日期之後，將無法發佈在「對象資格」節點或「退出條件」中使用批次對象的新歷程、草稿歷程和重複歷程。 自2026年6月發行版本以來，歷程畫布中已出現驗證警告。
...
**[退出條件](journey-properties.md#exit-criteria)**&#x200B;與進入節點依賴相同的&#x200B;**[!UICONTROL 對象資格]**&#x200B;機制，因此會受到相同的限制。 日後，在設定&#x200B;**[!UICONTROL 退出條件]**&#x200B;時使用串流對象。

## 您的歷程會如何受影響 {#impact}

在「對象資格」節點或退出條件中使用批次對象的即時歷程在2026年9月之後繼續執行。 但是，如果您停止、複製或重新發佈歷程，則會在更新設定前遭到封鎖。
