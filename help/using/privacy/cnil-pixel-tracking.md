...
電子郵件型別是在頻道設定層級設定。 在歷程或行銷活動中製作電子郵件時，作者必須選取電子郵件型別符合通訊性質的管道設定。 此分類會通知在傳遞前套用哪些同意檢查。
...
除了每則訊息的開啟追蹤切換外，Adobe Journey Optimizer的電子郵件Designer提供精細的控制功能，讓您掌控要追蹤的URL。 使用電子郵件Designer中的連結面板，作者可以檢視訊息中的所有追蹤URL，並個別設定每個連結的追蹤模式。
...
Adobe Journey Optimizer會透過Adobe Experience Platform (AEP) Consent and Preferences結構描述處理同意。 同意偏好設定會儲存在設定檔層級，並在歷程和行銷活動執行期間自動強制執行。
...
Adobe Journey Optimizer的電子郵件報告可透過即時報告和Customer Journey Analytics報告提供開啟和點按量度。 當針對郵件停用&#x200B;**[!UICONTROL 電子郵件開啟]**&#x200B;追蹤時，將不會收集該傳遞的開啟資料；報告只會反映點按次數和其他參與訊號。

## 檔案參考 {#references}

如需Adobe Journey Optimizer中電子郵件追蹤和同意管理的詳細資訊，請參閱以下檔案。

| 主題 | 檔案參考 |
|-------|------------------------|
| 啟用和停用開啟追蹤 | [訊息追蹤](../email/message-tracking.md) |
| 電子郵件選擇退出管理 | [電子郵件選擇退出管理](../email/email-opt-out.md) |
| List-Unsubscribe （電子郵件標題） | [設定List-Unsubscribe](../email/list-unsubscribe.md) |
| 偏好設定中心登陸頁面 | [登陸頁面使用案例](../landing-pages/lp-use-cases.md) |
| 同意和選擇退出管理 | [管理選擇退出](opt-out.md) |
| 同意原則 | [使用同意原則](../action/consent.md) |
| 電子郵件通道設定 | [設定電子郵件設定](../email/email-settings.md) |
| 禁止名單 | [管理隱藏清單](../configuration/manage-suppression-list.md) |
