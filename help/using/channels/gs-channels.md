...
**在此頁面上：** Adobe Journey Optimizer 的原生管道可讓您在傳出傳遞和傳入體驗中觸及客戶，比較電子郵件、簡訊、推播、即時動態、直接郵件、WhatsApp、LINE、應用程式內、網頁、程式碼型和內容卡，以檢視每項在歷程和行銷活動中受支援的位置。
...
直接在 iPhone 鎖定畫面與動態島上提供持續的即時更新。 針對個別使用案例 (訂單追蹤、設施狀態) 或廣播使用案例 (賽事分數、航班狀態)，從 API 觸發的行銷活動支援即時動態。

[深入瞭解 iOS 即時動態](../mobile-live/get-started-mobile-live.md)
:::
...
瞭解如何透過 Meta 的雲端 API 整合及使用 WhatsApp 訊息，以進行個人化和合規的通訊行銷活動與歷程。

[深入瞭解 WhatsApp 訊息](../../rp_landing_pages/whatsapp-landing-page.md)
:::
...
探索如何設定、建立及個人化 LINE 訊息，以便在行銷活動和歷程中有效通訊。

[深入瞭解 LINE 訊息](../../rp_landing_pages/line-landing-page.md)
:::
...
下表顯示不同歷程和行銷活動類型中每個管道的可用性，並指出其支援位置。

| 頻道 | 歷程 | 動作行銷活動 (行銷) | 動作行銷活動 (交易型) | API 觸發的行銷活動 | 協調的行銷活動 |
|----------------------|----------|------------------------------|----------------------------------|-------------------------|------------------------|
| 電子郵件 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 簡訊 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 推播通知 | ✅ | ✅ | ✅ | ✅ | ✅ |
