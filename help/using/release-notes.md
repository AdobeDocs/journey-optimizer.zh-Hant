...
* **全域檢視中的資料格式**  - 您現在可以在&#x200B;**執行**&#x200B;標籤中的&#x200B;**全域檢視**&#x200B;切換數字與百分比 。 [進一步了解](message-monitoring.md)
...
* **搜尋**  - 在每個頁面上，您現在可以直接從「統一的 Experience Cloud」搜尋欄位搜尋業務物件和說明文章。 [進一步了解](user-interface.md#unified-search)
...
* **背景**  - 即時預覽現在支援背景影像。 [進一步了解](preview.md)
* **一鍵選擇退出的連結**  - 您可以在電子郵件內容中插入新類型的連結：**選擇退出**&#x200B;連結可讓使用者只要按一下即可取消訂閱接收您的通訊，無須重新導向至登陸頁面以確認選擇退出。 [進一步了解](message-tracking.md#one-click-opt-out-link)
...
* **允許清單**  - 您現在可以透過 API 呼叫，在非生產沙箱上啟用和停用允許清單。 [進一步了解](allow-list.md#enable-allow-list)
...
* 已在校樣中限制&#x200B;**主旨行首碼** 允許的字元。 [閱讀全文](preview.md#send-proofs)
