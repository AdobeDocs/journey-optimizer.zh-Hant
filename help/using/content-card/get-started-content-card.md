...
**在此頁面上：**&#x200B;內容卡會直接在您的行動應用程式和網站中傳送個人化、非侵入式訊息，在內容中吸引使用者，而不會中斷他們原本要執行的操作。
...
內容卡片最適合用於希望訊息能持續保留，供使用者依自己的步調採取行動，而非在訊息送達當下立即中斷使用者操作的情境。

| 優點 | 原因 | 使用案例範例 |
| --- | --- | --- |
| 持續可見度 | 卡片會保留在應用程式或網站中，直到使用者將其解除為止 | 進行中的促銷活動，以及使用者可以再次造訪的功能亮點 |
| 非侵入式傳遞 | 整合至介面中，而非中斷使用者 | 被動更新會與其他內容一同顯示 |
| 無需選擇加入推播即可運作 | 於應用程式內或網站上傳送，不受推播通知同意所限 | 觸及已停用推播通知的使用者 |
| 視覺效果豐富的訊息 | 支援媒體和行動號召按鈕 | 產品重點推薦，搭配影像與「立即購買」按鈕 |

## 不適用情境

內容卡適用於使用者可依自己步調檢視的內容，因此並非適合所有情境。 在下列情況下，建議考慮採用其他管道：

* 訊息具有時效性，因為使用者可能無法及時開啟應用程式或網站來檢視內容卡
* 目標使用者處於非活躍狀態，由於內容卡僅顯示於應用程式或網站內，非活躍使用者永遠無法看到內容
* 內容涉及敏感資訊，或需要安全處理，而持續顯示的內容卡無法保證安全
* 訊息需要使用者立即採取行動，例如限時特賣，此時推播通知或電子郵件能促使使用者更快採取行動。
