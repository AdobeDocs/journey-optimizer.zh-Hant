...
可新增至「已協調的行銷活動」畫布的活動清單隨即顯示。 可用的活動取決於您在「已協調」行銷活動畫布中的位置。 例如，新增第一個活動時，您可以定義對象（例如，具有&#x200B;**建立對象**、**讀取對象**&#x200B;或&#x200B;**載入檔案**）、分割協調行銷活動路徑，或設定&#x200B;**等待**&#x200B;活動以延遲協調行銷活動的執行，藉此開始您的協調行銷活動。 另一方面，在&#x200B;**建立對象**&#x200B;活動後，您可以調整目標定位活動、傳送傳遞至具有頻道活動的對象，或組織具有流程控制活動的「協調流程」行銷活動程式。
...
將頻道活動新增至畫布後，[瞭解如何在電子郵件、簡訊和其他訊息中新增個人化](add-personalization.md)。

當畫布準備就緒時，執行協調的行銷活動並追蹤其任務的進度。 [瞭解如何開始協調的行銷活動並監視其執行](start-monitor-campaigns.md)
