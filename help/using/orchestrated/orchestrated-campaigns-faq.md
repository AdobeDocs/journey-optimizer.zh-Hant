...
* [開始使用協調的行銷活動](gs-orchestrated-campaigns.md)
* [Adobe Journey Optimizer產品說明](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"}
...
**最佳實務**：搭配使用 — 針對已觸發、回應式體驗的歷程，以及針對已規劃、以行事曆為基礎之方案的協調行銷活動。
...
當行銷活動在&#x200B;**草稿**&#x200B;中時，您可以在排程中定義&#x200B;**引數**，並為每個引數提供&#x200B;**測試值**&#x200B;以進行測試。 啟動工作流程，然後呼叫觸發程式API （使用排程設定的範例要求或您自己的具有相同端點的要求）以使用這些測試值執行行銷活動。 [瞭解如何完成並測試訊號觸發的行銷活動](trigger-orchestrated-campaign.md#build-and-test)。
...
1. **傳送確認擱置中（最常見）** — 對於非週期性行銷活動，訊息傳送依預設會暫停，直到您從管道活動的屬性窗格明確確認傳送為止。 此行銷活動顯示為&#x200B;**即時**，但在確認之前不會傳出任何訊息。 [進一步了解](start-monitor-campaigns.md#confirm-sending)

1. **行銷活動已排程在未來某個時間** — 如果已設定排程，行銷活動為即時，但尚未開始執行。 檢查排程設定，並等待設定的開始時間。 [進一步了解](create-orchestrated-campaign.md#schedule)

1. **儲存對象活動仍在擷取** — 儲存對象活動在發佈訊息活動之前執行。 如果對象擷取仍在進行中，訊息執行尚未開始。 監視畫布中的活動狀態指示器。 [進一步了解](start-monitor-campaigns.md#activities)

1. **對象為空** — 目標查詢傳回零個設定檔。 在重新發佈之前，請檢閱分段規則並驗證受眾規模。

1. **所有設定檔都選擇退出** — 在傳送時針對每個設定檔評估同意。 如果相關頻道上的所有目標設定檔都選擇退出，則不會傳送任何訊息。 [進一步了解](../action/consent.md)

1. **處於錯誤狀態的管道活動** — 管道活動上的橘色或紅色狀態指示器代表封鎖問題。 開啟&#x200B;**[!UICONTROL 記錄檔]**&#x200B;以取得有關錯誤的詳細資訊及解決方法。 [進一步了解](start-monitor-campaigns.md#logs-tasks)

1. **速率控制節流傳遞** — 如果在頻道活動上啟用速率控制，傳遞可能會比預期慢。 檢查頻道活動屬性窗格中的速率控制設定。 [進一步了解](activities/channels.md#rate-control)
...
在協調行銷活動中建立的物件（例如，對象和工作流程）屬於建立它們的沙箱。 若要在其他沙箱（例如，開發、預備或生產）中重複使用精心安排的促銷活動，請使用&#x200B;**沙箱工具**&#x200B;複製它：將促銷活動新增至套件、發佈套件，然後匯入目標沙箱。 匯入的復本是在&#x200B;**草稿**&#x200B;中建立的，而&#x200B;**重新匯入相同的套件會建立新的行銷活動**，而非更新現有的行銷活動。 完整移動通常需要&#x200B;**多個步驟**：您可能需要透過相同封裝或其他封裝匯入來調整&#x200B;**通道設定** （目標中的相符名稱）、**結構描述**&#x200B;和&#x200B;**資料集** — 通道設定不會與行銷活動一起複製。 UI中沒有完整的匯出前檢查清單；請使用匯入對應流程和&#x200B;**匯入後警示**&#x200B;來完成設定。 如需詳細資訊和限制，請參閱[在沙箱之間複製Journey Optimizer物件](../configuration/copy-objects-to-sandbox.md)。

**最佳作法**

* 維護&#x200B;**個獨立的沙箱**&#x200B;以用於實驗、QA和生產。
* 每次匯入後，請先驗證活動在目標沙箱中端對端地進行，然後再發佈。
* 記錄設定並與治理團隊保持一致，以減少環境之間的設定漂移。

**進一步了解**

* [在沙箱之間複製Journey Optimizer物件](../configuration/copy-objects-to-sandbox.md)
* [開始使用協調的行銷活動](gs-orchestrated-campaigns.md)
* [護欄與限制](guardrails.md)
