...
title: 使用 Adobe Journey Optimizer 建立並安排協調的行銷活動
description: 瞭解如何使用 Adobe Journey Optimizer，建立並安排協調的行銷活動
badge: label="Alpha"
hide: true
hidefromtoc: true
...
請在[!DNL Adobe Journey Optimizer]中建立協調行銷活動，設定執行排程，以便控制啟動時間和執行頻率。 選擇在特定日期與時間，立即啟動行銷活動，或可使用彈性排程選項，例如按照每日、每週或每月頻率，重複啟動行銷活動。
...
預設情況下，可在手動啟動時開始協調的行銷活動，等到執行相關活動後就會結束。 如果您偏好延後執行，或是循環執行行銷活動，就可以定義行銷活動的排程。

安排協調行銷活動時，請考量下列最佳實務，以便確保能獲得最佳效能和預期行為：

* 請勿將精心安排的行銷活動安排為超過每 15 分鐘執行一次，原因是它可能會阻礙整體系統效能，還會在資料庫中建立區塊。
* 如果您想在協調的行銷活動中傳送單一訊息，可將活動設定為執行&#x200B;**一次**。
* 如果您想要在協調的行銷活動中傳送循環訊息，就必須使用&#x200B;**排程**&#x200B;選項，然後設定執行頻率。 循環傳遞活動不允許您自己定義排程。
...
在下列範例中，活動已設定為讓協調的行銷活動在 2025 年 10 月 1 日至 2026 年 1 月 1 日這週每天執行兩次，時間為 9 點和 12 點。
