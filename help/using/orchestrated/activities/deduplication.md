...
**在此頁面上：**&#x200B;瞭解如何在協調的行銷活動中使用重複資料刪除目標定位活動，在使用目標定位資料之前，從傳入活動的結果中移除重複記錄。
...
>title="用於識別重複資料的欄位"
>abstract="在&#x200B;**用於識別重複項目的欄位**&#x200B;區段，按一下「**新增屬性**」按鈕，指定可允許識別重複之相同值的欄位，例如：電子郵件地址、名字、姓氏等。欄位的排序方式讓您能夠指定優先處理的欄位。"
...
1. 在&#x200B;**[!UICONTROL 用於識別重複項目的欄位]**&#x200B;區段，按一下「**[!UICONTROL 新增屬性]**」按鈕，指定可允許識別重複之相同值的欄位，例如：電子郵件地址、名字、姓氏等。欄位的排序方式讓您能夠指定優先處理的欄位。
