...
使用 [!DNL Journey Optimizer]，您可以監控自動排除而無法傳送歷程或行銷活動的所有電子郵件地址，例如：
...
這些電子郵件地址會自動收集到Journey Optimizer中 **隱藏清單**. 進一步了解隱藏清單概念和在 [本節](../reports/suppression-list.md).
...
從 **[!UICONTROL Suppression list]** 視圖中，也可以編輯隱藏規則。 [深入了解](retries.md)
...
1. 選取 **[!UICONTROL Download CSV]** 按鈕。
...
* **手動**:您也可以手動將電子郵件地址或網域新增至隱藏清單。 [深入了解](#add-addresses-and-domains)
...
>[!NOTE]
>
>取消訂閱的使用者不會收到來自 [!DNL Journey Optimizer]，因此其電子郵件地址無法傳送至隱藏清單。 其選擇是在Experience Platform層級處理。 [進一步了解選擇退出](../privacy/opt-out.md)
...
>abstract="您可以手動填入Journey Optimizer隱藏清單，以排除特定的電子郵件地址和/或網域，不讓您的傳送進行。"
