...
**在此頁面上：**&#x200B;檢閱Adobe Journey Optimizer中的電子郵件傳遞錯誤完整清單，每個錯誤清單都有其錯誤標籤、錯誤型別、技術值和說明。
...
| **軟退信** | 軟退信 | 20 | 訊息發生暫時性失敗。 在未來的重試中可能會成功。 |
| **DNS失敗** | 軟退信 | 21 | 郵件傳遞發生暫時性DNS失敗。 在未來的重試中可能會成功。 |
| **郵箱已滿** | 軟退信 | 22 | 收件者的信箱已滿，郵件發生暫時傳遞失敗。 |
...
| **拒絕轉送** | 軟退信 | 54 | 無法接受此訊息，因為目的地網域未被列入允許轉送清單。 |
| **自動回覆** | 已忽略 | 60 | 除非啟用轉寄，否則接收時[!DNL Journey Optimizer]會捨棄這些郵件。 |
| **暫時性失敗** | 已忽略 | 70 | 傳遞將以節流率重試，或可能在暫停時延遲。 |
| **挑戰 — 回應** | 軟退信 | 100 | 傳遞可能會永久失敗，因為[!DNL Journey Optimizer]不支援挑戰 — 回應驗證機制。 |
