...
>title="排除行銷活動對象"
>abstract="選取行銷活動，以從目前階段中排除其對象。 這可防止先前聯絡的設定檔再次成為目標；僅會排除透過歷程收到通訊的設定檔。"
...
若要定義IP準備計畫的階段，您必須為每個階段選取行銷活動、設定網域和對象的排除專案，並管理設定檔目標定位。 每個階段都可以包含將在下一區段中設定的多個執行。 請依照下列步驟執行：
...
1. 選取您要與IP熱身計畫第一階段關聯的行銷活動。
   ...
   >[!IMPORTANT]
   >
   >* 只有啟用&#x200B;**[!UICONTROL IP熱身計畫啟用]**&#x200B;選項的促銷活動才可供選取。 [進一步瞭解](#create-ip-warmup-campaign)
   >* 只有使用與所選IP熱身計畫相同設定的行銷活動才可供選擇。
   >* 無法選取已用於其他IP熱身計畫中的行銷活動。 相同的行銷活動可用於相同計畫的多個階段。

1. 為目前階段選取行銷活動後，會顯示要排除設定檔、行銷活動對象和網域群組的區段。 請注意，一旦啟動執行，除非您將執行[分割為新階段，否則無法再修改排除專案。](#split-phase)

   1. 從&#x200B;**[!UICONTROL 排除的網域群組]**&#x200B;區段中，選取您要從該階段排除的網域。

      >[!NOTE]
      >
      >網域排除需要未執行的階段，因此您可能需要[分割執行中的階段](#split-phase)以新增排除。 此外，您只能排除已新增至[IP熱身計畫範本](ip-warmup-plan.md#prepare-file)的自訂網域群組 — 如果沒有，請以自訂網域群組更新範本，然後[重新上傳計畫](#re-upload-plan)。
   ...
   1. 在排除設定檔&#x200B;]**區段的**[!UICONTROL &#x200B;歷程中，選取包含您要從目前階段排除之對象的歷程。
   ...
   1. 在&#x200B;**[!UICONTROL 先前執行]**&#x200B;中定位的設定檔區段中，您可以看到永遠排除該階段先前執行的設定檔（此區段為唯讀）。 例如，如果在Run #1中，前4800位目標人物中涵蓋某個設定檔，系統會自動確保相同的設定檔不會在Run #2中收到電子郵件。
...
1. 使用&#x200B;**[!UICONTROL 刪除階段]**&#x200B;按鈕移除任何不想要的階段。 只有在階段中未執行任何執行時，才能使用此動作。<!--Once a run is executed, deletion is not allowed.-->
   ...
   >[!CAUTION]
   >
   >您無法復原&#x200B;**[!UICONTROL 刪除階段]**&#x200B;動作。 如果您刪除所有階段，建議重新上傳計畫。 [進一步瞭解](#re-upload-plan)
...
>title="在參與上篩選"
>abstract="例如，此欄是僅針對過去20天內與您的品牌互動的使用者的篩選器。 您也可以透過&#x200B;**編輯執行**&#x200B;選項變更此設定。"
...
>title="檢視合格的設定檔"
>abstract="此欄顯示合格的設定檔數目。 評估對象的執行後，如果目標設定檔超過合格的設定檔，則仍會執行執行，除非啟用&#x200B;**在發生錯誤時取消啟用的執行**&#x200B;選項。 在此情況下，執行會被取消。"

定義IP熱身計畫的階段後，您需要在每個階段中設定個別執行。 每次執行都需要排程，您可以選擇設定參與篩選器、錯誤處理和重試視窗，以確保最佳執行。 請依照下列步驟執行：
...
1. 或者，您可以定義一個時間範圍，在萬一[對象評估](https://experienceleague.adobe.com/docs/experience-platform/segmentation/home.html#how-segmentation-works){target="_blank"}有任何延遲時，可以執行IP熱身行銷活動。 若要這麼做，請按一下左上角計畫名稱旁的「屬性」圖示，然後使用&#x200B;**[!UICONTROL 重試執行時間]**&#x200B;下拉式清單來選取持續時間 — 最多240分鐘（4小時）。
   ...
   >[!NOTE]
   >
   >每隔30分鐘重試一次，直到定義的時間範圍結束。 如果未指定時間視窗，則在傳送時嘗試執行，如果未完成對象評估，則執行將失敗。
...
1. 如有需要，請從[更多動作]圖示中選取[編輯執行]。 ****&#x200B;您可以在此處更新每欄中的地址數。 例如，您也可以更新「**[!UICONTROL 上次參與]**」欄位，以僅針對過去20天內與您品牌參與的使用者。
   ...
   >[!NOTE]
   >
   >建議您諮詢傳遞能力專家，修改這些數字。 若要停用回合的參與期間，請在&#x200B;**[!UICONTROL 上次參與]**&#x200B;欄位中輸入0。
...
1. **[!UICONTROL 啟動]**&#x200B;執行。 [進一步瞭解](#activate-run)
...
* 啟用任何回合時：
   ...
   * 已建立[對象構成](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/audience-composition.html){target="_blank"}，對應促銷活動將傳送至的對象，其命名慣例如下： `<warmupName>-Phase<phaseNo>-Run<runNo>`。
...
* **[!UICONTROL 已完成]**：此回合的行銷活動執行已完成。 您可以按一下表格中的&#x200B;**[!UICONTROL 檢視報告]**&#x200B;按鈕，存取詳細的執行報告。 此選項可讓您追蹤執行的電子郵件傳送狀態，包括網域群組的特定劃分，以提升監控能力。 請注意，與其關聯的行銷活動將會設定為「已停止」。[深入瞭解](#reports)
* **[!UICONTROL 已取消]**：已使用&#x200B;**[!UICONTROL 取消]**&#x200B;按鈕取消&#x200B;**[!UICONTROL 即時]**&#x200B;執行。[進一步瞭解](#define-runs)
...
您也可以從[行銷活動功能表](../campaigns/manage-campaigns.md#access)存取報告，因為您的計畫可能會使用不同的行銷活動。 [在行事曆→](../campaigns/manage-campaigns.md#calendar)中檢視
