...
>title="排除行銷活動對象"
>abstract="選取行銷活動以將其對象排除在目前階段外。這可以防止先前聯絡過的輪廓再次成為目標；只有透過歷程收到通訊的人才會被排除在外。"
...
>title="取消出現對象錯誤的執行"
>abstract="當針對執行評估對象，如果合格輪廓少於目標輪廓，選取此選項以取消該執行。"

>[!CONTEXTUALHELP]
>id="ajo_admin_ip_warmup_qualified"
>title="檢視合格輪廓"
>abstract="此資料欄顯示合格輪廓的數量。評估執行的對象後，如果目標輪廓多於合格輪廓，則執行仍會進行，除非「**發生錯誤時取消已啟動之執行**」選項已啟用。在此情況下，會取消此執行。"
...
您也可以從[行銷活動功能表](../campaigns/modify-stop-campaign.md#access)存取報告，因為您的計畫可能會使用不同的行銷活動。
