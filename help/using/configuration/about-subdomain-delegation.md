...
**在此頁面上：**&#x200B;瞭解為何以及如何在Adobe Journey Optimizer中設定電子郵件傳送子網域，比較完整委派、CNAME和自訂委派方法以保護您的網域信譽和可遞送性。
...
| 設定方法 | 運作方式 | 所需投入的精力 |
|---|---|---|
| **完全委派** | 建立子網域和命名空間記錄。 Adobe接著會設定Adobe Journey Optimizer所需的所有DNS記錄。<br/><br/>在此設定中，Adobe會完全負責管理子網域和所有DNS記錄。 | 低 |
| **CNAME方法** | 建立子網域和命名空間記錄。 Adobe接著會提供要放置在DNS伺服器中的記錄，並在Adobe Journey Optimizer DNS伺服器中設定對應的值。<br/><br/>在此設定中，您和 Adobe 都有責任維護 DNS。 | 高 |
