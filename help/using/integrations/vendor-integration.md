...
客戶有責任確保其使用AJO整合功能及任何相關協力廠商或整合服務的行為符合所有適用法律及法規，例如HIPAA。
...
* **內容管理系統：** [內容](#contentful)，[Sitecore](#sitecore)，[Salsify](#salsify)，[內容棧疊](#contentstack)，[Akeneo](#akeneo)，[Magnolia](#magnolia)
* **忠誠度與獎勵：** [Voucherify](#voucherify)，[Talon.One](#talon-one)，[Antavo](#antavo)，[Salesforce忠誠度](#salesforce-loyalty)，[毛細管](#capillary)
* **範本、個人化和建議：** [Stensul](#stensul)，[Marigold](#marigold)，[Adobe Target建議](#adobe-target-recommendations)
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Contentubly維護或正式支援。 使用有爭議的檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Sitecore維護或正式支援。 透過Sitecore檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 在下載順序路徑上設定&#x200B;**GET**、設定每個Sitecore的授權標頭、從內容對應`id`、貼上範例JSON、對應欄位，以及調整資產延遲的逾時。
...
1. 選取設定表格中顯示的HTTP方法，通常為GET （除非另有說明）。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Salsify維護或正式支援。 使用Salsify檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它不是由Contentstack維護或正式支援。 使用Contentstack檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Akeneo維護或正式支援。 使用Akeneo檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 搭配持有人權杖使用&#x200B;**GET**、在查詢旗標中僅要求所需的屬性選項、貼上範例JSON、對應最小屬性集、測試、啟動。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 其並非由Magnolia維護或正式支援。 使用Magnolia檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Voucherify維護或正式支援。 使用Voucherify檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Talon.One維護或正式支援。 使用Talon.One檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 在您需要的設定檔或成就路徑上使用&#x200B;**GET**、將`Authorization: ApiKey-v1 <key>`設為記錄、貼上範例JSON、對應欄位、測試、啟動。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Antavo維護或正式支援。 透過Antavo檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 Salesforce並不負責維護或正式支援此功能。 透過Salesforce檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 Capillary不負責維護或正式支援。 透過Capillary檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 Stensul不維護或正式支援該功能。 透過樣板檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Marigold維護或正式支援。 使用Marigold檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定是由Adobe Journey Optimizer團隊測試的說明性模式。 Adobe Target Recommendations是獨立的Adobe產品，有其自己的發行週期和API版本設定。 在生產環境中部署之前，請一律先透過[Adobe Target開發人員檔案](https://experienceleague.adobe.com/zh-hant/docs/target-dev/developer/overview)確認目前的API詳細資料。
...
➡️啟用整合後，瞭解如何[在內容製作中新增整合](integrations-personalization.md#apply-integration-personalization)以及[在訊息範本中使用Adobe Target資料](integrations-personalization.md#use-adobe-target-in-templates)。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 AccuWeather並不負責維護或正式支援此功能。 透過AccuWeather檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 除非您的訂閱另有需求，否則請使用&#x200B;**GET**、附加`apiKey`查詢引數、對應`locationKey`以及設定檔/內容中的其他變數、貼上範例JSON、對應欄位，然後測試。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 ShipStation不提供維護或正式支援。 透過ShipStation檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由RevenueCat維護或正式支援。 使用RevenueCat檔案確認目前的API詳細資訊。
...
範例模式：使用RevenueCat的&#x200B;**取得來自[RevenueCat檔案](https://docs.revenuecat.com/){target="_blank"}的產品** （或同等產品/權益GET），以及專案的基本URL和版本。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Databricks維護或正式支援。 使用Databricks檔案確認目前的API詳細資訊。
...
以下&#x200B;**GET**&#x200B;工作範例是說明性的；對於SQL驅動的個人化，偏好使用您的工作區支援的[陳述式執行API](https://docs.databricks.com/api/workspace/statementexecution){target="_blank"}模式。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Bynder維護或正式支援。 透過Bynder檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 在選取的端點上設定&#x200B;**GET** （一個常見的模式是使用者清單）、根據[Bynder](https://developer.bynder.com/){target="_blank"}完成OAuth、避擴音取不必要的資料頁面、對應欄位、測試，然後啟用。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Trustpilot維護或正式支援。 透過Trustpilot檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 Bazaarvoice不負責維護或正式支援此功能。 使用Bazaarvoice檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 使用&#x200B;**GET**&#x200B;搭配`passkey`作為Conversations API上的查詢引數，設定`Accept: application/json`，貼上範例JSON，對應欄位，測試，啟動。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 OneTrust不維護或正式支援此功能。 透過OneTrust檔案確認目前的API詳細資料。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 Meta並不負責維護或正式支援此功能。 透過Meta檔案確認目前的API詳細資訊。
...
1. 遵循[使用整合](integrations.md)。 圖表呼叫通常是具有版本化路徑的&#x200B;**GET**；處理權杖到期、貼上範例JSON、對應欄位、測試、啟用。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Aprimo維護或正式支援。 透過Aprimo檔案確認目前的API詳細資訊。
...
>[!IMPORTANT]
>
>此設定範例已由Adobe以範例模式獨立測試。 它並非由Epsilon維護或正式支援。 透過Epsilon檔案確認目前的API詳細資料。
