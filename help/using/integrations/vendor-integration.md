...
1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。
...
授權：持有人&lt;CONTENTABLE_DELIVERY_TOKEN>
...
1. 遵循[使用整合](external-sources.md)。 在下載順序路徑上設定&#x200B;**GET**、根據Sitecore設定授權標頭、從內容對應`id`、貼上範例JSON、對應欄位，以及針對資產延遲調整逾時。
...
1. 選取設定表格中顯示的HTTP方法（除非另有說明，否則通常為GET）。
...
1. 遵循[使用整合](external-sources.md)。 比起大量目錄呼叫、設定持有人驗證、貼上範例JSON、對應欄位、測試、啟動，偏好使用單一產品擷取。
...
1. 遵循[使用整合](external-sources.md)。 根據Contentstack需要新增`api_key`和`access_token`標頭、包含`environment`查詢引數、貼上範例JSON、對應欄位、測試、啟用。
...
| 金鑰名稱 | 索引鍵值 | 新增至 |
| --- | --- | --- |
| `api_key` | `<YOUR_STACK_API_KEY>` | 標頭 |
| `access_token` | `<YOUR_DELIVERY_TOKEN>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 使用&#x200B;**GET**&#x200B;搭配持有人權杖、在查詢旗標中僅要求所需的屬性選項、貼上範例JSON、對應最小屬性集、測試、啟動。
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | `Bearer <YOUR_ACCESS_TOKEN>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 使用您的模組公開的公開傳送URL模式、根據Magnolia指引進行驗證（匿名傳送與受保護內容的Token）、貼上範例JSON、對應欄位、測試、啟用。
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | `<bearer_token>` | 標頭 |

注意：傳送API是用於不需要登入的內容之其餘匿名角色。 為了安全存取受保護的資料，建議使用更健全的方法，例如API權杖或OAuth 2.0
...
1. 遵循[使用整合](external-sources.md)。 設定叢集的基底URL、新增必要的標頭(`X-APP-ID`、`X-APP-TOKEN`)、使用篩選器或ID限制清單端點、貼上範例JSON、對應欄位、測試、啟動。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用忠誠度/REST API設定端點。 範例URL模式：

1. 您所在地區的每個Voucherify OpenAPI基本URL
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | X-APP-TOKEN | `<YOUR-APP-TOKEN>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 在您需要的設定檔或成就路徑上使用&#x200B;**GET**，將`Authorization: ApiKey-v1 <key>`設為紀錄，貼上範例JSON，對應欄位，測試，啟動。
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | ApiKey-v1 `<YOUR_API_KEY>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 使用供應商的驗證（例如查詢中的API金鑰）設定&#x200B;**GET**，避免公開PII違反原則、貼上範例JSON、對應欄位、測試、啟動。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Antavo Enterprise API設定端點。 範例URL模式：

1. 租使用者中記錄的每個Antavo棧疊基底URL
...
1. 遵循[使用整合](external-sources.md)。 使用您的團隊核准的忠誠度整合端點、完成Salesforce OAuth、貼上範例JSON、對應欄位、遵守複合API限制、測試、啟用。
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | `<access_token>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 視需要設定標頭，例如`CAP-API-ACCESS-TOKEN`、貼上範例JSON、對應欄位、測試、啟動。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Capillary API設定端點。 範例URL模式：
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | CAP-API-ACCESS-TOKEN | `<YOUR_ACCESS_TOKEN>` | 標頭 |
...
1. 使用樣板範本API URL （範例模式）設定端點： `https://api.stensul.com/v1/templates/{template_id}`
...
1. 遵循[使用整合](external-sources.md)。 指向您地區的Marigold主機、設定驗證（下列範例使用包含金鑰和密碼的`X-Api-Key`）、貼上範例JSON、對應欄位、測試、啟動。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Marigold REST API設定端點（根據您的整合指南設定端點）。 範例URL模式：

1. 使用Marigold API檔案中提供的基底URL和路徑
...
1. 歐洲： https://{{customername}}.module.slgnt.eu
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | X-Api-Key | `<apiKey>:<apiSecret>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 傳遞呼叫通常是具有JSON內文的&#x200B;**POST**。 根據[目標驗證](https://experienceleague.adobe.com/zh-hant/docs/target-dev/developer/api/configure-authentication){target="_blank"}設定OAuth、貼上範例回應、對應欄位、在預期的磁碟區下測試。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Target Recommendations/傳送API設定端點（根據Adobe檔案中的整合模式）。 範例URL模式：

1. 請參閱使用案例的Adobe Target Recommendations API檔案
...
1. 遵循[使用整合](external-sources.md)。 除非您的訂閱另有需求，否則請使用&#x200B;**GET**，從設定檔/內容附加`apiKey`查詢引數（或如檔案所述）、對應`locationKey`和其他變數、貼上範例JSON、對應欄位，然後測試。
...
整合欄位範例。 詳細資訊和層級在[AccuWeather API](https://developer.accuweather.com/apis){target="_blank"}中說明。 您經常使用個別的位置搜尋呼叫（例如`locationKey`）來解析`.../locations/v1/cities/search?q={{cityName}}`。
...
1. 遵循[使用整合](external-sources.md)。 鎖定您需要的資源（訂單與出貨）、根據[ShipStation API](https://www.shipstation.com/docs/api/){target="_blank"}進行驗證、貼上範例JSON、對應欄位、測試、啟動。
...
1. 遵循[使用整合](external-sources.md)。 呼叫下面模型化的REST **GET**、使用機密金鑰標頭進行驗證、貼上範例JSON、對應欄位、測試、啟動。
...
範例模式：使用RevenueCat的&#x200B;**「從** RevenueCat檔案[取得產品](https://docs.revenuecat.com/){target="_blank"} （或同等產品/權益GET）」以及專案的基本URL和版本。
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | `Bearer <token>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 偏好較窄的讀取路徑；如果您使用&#x200B;**POST**&#x200B;陳述式執行，包括API所需的JSON內文、貼上範例成功回應以進行對應、仔細測試延遲、啟動。
...
1. 遵循[使用整合](external-sources.md)。 在選取的端點上設定&#x200B;**GET** （常見的模式為使用者清單）、根據[Bynder](https://developer.bynder.com/){target="_blank"}完成OAuth、避擴音取不必要的資料頁面、對應欄位、測試，然後啟用。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Bynder API v4設定端點（範例：使用者清單模式）。 範例URL模式：
...
1. 遵循[使用整合](external-sources.md)。 使用必要的查詢驗證設定&#x200B;**GET**、從設定檔或內容對應識別碼、貼上範例JSON、對應欄位、測試、啟動。
...
1. 遵循[使用整合](external-sources.md)。 使用&#x200B;**GET**&#x200B;搭配`passkey`做為Conversations API上的查詢引數，設定`Accept: application/json`，貼上範例JSON，對應欄位，測試，啟動。
...
1. 遵循[使用整合](external-sources.md)。 使用已發佈的結構描述或偏好設定中心路徑您的訂閱檔案，視需要完成OAuth、貼上範例JSON、對應欄位、測試、啟用。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用OneTrust API設定端點。 範例URL模式：

1. 每個OneTrust開發人員入口網站基本URL
...
1. 遵循[使用整合](external-sources.md)。 圖表呼叫通常是有版本化路徑的&#x200B;**GET**；處理權杖到期、貼上範例JSON、對應欄位、測試、啟用。
...
1. 遵循[使用整合](external-sources.md)。 在所需的記錄路徑上使用&#x200B;**GET**、傳送必要的標頭（例如`API-VERSION`）、貼上範例JSON （傳回的HAL或JSON）、對應最小欄位集、測試、啟用。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Aprimo DAM /記錄API設定端點。 範例URL模式：

1. 您的租使用者的每個Aprimo API基本URL和資源路徑
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | Authorization | 持有人`<token>` | 標頭 |
...
1. 遵循[使用整合](external-sources.md)。 請勿猜測公開URL。 使用Epsilon的規格，貼上範例JSON，對應欄位，測試，啟動。

1. 在Journey Optimizer中，前往「設定>管理」，然後選取「建立整合」。

1. 輸入不含空格的整合名稱。

1. 使用Epsilon API設定端點（根據您的整合規格）。 範例URL模式：
...
| 類型 | api金鑰名稱 | API金鑰值 | 位置 |
| --- | --- | --- | --- |
| API金鑰 | `<your_username>` | `<EPSILON3_API_KEY>` | 標頭 |
