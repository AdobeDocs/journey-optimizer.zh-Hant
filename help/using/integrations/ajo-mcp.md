...
* **驗證頻道設定詳細資料** — 在歷程或行銷活動中使用頻道設定之前，請檢查寄件者網域、取消訂閱設定和IP集區。
* **確認治理原則** — 檢視哪些行銷動作和治理原則已附加到管道設定。
...
| 工具 | 說明 |
|---|---|
| **列出頻道設定** | 依名稱、狀態（草稿、作用中、已封存、已停用）或所有AJO頻道的頻道型別篩選頻道設定：電子郵件、行動訊息、推播通知、WhatsApp、直接郵件、應用程式內傳訊、Web、程式碼型體驗、內容卡、LINE、即時活動。 |
| **取得頻道設定** | 擷取特定通道設定的完整設定詳細資料，包括寄件者/回覆位址、子網域、IP集區和取消訂閱設定。 |
| **列出組態資源** | 列出頻道設定參考的支援資源，例如推播認證、電子郵件子網域、IP集區、SMS認證、WhatsApp認證、直接郵件路由、LINE頻道設定和即時活動登入。 |
| **取得設定資源** | 依型別和ID擷取單一設定資源的完整詳細資料。 |
| **列出行銷動作** | 列出資料治理原則強制的可用行銷動作。 |
...
| **頻道設定** | 我的沙箱中有哪些管道預設集可用？ /顯示我的所有電子郵件通道設定。 /我是否設定任何WhatsApp設定？ /我的行銷電子郵件設定設定設定了哪些寄件者地址和回覆？ |
...
您可以存取行銷活動、歷程、優惠、管道設定、設定資源和沙箱資訊。 操作是唯讀的（擷取API）；目前版本不支援寫入操作。
+++
