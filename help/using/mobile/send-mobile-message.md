---
solution: Journey Optimizer
product: journey optimizer
title: 檢查並測試您的行動裝置訊息
description: 瞭解如何在Journey Optimizer中檢查並傳送您的行動訊息
feature: SMS
topic: Content Management
role: User
level: Beginner
exl-id: 31c9b080-e334-4a11-af33-4c6f115c70a4
TQID: https://experienceleague.adobe.com/JPjBxyZzo13tgSLo0dqd5bFOwn9C6MHkA-DjLzlAdEI
product_v2: id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2: id: d0a62d3c-b79e-47e4-929e-40ef3cffa037
subfeature_v2: id: b3a93754-a8b8-46eb-9421-7eccaeeb3dffid: c41e8697-e629-4c38-96b3-564faaa17acfid: f8d2e9f0-69c9-40cd-890f-71336c8dfff7id: b3b09fe1-10f1-4793-9f6b-1ca0269eebe7
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550ccid: cdd65e7e-8839-44a2-bc21-0e03623b5dd1id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: a4e4f5ca5c3eb9dbfb5691cb5de420009ed7e5a5
workflow-type: tm+mt
source-wordcount: 534
ht-degree: 2%

---

# 檢查並傳送您的行動訊息 {#send-sms}

## 預覽您的行動裝置訊息 {#preview-sms}

定義訊息內容後，您可以使用任一模擬方法來預覽其內容：

* 按一下&#x200B;**[!UICONTROL 模擬內容]**&#x200B;以測試內容變數與範例輸入資料或AI自動產生。 [瞭解如何模擬內容變化](../test-approve/simulate-sample-input.md)
* 按一下&#x200B;**[!UICONTROL 模擬內容]**，然後從下拉式清單中選取&#x200B;**[!UICONTROL 模擬內容（AEP設定檔）]**，以使用測試設定檔預覽。

![](assets/sms_preview_2.png)

有關如何預覽及測試內容的詳細資訊，請參閱[內容管理](../content-management/preview-test.md)區段。

### 字元編碼和限制 {#sms-character-limits}

從&#x200B;**[!UICONTROL Simulate content]**&#x200B;存取任一模擬方法時，會顯示字元計數，以協助規劃及管理行動訊息。

![](assets/sms_preview_3.png)

Journey Optimizer在其SMS編輯器中使用UTF-8編碼，可讓您輸入或貼上雙位元組或Unicode字元。 然後這些字元會傳輸到服務提供者進行傳遞。 大部分的SMS提供者會針對具有160個字元限制的標準訊息使用GSM 7位元編碼，並在偵測到具有70個字元限制的非GSM字元時切換至UTF-16 (UCS-2)。

請注意，字元計數不會反映動態個人化或非GSM 7位元特殊字元引進的變數。

>[!IMPORTANT]
>
>Journey Optimizer SMS傳遞報告不會將串連訊息和動態個人化列入考量，因此可能無法反映提供者所傳送的實際訊息數量。 如需詳細使用情況和計費資訊，請聯絡您的Adobe代表。
>
>若要瞭解儘量降低SMS計費超額的最佳實務，請參閱[字元最佳化的SMS最佳實務](mobile-cost-optimization.md)。

## 驗證您的內容 {#sms-validate}

>[!NOTE]
>
> 若要改善您的傳遞能力，請以提供者支援的格式使用電話號碼。 例如，Twilio和Sinch只支援E.164格式的電話號碼。

您必須檢查編輯器上半區段的警示。 其中一些是簡單的警告，但其他警告可能會阻止您傳送訊息。 可能會發生兩種型別的警報：警告和錯誤。

![](assets/sms-alert-button.png)

* **警告**&#x200B;參考建議和最佳實務。 例如，如果您的行動訊息為空白或動態內容可能超過字元限制，則會顯示警告訊息。

  **字元限制：**&#x200B;每個區段160個字元（GSM 7位元），Unicode/表情符號最多70個，總共1500個字元。

* **錯誤**&#x200B;會阻止您測試或啟動歷程，或發佈行銷活動，只要這些錯誤未解決。 例如，當主旨行遺失時，錯誤訊息會警告您。

即使您的模擬訊息較短，也會出現警示&#x200B;**「已超過SMS文字字元限制」**，因為驗證會透過評估所有條件分支、個人化欄位及動態內容的最長長度來計算&#x200B;**最大可能長度**。

驗證會計算所有可能設定檔資料的最大長度，而模擬會顯示一個測試設定檔的實際輸出。

## 傳送您的行動裝置訊息 {#sms-send}

>[!IMPORTANT]
>
> 如果您的行銷活動受核准政策的約束，您將需要請求核准才能傳送您的行動裝置訊息。 [了解更多](../test-approve/gs-approval.md)

當您的行動裝置訊息就緒時，請完成您的[歷程](../building-journeys/journey-gs.md)或[行銷活動](../campaigns/create-campaign.md)的設定以進行傳送。

**相關主題**

* [設定簡訊頻道](mobile-configuration.md)
* [簡訊/RCS/多媒體簡訊報告](../reports/journey-global-report-cja-sms.md)
* [建立行動訊息](create-mobile-message.md)
* [在歷程中新增訊息](../building-journeys/journey-action.md)
