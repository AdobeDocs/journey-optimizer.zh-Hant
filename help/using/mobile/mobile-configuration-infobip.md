...
**在此頁面：**&#x200B;瞭解如何設定SMS API認證，將Infobip設定為您的Adobe Journey Optimizer提供者，以及如何透過自訂提供者整合啟用RCS傳訊。
...
Adobe Journey Optimizer使用[自訂SMS提供者](mobile-configuration-custom.md)功能，透過Infobip支援RCS傳訊。 這可透過已驗證的企業設定檔傳送豐富的互動式訊息，並納入輪播、按鈕和多媒體內容等元素。
...
1. **建立SMS Webhook**
   ...
   [在Journey Optimizer中設定自訂SMS webhook](mobile-configuration-custom.md#webhook)。 此webhook將負責處理來自Infobip平台的傳遞回條、入站RCS訊息和狀態更新。
