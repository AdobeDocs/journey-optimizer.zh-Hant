...
單一事件會連結到特定的設定檔。它們可能是以規則為基礎或由系統產生。更多關於酉事件的內容 [此部分](../event/about-events.md)。

以下是配置新事件的第一步：

1. 在「管理」(ADMINISTRATION)菜單部分，選擇 **[!UICONTROL 配置]**。 在  **[!UICONTROL 事件]** ，按一下 **[!UICONTROL 管理]**。 畫面隨即顯示事件清單。

   ![](assets/jo-event1.png)

1. 按一下 **[!UICONTROL 建立事件]** 的子菜單。 事件設定窗格會在畫面右側開啟。
...
1. 在 **[!UICONTROL 類型]** ，選擇 **酉**。

   ![](assets/jo-event3bis.png)

1. 在 **[!UICONTROL 事件ID類型]** 欄位中，選擇要使用的事件ID類型： **基於規則** 或 **系統生成**。 在中閱讀有關事件ID類型的更多資訊 [此部分](../event/about-events.md#event-id-type)。

   ![](assets/jo-event4.png)

1. 使用此事件的行程數顯示在 **[!UICONTROL 用於]** 的子菜單。 您可以按一下 **[!UICONTROL 查看行程]** 表徵圖，顯示使用此事件的行程清單。
...
1. 定義架構和負載欄位：這是您選擇事件資訊（通常稱為有效負載）旅行預期接收的地方。 接著，您就可以在歷程中使用這項資訊。請參閱[本節](../event/about-creating.md#define-the-payload-fields)。
   ...
   >[!NOTE]
   >
   >選擇 **[!UICONTROL 系統生成]** 類型，只有具有eventID類型欄位的架構可用。 選擇 **[!UICONTROL 基於規則]** 類型，所有體驗事件架構都可用。

1. 對於基於規則的事件，按一下 **[!UICONTROL 事件ID條件]** 的子菜單。 使用簡單表達式編輯器，定義系統將用於標識將觸發行程的事件的條件。
   ![](assets/jo-event6.png)

   在我們的例子中，我們根據檔案的城市寫了一個條件。 這意味著，每當系統收到與此條件匹配的事件(**[!UICONTROL 城市]** 欄位和 **[!UICONTROL 巴黎]** 價值)，它會傳到旅程。

   >[!NOTE]
   >
   >定義高級表達式編輯器時不可用 **[!UICONTROL 事件ID條件]**。 在簡單表達式編輯器中，並非所有運算子都可用，它們取決於資料類型。 例如，對於欄位的字串類型，可以使用&quot;contains&quot;或&quot;equal to&quot;。
...
1. 從清單中選擇XDM架構，然後按一下 **[!UICONTROL 欄位]** 或 **[!UICONTROL 編輯]** 表徵圖
...
1. 選擇要從事件接收的欄位。 這些是業務用戶在旅途中將利用的欄位。 它們還必須包括用於標識與事件關聯的人員的密鑰(請參閱 [此部分](../event/about-creating.md#define-the-event-key))。
   ...
   >[!NOTE]
   >
   >對於系統生成的事件， **[!UICONTROL 事件ID]** 欄位將自動添加到選定欄位清單中，以便 [!DNL Journey Optimizer] 可以識別事件。 推送事件的系統不應生成ID，它應使用負載預覽中可用的ID。 請參閱[本節](../event/about-creating.md#preview-the-payload)。

1. 選擇完所需欄位後，按一下 **[!UICONTROL 確定]** 按 **[!UICONTROL 輸入]**。

   所選欄位的數量顯示在 **[!UICONTROL 欄位]** 的子菜單。
...
如果選擇具有主標識的架構，則 **[!UICONTROL 探查器標識符]** 和 **[!UICONTROL 命名空間]** 欄位已預填充。 如果沒有定義標識，我們將選擇 _identityMap > id_ 鍵。 然後，必須選擇一個命名空間，並且該鍵將預填充(位於 **[!UICONTROL 命名空間]** 欄位使用 _identityMap > id_。
...
要使用儲存在AdobeReal-time Customer Profile資料庫中的資料，事件鍵必須是在中定義為配置檔案標識的資訊 [即時客戶配置檔案服務](https://experienceleague.adobe.com/docs/experience-platform/profile/home.html?lang=zh-Hant){target="_blank"}。

配置檔案標識符允許系統執行事件與個人配置檔案之間的協調。 如果選擇具有主標識的架構，則 **[!UICONTROL 配置檔案標識符]** 和 **[!UICONTROL 命名空間]** 欄位已預填充。 如果沒有定義標識， _identityMap > id_ 是主鍵。 然後，必須選擇一個命名空間，並且該鍵會自動使用 _identityMap > id_。
...
您還可以訪問高級表達式函式(**[!UICONTROL 高級模式]**)。 通過這些函式，您可以處理用於執行特定查詢的值，例如更改格式、執行欄位連接，只考慮欄位的一部分（例如10個首字元）。 請參閱此[頁面](../building-journeys/expression/expressionadvanced.md)。
...
1. 按一下 **[!UICONTROL 查看負載]** 表徵圖，預覽系統所需的負載。
