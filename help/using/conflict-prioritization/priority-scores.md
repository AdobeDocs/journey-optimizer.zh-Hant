...
>title="優先順序"
>abstract="為行銷活動指派優先順序分數。 優先順序是當有強加的限制（例如頻率上限）時排列行銷活動優先順序的必要條件。請輸入數值（從0到100）。 請注意，數字越高，優先順序越高。 若是兩個行銷活動有相同優先順序分數的情況，將會顯示最先啟用的行銷活動。"

>[!CONTEXTUALHELP]
>id="ajo_journey_priority"
>title="優先順序"
>abstract="指派歷程的優先順序分數。 當有強制的限制（例如頻率上限）時，優先順序對於安排歷程的優先順序至關重要。輸入數值（從0到100）。 請注意，數字越高，優先順序越高。 如果兩個歷程的優先順序分數相同，則會顯示先啟動的歷程。"
