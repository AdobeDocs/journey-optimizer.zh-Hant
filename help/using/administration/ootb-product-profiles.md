...
**在此頁面上：**&#x200B;探索內建角色和每個角色包含的許可權，以便您可以快速授予使用者符合其職責的現成存取層級。
...
* **TL；DR：** Journey Optimizer隨附內建角色（從Campaign管理員到協調的Campaign檢視器），每個角色都隨附現成的許可權集，因此管理員可快速授予使用者與其職責相符的存取層級，而不需從頭開始建立角色。

**意圖：**

* 找出最符合使用者工作責任的內建角色
* 瞭解每個內建角色可以做什麼，不能做什麼（包括發佈許可權）
* 比較歷程、行銷活動和協調行銷活動網域中的角色
* 指派現成的角色，而非建立自訂角色
* 瞭解哪些角色包含AI助理存取權

**字彙表：**

* **內建角色**：預先定義的許可權與資源許可權集，可指派給沒有自訂組態的使用者&#x200B;*（產品專用）*
* **歷程管理員**：內建角色，可管理及發佈歷程與決定管理，包括頻道設定和資料治理許可權&#x200B;*（產品專用）*
* **行銷活動管理員**：內建角色，可管理及發佈行銷活動和決策管理，包括頻道設定&#x200B;*（產品專用）*
* **決策管理員**：內建角色，提供對決策管理功能表的獨佔存取權；可以管理、檢視和發佈決策&#x200B;*（產品專屬）*
* **內容庫管理員**：內建角色，僅提供內容範本功能表的存取權；無法存取歷程或行銷活動&#x200B;*（產品專屬）*
* **測試模式**：在管理歷程和發佈歷程許可權中參考的歷程執行模式（歷程管理員可以在測試模式中停止歷程；發佈歷程許可權包括啟動測試模式） *（產品特定）*
* **試執行**：在「管理歷程與發佈歷程」許可權中參考的歷程執行模式，以及測試模式&#x200B;*（產品特定）*

**術語：**

* 正式名稱：內建角色 — 變體：現成可用的角色、OOTB角色、產品設定檔
* 請勿混淆：「行銷活動核准者」（可以核准並發佈行銷活動）≠「行銷活動管理員」（可以建立和編輯行銷活動，但無法發佈）
* 請勿混淆：「歷程核准者」（可以核准並發佈歷程）≠「歷程管理員」（可以建立和編輯歷程，但無法發佈）
* 請勿混淆：「歷程檢視器」（對歷程和決策管理的唯讀存取權）≠「行銷活動檢視器」（對行銷活動和決策管理的唯讀存取權）
* 請勿混淆：「協調的行銷活動管理員」（管理協調的行銷活動，包括AI助理和資料擷取/管理）≠「行銷活動管理員」（管理標準行銷活動；不包括協調的行銷活動許可權）
* 請勿混淆：「測試模式」（可作為歷程執行狀態參考，可透過管理歷程/發佈歷程來停止或啟動）≠「練習」（這些相同許可權中也會參考單獨的歷程執行模式）

**常見問題集：**

* **問：哪些內建角色可以發佈歷程？**  — 歷程管理員和歷程核准者可以發佈歷程。
* **問：Journey Manager可以發佈歷程嗎？**  — 否；Journey Manager可以建立和編輯歷程，但該角色不包含發佈歷程許可權。
* **問：哪個角色只授予決策管理功能表的存取權？**  — 決策管理員。
* **問：哪個角色只提供內容範本的存取權？**  — 內容庫管理員。
* **問：哪些內建角色包含「啟用AI小幫手」許可權？**  — 協調的行銷活動管理員、協調的行銷活動核准者、協調的行銷活動管理員和協調的行銷活動檢視者。
