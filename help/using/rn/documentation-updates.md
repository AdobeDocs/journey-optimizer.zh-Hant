...
* 內容最佳化檔案已重新建構，以更清楚明瞭。 主要最佳化頁面已分割成四個重點子頁面：快速入門頁面、鎖定目標的專屬頁面、實驗頁面，以及結合兩種方法的其他頁面。 [閱讀全文](../campaigns/gs-message-optimization.md)

* 已從三個歷程警報（已發佈歷程、歷程已完成和已觸發自訂動作上限）中移除可用性限制附註，因為這些功能現在已普遍可用。 [閱讀全文](../reports/alerts.md)
...
* 行銷活動中的最佳化頁面已更新，以反映最佳化，現在也可用於歷程的事實。[閱讀全文](../campaigns/gs-message-optimization.md)

* 已新增教學課程影片的連結，說明如何在行銷活動中運用訊息最佳化。[閱讀全文](../campaigns/gs-message-optimization.md)
