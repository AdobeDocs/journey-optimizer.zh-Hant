...
* 結構描述概觀檔案中已新增資訊，以區分用於協調行銷活動的標準和關聯式結構描述。 [閱讀全文](../data/gs-data.md)

* 已在決策和決定管理檔案中新增有關成功訓練[自動最佳化](../experience-decisioning/ranking/auto-optimization-model.md)和[個人化最佳化](../experience-decisioning/ranking/personalized-optimization-model.md)模型的要求的資訊。

* 澄清互動式訊息執行REST API呼叫具有60秒的逾時時間，且內部會重試以確保傳送。 [閱讀全文](../campaigns/trigger-campaigns.md)

* 已更新「決定專案集合」頁面，以釐清定義規則時&#x200B;**CONTAINS**&#x200B;運運算元的行為。 [閱讀全文](../experience-decisioning/collections.md)
...
* 新增了一個頁面，其中列出使用[!DNL Journey Optimizer]設計無障礙電子郵件和登入頁面內容的最佳實務。 [閱讀全文](../email/accessible-content.md)
