...
* 新增資料集查詢的最佳實務：隨時切換以避免索引問題，並瞭解批次刪除如何影響查詢資料。 [閱讀全文](../data/lookup-aep-data.md#guardrails--guidelines-guidelines)

* 新增限制，以說明搭配補充識別碼使用讀取對象歷程時，僅支援整合設定檔服務對象。 [閱讀全文](../building-journeys/supplemental-identifier.md#guardrails--limitations-guardrails)

## 2025 年 9 月 {#september-2025}

* 已針對協調行銷活動發行「常見問題集」頁面。 [閱讀全文](../orchestrated/orchestrated-campaigns-faq.md)

* 已在歷程步驟事件檔案中新增疑難排解章節，其中包含最常捨棄eventTypes的定義、常見原因和疑難排解步驟。 [閱讀全文](../reports/sharing-field-list.md#troubleshoot-discarded-event-types-in-journey_step_events)

* 如何在歷程中使用補充識別碼的檔案現在包含一個表格，詳細說明當使用補充ID在歷程中套用退出條件時，設定檔的行為方式。 [閱讀全文](../building-journeys/supplemental-identifier.md#exit-criteria)

* 疑難排解區段已新增到暫停歷程中瞭解的設定檔捨棄。 [閱讀全文](../building-journeys/journey-pause.md#troubleshoot-profile-discards-in-paused-journeys)
