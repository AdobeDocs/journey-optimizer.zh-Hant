...
* 新頁面現已推出，說明什麼是程式碼型體驗介面，包括介面URI、介面如何在任何階層層級（網頁、行動應用程式、內容位置）作為容器運作，以及它們如何將應用程式實作連結到通道設定。 [閱讀全文](../code-based/code-based-surface.md)

* 現在已推出新頁面，說明如何發佈及管理程式碼型體驗，包括啟用行銷活動、監控傳送、修改及停止行銷活動。 [閱讀全文](../code-based/publish-code-based.md)

* 測試程式碼型體驗現在提供新頁面，涵蓋如何使用測試設定檔預覽內容、使用瀏覽器開發人員工具或行動偵錯工具驗證實施，以及驗證內容在發佈前是否正確顯示。 [閱讀全文](../code-based/test-code-based.md)
