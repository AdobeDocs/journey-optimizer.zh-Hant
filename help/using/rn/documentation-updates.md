...
* 已在[護欄和限制](../start/guardrails.md#profile-management-inbound)頁面中新增以傳入頻道定位假名設定檔的護欄。 此護欄指定目標定位尚未驗證或未知的設定檔會增加可參與的設定檔總數，並可能影響您的帳單。 這就是為什麼Adobe建議設定自動刪除設定檔的存留時間(TTL)，以管理您的參與設定檔計數和相關成本。
