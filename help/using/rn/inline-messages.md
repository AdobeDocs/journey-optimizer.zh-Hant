...
* **產品更改**:在GA（7月25日），您的渠道內容將在每次行程中建立和管理。 的 **消息** 菜單，在左側導航中不再可用([瞭解更多](../rn/inline-messages.md#change))。 我們將針對您現有的行程進行遷移。

* **時間軸**:每個區域在夜間遷移，通過多個 [迭代](../rn/inline-messages.md#iterations)。
...
Adobe的願景是不斷簡化產品，以提供高效和優化的用戶流。 這種建立消息的新方法使用戶流程更加簡化。

我們設計了此新工作流，將內容集中到一個位置，直接將其用在何處。

現在，內容建立將直接在行程中執行。 立即 **利益** 得到：
...
在迭代過程中，我們將逐次遍歷並盡可能遷移它們。 有些情況下，我們不想自動遷移：當旅程是即時的時候（這意味著其中仍可能包含個人資料）。 在這些情況下，我們要求您執行操作，然後下一個迭代將遷移在以前的迭代中無法遷移的這些行程。
...
通過幾次迭代，在一夜之間部署更改。 瞭解更多 [迭代](../rn/inline-messages.md#inline-authoring)。
...
對於非即時或關閉的行程，遷移完全自動。 我們不想影響直播或關閉的旅程，以避免任何生產影響。 我們要求您發佈我們為您建立的新版本。

客戶ORG的所有沙箱都同時處理。 在更改部署期間，將執行以下操作：
...
在遷移期間修改消息的草稿版本。 他們不再引用留言了。 的 **消息** 活動被相應的渠道行動活動取代。 每個通道參數和內容。
...
遷移期間將建立此行程的新草稿版本。 此新草稿版本是您的即時版本的副本，但郵件將替換為內聯創作的通道操作。 每個頻道操作活動包括頻道參數和內容。 內容不會丟失。 報告不丟失。
...
在查看行程報告時，報告現在更豐富，包括以前在消息報告中提供的資訊級別。
...
您可以自動轉換行程，但需要執行幾個步驟。 瞭解有關此步驟中所需步驟的更多資訊 [頁](../rn/inline-messages-steps.md)。
...
遷移在夜間進行。 您將能夠使用該產品。 但不要建立、編輯或刪除旅程。
...
內聯創作功能將影響權限。 每個與郵件相關的權限，如 [!DNL View Messages] 或 [!DNL Manage Messages]，將自動包含在連結到Journeys功能的權限中。

瞭解更多資訊 [頁](../administration/ootb-product-profiles.md)。
