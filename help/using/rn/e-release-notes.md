...
* **CX同事忠誠度建議技能** — 行銷人員現在可以在CX同事的對話介面中直接要求&#x200B;**挑戰機會**，根據真正的忠誠度計畫趨勢獲得實際的挑戰想法，並在不離開聊天室的情況下將其轉換為即時挑戰。<!-- Documentation link: TBD -->
...
* 歷程模擬中的&#x200B;**決策路徑實驗** - **路徑實驗** （決策中最佳化活動的一部分）現在支援歷程模擬。<!-- Documentation link: TBD -->

* **歷程模擬中的補充ID支援** - **歷程模擬現在支援Supplemental ID**，可讓您針對讀取對象和事件觸發的歷程測試複雜的使用者案例。<!-- Documentation link: TBD -->

* **已改良批次對象評估等待邏輯** — 在&#x200B;**讀取對象活動**&#x200B;中，歷程中的「批次對象評估後觸發」選項現在只有在批次分段正在進行中，且要啟動的批次與上次執行中使用的批次不同時，才會等待新的對象評估，以避免不需要等待的歷程發生不必要的延遲。<!-- Documentation link: TBD -->
...
<thead>
<tr>
<th><strong>歷程中的目的地頻道</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Adobe Journey Optimizer現在於歷程畫布上加入新的<strong>Destinations節點</strong>，好讓Adobe Experience Platform Real-Time CDP和Journey Optimizer的共同客戶直接在歷程中新增或移除外部付費媒體對象（例如Facebook和Google）的設定檔。</p>
<!-- GIF placeholder: to be added -->
<!-- Documentation link: TBD -->
</td>
</tr>
...
* **自訂SMS BYOP驗證彈性** — 您現在可以在連線您的SMS提供者的OAuth設定時，設定&#x200B;**自訂驗證標頭**，包括權杖在傳出訊息中的放置位置以及權杖請求本身的格式。<!-- Documentation link: TBD -->
...
<thead>
<tr>
<th><strong>OR加入活動</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>AND — 聯結活動已升級為一般<strong>聯結活動</strong>，可讓您選擇AND和OR聯結條件。</p>
<!-- GIF placeholder: to be added -->
<!-- Documentation link: TBD -->
</td>
</tr>
...
<tr>
<td>
<p>協調的行銷活動現在支援<strong>即時警報</strong>，包括行銷活動失敗、執行時間超過定義的臨界值或點選活動層級錯誤時的關鍵通知，因此行銷人員可以擷取並解決問題，而不需要等候行銷活動完成。</p>
<!-- GIF placeholder: to be added -->
<!-- Documentation link: TBD -->
</td>
</tr>
...
* **行銷活動的資料夾** — 您現在可以將行銷活動整理到&#x200B;**資料夾**&#x200B;中，以改善介面中的導覽和管理。<!-- Documentation link: TBD -->
...
* **從CX Co-worker產生決策規則** — 先前透過右邊欄提供的&#x200B;**AI輔助決策規則產生**&#x200B;體驗現在可透過CX Co-worker存取，以取代右邊欄，作為使用AI建立規則的方式。<!-- Documentation link: TBD -->
...
* **電子郵件Designer中的新資料表元件** — 電子郵件Designer現在包含內建&#x200B;**資料表元件**，可讓您直接在電子郵件中建構列和欄的內容。 將元件拖放至畫布上、自訂列和欄數，並獨立設定每個儲存格的樣式，以建立清晰、有條理的版面配置，而不依賴自訂HTML。<!-- Documentation link: TBD -->

* **電子郵件主題中自訂字型的遞補字型** — 您現在可以為任何透過電子郵件主題套用的自訂(Web)字型定義遞補字型。 如果訂閱者的電子郵件使用者端不支援自訂字型，Adobe Journey Optimizer會自動顯示指定的遞補字型，而不會將選項保留給電子郵件使用者端的預設值。 這可讓電子郵件印刷樣式更貼近您的品牌方針，並減少電子郵件使用者端間的字型轉譯不一致問題。<!-- Documentation link: TBD -->

### 管理 {#sep-26-administration}

此版本中的管理即將進行下列改進。

* **自訂子網域的Feedback Loop OTP程式** — 直接在產品UI中顯示Yahoo寄件者中心&#x200B;**一次性密碼(OTP)**，已改善Feedback Loop (FBL)自訂子網域設定程式。 使用者現在可以自動擷取及顯示Yahoo寄件者中心網域擁有權驗證期間產生的OTP。<!-- Documentation link: TBD -->
