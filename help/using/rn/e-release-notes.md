...
* **由歷程設定的頻率上限** — 您現在可以建立規則集以套用至您的歷程，讓您限制每日、每週或每月的歷程次數，以及控制同時執行的並行歷程次數。

* **優先順序分數** — 您現在可以指派優先順序分數至行銷活動或歷程，範圍從0到100。 數字越高表示優先順序越高。 當兩個行銷活動或歷程使用相同的管道設定時，Journey Optimizer會選取優先順序分數最高的行銷活動或歷程。 如果行銷活動具有相同的分數，將會選擇最近修改的行銷活動。 優先順序分數適用於行銷活動中的所有傳入頻道，以及歷程中的應用程式內頻道。

* **檢視衝突** — 歷程和行銷活動中的新&#x200B;**檢視衝突**&#x200B;按鈕現在可讓您檢查與其他歷程或行銷活動重疊的可能性，例如開始日期、目標對象或選取的頻道設定。

>[!AVAILABILITY]
>
>特定客戶群組可在「有限可用性」中使用衝突和優先順序管理功能。 請注意，這些功能將在未來逐步向更多使用者推出。 如果您有興趣加入此功能的輪候表，請洽詢您的客戶團隊。
...
* **頻道設定個人化** — 在行銷活動或歷程中使用個人化設定時，您現在可以預覽電子郵件內容，以使用您定義的動態設定檢查潛在錯誤。
