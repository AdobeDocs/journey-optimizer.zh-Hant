...
<thead>
<tr>
<th><strong>歷程和營銷活動中的批准（全面發佈）</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td>
<p>您可以在建立電子郵件管道設定時，馬上定義動態子網域和個人化標題參數，就能提高電子郵件設定彈性，更能夠掌握得當。</p><p>在行銷活動或歷程中使用個性化配置允許您預覽电子郵件內容，以檢查您定義的動態設置是否存在潜在錯誤。</p>
<p>電子郵件配置個人化以前可用于一組組織 （LA），現在可供所有使用者 （GA） 使用。</p>
...
<li><b>檢視潛在衝突</b>：現在，歷程和營銷活動中的新「檢視潛在衝突」按鈕允許您識別與其他歷程或營銷活動的重疊，例如開始日期、定位對象或所選通道配置。</li>
<li><b>旅程仲裁</b>：這項新功能使您能夠為客戶確定最重要的旅程的優先順序。 您可以創建一個規則，以在客戶有資格進入優先順序較高的即將到來的旅程時禁止進入優先順序較低的旅程。</li></ul></p>
<!--<p>For more information, refer to the <a href="../email/surface-personalization.md">detailed documentation</a>.</p>-->
<p>衝突和優先順序管理功能在有限可用性中可供選定群組客戶使用。 請注意，這些功能將在未來逐步向更多使用者推出。 如果您有興趣新增至這些功能的輪候表，請洽詢您的客戶團隊。</p>
...
<p>歷程中的實驗以前可用於一組組織 （LA），現在可供所有使用者 （GA） 使用。</p>
</td>
</tr>
</tbody>
</table>
...
<tr>
<td>
<p>您現在可以創建精細的頻率限定規則，並將其應用於您的消息或通過規則集的旅程。 借助這項新功能，您可以通過設置跨通道規則來控制受眾接收消息的頻率，這些規則會自動從消息和操作中排除過度請求的配置檔。</p><p>它還允許您限制每天、每周或每月的旅程數，以及控制同時運行的併發旅程數。</p>
<p>規則集以有限可用性提供給選定群組客戶使用。 請注意，這些功能將在未來逐步向更多使用者推出。 如果有興趣被添加到此功能的候補名單中，請聯繫您的帳戶團隊。</p>
<!--p>For more information, refer to the <a href="../configuration/business-rules.md">detailed documentation</a>.</p-->
</td>
</tr>
</tbody>
...
<thead>
<tr>
<th><strong>歷程和行銷活動中的多語言消息（全面發佈）</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
...
* **存留時間護欄** — 從2024年11月1日開始，將在新沙箱和新組織中向Journey Optimizer系統產生的資料集推出存留時間(TTL)護欄，如下所示：
  ...
  此變更隨後將在第二階段推廣到現有客戶沙盒。

  此外，從 11 月 1 日開始，流式處理細分將不再支援使用來自跟蹤和反饋數據集的發送和打開事件。 屆時，此變更將套用至所有客戶沙盒和組織。 [了解更多](../data/datasets-ttl.md)

* **自定義操作** 中的參數（可用日期：2024 年 10 月 3 日）- 自定義操作現在支援 NULL 和可選參數。 [了解更多](../action/about-custom-action-configuration.md#define-the-message-parameters)
...
* 現在可以使用內容範本。 您可以從開發人員構建的內容範本開始，加快創作基於代碼的體驗的速度。 使用 內容 範本將允許行銷人員只修改某些值或字段，而不是將整個 HTML 或 JSON 組合內容有效負載。
