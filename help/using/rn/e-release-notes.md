...
<tr>
<td>
<p>您現在可以使用公式，根據客戶設定檔屬性和情境式因素自動排定歷程的優先順序及仲裁順序，確保客戶輸入最相關的歷程。</p>
<p>先前以「有限可用性」發行，現在此功能可供所有客戶使用（一般可用性）。</p>
...
* **在歷程模擬中支援外部對象（CSV和FAC）** — 歷程模擬現在支援外部對象。 模擬以CSV或同盟對象構成(FAC)對象為目標的歷程時，您可以直接透過UI表單或JSON匯入，從這些對象中模擬擴充屬性。 UI只會動態顯示歷程邏輯中使用的特定擴充屬性，以在決策分支和個人化規則上線之前進行精確驗證。 ([DOCAC-15074](https://jira.corp.adobe.com/browse/DOCAC-15074)) <!-- Documentation link: TBD -->
...
* **[OC]管理設定檔目標維度的能力** — 您現在可以刪除設定檔目標Dimension，或編輯並交換其設定的身分名稱空間，讓您對資料設定有更優異的控制力和彈性。 ([DOCAC-15018](https://jira.corp.adobe.com/browse/DOCAC-15018)) <!-- Documentation link: TBD -->
...
* **電子郵件範本**&#x200B;的`<head>`支援運算式片段 — 您現在可以在任何電子郵件的`<head>`中新增運算式片段。 如果未來需要更新樣式，您可以只更新一個片段並重新發佈 — 從範本建立的每封電子郵件都會獲得片段中的最新程式碼，因此也會獲得最新的樣式。 ([DOCAC-15257](https://jira.corp.adobe.com/browse/DOCAC-15257)) <!-- Documentation link: TBD -->

* **「AI小幫手」已重新命名為「產生內容」** — 「AI小幫手」已重新命名為「透過Adobe Journey Optimizer產生內容」。 本次更新僅限於命名和術語 — 內容產生、影像產生、個人化運算式和內容實驗等的導覽標籤、按鈕、選單和對話方塊已從「AI助理」重新命名為「產生內容」。 未匯入任何功能變更。 ([DOCAC-15230](https://jira.corp.adobe.com/browse/DOCAC-15230)) <!-- Documentation link: TBD -->

* **使用AEM Asset Essentials產生內容** — 產生內容現在會在產生電子郵件、網頁和推播通知時，直接從Adobe Experience Manager Assets自動擷取品牌核准的影像。 如此一來，您就不需要手動搜尋資產或仰賴一般 AI 遞補功能，確保每個視覺效果都完全正確且符合品牌規範。 ([DOCAC-14761](https://jira.corp.adobe.com/browse/DOCAC-14761)) <!-- Documentation link: TBD -->
...
* **個人化運算式中的新協助程式功能** — 個人化運算式現在有新的協助程式功能可用： `appendQueryParams`、`dateBetween`、`equalsAnyIgnoreCase`、`getUrlFragment`、`join`和`decode64`。 `concat`函式也已增強，現在支援兩個或多個引數。 此外，下列範本移轉功能現在可用於協助將現有範本移轉到Journey Optimizer： `ampCompare`、`ampSubstr`和`compareTo`。 ([DOCAC-15099](https://jira.corp.adobe.com/browse/DOCAC-15099)) <!-- Documentation link: TBD -->
...
* **WhatsApp管道：支援WhatsApp流程範本** — 您現在可以在Adobe Journey Optimizer中傳送WhatsApp流程範本，以提供互動式的多熒幕體驗，例如調查和潛在客戶擷取。 回應會在提交時擷取，並儲存為新AJO管道追蹤事件資料集中的原始JSON裝載。 ([DOCAC-15223](https://jira.corp.adobe.com/browse/DOCAC-15223)) <!-- Documentation link: TBD -->

* **輸送量的效能附加元件 — 推播** — 在API觸發的行銷活動中提供新的高輸送量異動訊息模式，專為大規模、即時異動訊息而設計，每秒支援高達5,000個異動以及更高的可用性。 此功能先前僅適用於電子郵件頻道，現在也可用於推播頻道，適用於已購買Adobe高輸送量異動訊息附加元件產品的組織。 請聯絡您的 Adobe 代表以取得更多資訊。 ([DOCAC-14717](https://jira.corp.adobe.com/browse/DOCAC-14717)) <!-- Documentation link: TBD -->
...
<thead>
<tr>
<th><strong>Web應用程式防火牆(WAF) IP白名單</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Adobe Journey Optimizer現在支援將登陸頁面列入網頁應用程式防火牆(WAF) IP白名單，方便組織強制所有傳入要求都透過其設定的WAF基礎架構專門路由。 透過這項增強功能，客戶可將 Journey Optimizer 設定為拒絕任何略過 WAF 層的直接請求，確保一致套用 Imperva 等工具中定義的安全性原則。</p>
<p>此功能可加強具有嚴格網路存取需求之企業的安全性態勢，讓企業能夠完全控制其 AJO 託管登陸頁面的流量。</p>
<p>推出日期： 2026年7月22日</p>
