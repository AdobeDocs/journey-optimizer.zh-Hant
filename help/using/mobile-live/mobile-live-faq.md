...
「即時」活動可提供「鎖定畫面」和「動態島」上的持續即時更新，使用者無須解除鎖定裝置。 推播通知是暫時性警報，在關閉後便會消失。 即時活動會維持可見狀態，而且可以多次更新，直到明確結束為止。

+++

+++一次可以啟用多少個已上線活動執行個體？

iOS應用程式可同時執行多個Live活動執行個體，包括使用相同`ActivityAttributes`型別的多個執行個體。

開發人員對於特定屬性型別的已上線活動例項數量沒有硬性限制。 例如，您可以視應用程式邏輯需求，針對每個持續傳遞或歷程啟動一次。 不過，iOS會針對一次可以作用中或可見的已上線活動執行個體數量實施系統層級限制。

實際操作中：

* iOS通常支援每個應用程式最多五個同時上線的活動例項。

* 如果超過此數目，系統可能會停止顯示某些活動例項，或終止較舊的例項，以節省資源。

* 每個已上線活動執行個體都有唯一的`Activity.id`，可讓您個別更新或結束活動。

+++

+++使用者需要開啟應用程式才能接收即時活動更新嗎？

不會。 即使應用程式完全關閉，您也可以從遠端開始、更新及結束即時活動，這是該功能的主要優點之一。
...
* iOS 18+：對受眾型Live活動的廣播頻道支援
+++

+++已上線活動可以維持作用中狀態多久？

Apple將即時活動限製為&#x200B;**8小時的作用中更新**。 之後，系統會自動結束活動，不過在移除之前，它最多可能會再顯示&#x200B;**12小時**。 您也可以在應用程式中設定`dismissalDate`或明確呼叫`activity.end()`，以更快結束即時活動。

+++

+++ 有哪些速率限制？

行銷活動的預設速率限製為所有管道每秒最多500則交易式訊息，包括iOS Live活動。 此限制適用於所有合併頻道，而iOS Live活動沒有專門的個別速率限制。
...
+++我可以針對本機與遠端Live活動使用相同的`LiveActivityAttributes`類別嗎？

可以。 相同的屬性類別適用於本機啟動和遠端啟動（推送至啟動）的即時活動。 您必須確定向`Messaging.registerLiveActivity()`註冊。

+++

+++如果傳送不存在之已上線活動的更新時，會發生什麼情況？

如果您傳送不存在`liveActivityID`或`channelID`的更新或結束事件，要求將會在裝置上無訊息地失敗。 請一律確保您追蹤每個使用者使用中的哪些上線活動例項。
...
* **本機**：這包括使用&#x200B;**ActivityKit API**，直接從您的應用程式建立、更新及結束Live活動。

* **遠端**：若要遠端測試Live活動功能，請將我們的傳訊SDK整合至您的應用程式，並使用提供的執行API傳送遠端開始、更新及結束至您的測試裝置或iOS模擬器。 類似於目前使用Adobe SDK整合來測試推播通知的方式。
...
* `liveActivityID`：用於針對特定使用者的個別（單一）上線活動。 Each ID represents a unique Live activity instance.
...
[Monitor your campaign analytics](../reports/campaign-global-report-cja-activity.md) in Adobe Journey Optimizer. 您可以追蹤傳遞率、失敗和參與量度。 您也可以考慮在應用程式中實作自訂分析事件。
+++
...
+++如果我為已存在的上線活動傳送「開始」事件，會發生什麼情況？

透過Adobe的執行API從遠端開始即時活動時：

* 您可以在請求中包含`x-request-id`標頭。 理想情況下，每個`liveActivityID`與其對應的`x-request-id`之間應該有一對一的關係。 這可確保在使用相同`x-request-id`和`liveActivityID`組合提出多個請求時，裝置上只會啟動一個已上線活動執行個體，且會忽略重複請求。

* 如果省略`x-request-id`標頭，則會個別處理每個要求，而可能導致使用相同的`liveActivityID`建立多個已上線活動執行個體。 在這種情況下，未來的更新可能會失敗或只適用於其中一個使用中的執行個體。

* `x-request-id`值不應在個別的API要求中跨不同的`liveActivityIDs`重複使用。

+++

+++我可以使用A/B測試不同的上線活動體驗嗎？
...
* `dismissal-date`：即時活動應自動關閉的未來紀元時間，僅適用於「結束」事件。
...
是的，這是預期行為。 當您的商業邏輯需要時，後端會呼叫Adobe Journey Optimizer Headless API以觸發即時活動事件。
...
>[!TIP]
>
>如需完整的疑難排解指南，請參閱[疑難排解上線活動](troubleshoot-mobile-live.md)。
...
* 確保類別名稱與您的Swift結構名稱完全符合&#x200B;**1&rbrace; （區分大小寫）**
* 確認結構已正確定義並註冊
* 檢查JSON裝載中的拼寫錯誤
* 確認安裝的應用程式版本具有上線活動實作
...
不可以。 `alert`欄位是選用欄位，在某些條件下（例如「請勿打擾」模式），可能會被iOS隱藏。 上線活動可以無訊息更新，這通常是預期的行為。 傳送遠端啟動時，必須填寫警示欄位，否則apple會將它視為無訊息背景通知。

+++

+++我可以刪除或清除使用者的所有已上線活動執行個體嗎？

您需要為每個作用中的上線活動執行個體傳送「結束」事件。 追蹤系統中每個使用者的作用中上線活動例項，以便您正確清除。
