...
1. 定義體驗事件結構描述。 在此使用案例中，系統會建立「優惠方案內容」結構，且優惠方案內容欄位是「優惠方案內容」欄位群組的一部分。 事實上，欄位群組會新增至體驗事件結構描述，用於與「Edge收集網路」資料流相關聯的資料收集。
   ...
   瞭解如何在Adobe Experience Platform [Experience Data Model (XDM)指南](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/home){target="_blank"}中使用結構描述
