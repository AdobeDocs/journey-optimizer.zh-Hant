...
1. 定義 **[!UICONTROL 優先順序]** 相較於其他優惠方案的數量（如果使用者符合多個優惠方案的資格）。 優惠的優先順序越高，相較於其他優惠的優先順序就越高。
...
1. 選擇您要將上限套用至所有使用者，還是隻套用一個設定檔。 [了解更多](#capping-type)
...
在準備電子郵件時會計算建議某個優惠方案的次數。 例如，如果您準備包含多個優惠方案的電子郵件，無論是否有傳送電子郵件，這些數量都會計入您的次數上限中。
...
>[!CAUTION]
>
>對於決策事件以外的所有上限事件，決策管理意見反應可能不會自動收集，這可能會導致上限計數器無法正確增加。 [了解更多](../data-collection/data-collection.md)
>
>為確保每個上限事件都能在上限計數器中被追蹤和說明，請確定用來收集體驗事件的結構描述包含該事件的正確欄位群組。 [了解更多](../data-collection/schema-requirement.md)
...
| ...新的開始日期在目前的結束日期之前， | ...上限會以新的開始日期繼續，而每個設定檔的先前上限計數則會結轉。 | 無 |
...
| 案例：<br>若…… | 會發生什麼情況：<br>然後…… | 對上限計數的可能影響 |
|--- |--- |--- |
| ...決策請求發生在原始優惠方案結束日期之前， | ...上限計數將更新，每個設定檔的上一個上限計數將結轉。 | 無 |
