...
您可以在提供容器ID的同時，向[!DNL Offer Library] API提出POST請求，以建立位置。
...
成功的回應會傳回新建立位置的詳細資訊，包括其唯一例項ID和位置`@id`。 您可以在後續步驟中使用例項ID來更新或刪除您的位置。 您可以在後續教學課程中使用您獨特的`@id`位置來建立決策、決策規則和備援選件。
