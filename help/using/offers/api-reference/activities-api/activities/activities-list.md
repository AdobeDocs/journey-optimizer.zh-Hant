...
| 參數 | 說明 | 範例 |
| --------- | ----------- | ------- |
| `property` | 選用的屬性篩選器： <ul><li> 屬性會依AND作業分組。 <br><br>  — 引數可以重複執行，如下所示：property=`<property-expr>`[屬性(&amp;P)=`<property-expr2>`...] 或屬性=`<property-expr1>`[&amp;`<property-expr2>`...] <br><br>  — 屬性運算式的格式為 [！]欄位[op]值，含運算式 [==！=，&lt;=，>=，&lt;，>，~]，支援規則運算式 </li></ul> | `property=name!=abc&property=id~.*1234.*&property=description equivalent with property=name!=abc,id~.*1234.*,description.` |
