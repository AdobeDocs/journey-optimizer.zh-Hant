...
# 使用決策API提供服務 {#decisioning-api}

使用決策管理，您可以使用業務邏輯和決策規則跨渠道和應用程式建立和提供最終用戶個性化的服務體驗。 聘用是一條市場營銷消息，可能具有與其關聯的規則，指定誰有資格查看聘用。

您可以通過向Oracle Server發出POST請求來建立和交付優惠 [!DNL Decisioning] API。
...
| `xdm:profiles.xdm:decisionRequestId` | 客戶端生成的ID，可用於唯一標識配置檔案決策請求。 此ID在響應中回復，不影響決定的結果。 | `"xdm:decisionRequestId": "0AA00002-0000-1224-c0de-cjf98Csj43"` |
...
|  ---  |  ---  |
| 200 | 成功. 已就特定活動作出決定 |
| 400 | 無效的請求參數。 由於語法格式不正確，伺服器無法理解該請求。 |
| 403 | 禁止，權限不足。 |
| 422 | 無法處理的實體。 但是，由於語義錯誤，無法處理請求語法。 |
| 429 | 請求太多。 用戶在給定的時間內發送了太多請求。 |
| 500 | 內部伺服器錯誤。 伺服器遇到意外情況，無法完成請求。 |
| 503 | 服務因伺服器過載而不可用。 由於臨時超載，伺服器當前無法處理該請求。 |
