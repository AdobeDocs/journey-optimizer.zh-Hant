...
[Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html?lang=zh-Hant#video-overview)是使用者端的JavaScript資料庫，可讓Adobe Experience Cloud客戶透過Experience PlatformEdge Network與Experience Cloud中的各種服務互動。
...
有兩種方式可透過[平台Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html?lang=zh-Hant#video-overview)實作決定管理。 一種方式是針對開發人員，且需要網站和程式設計知識。 另一種方式是使用Adobe Experience Platform使用者介面來設定選件，這只需要在HTML頁面的標頭中參照小型指令碼。
...
包含選項2中的下列JavaScript程式碼片段：在HTML頁面的`<head>`區段中，於[此頁面](https://experienceleague.adobe.com/docs/experience-platform/edge/fundamentals/installing-the-sdk.html?lang=zh-Hant)預先建立的獨立版本。
