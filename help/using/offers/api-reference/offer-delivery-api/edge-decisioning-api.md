...
[Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html#video-overview)是使用者端的JavaScript資料庫，可讓Adobe Experience Cloud客戶透過Experience Platform Edge Network與Experience Cloud中的各種服務互動。
...
有兩種方式可透過[平台Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html#video-overview)實作決定管理。 一種方式是針對開發人員，且需要網站和程式設計知識。 另一種方式是使用Adobe Experience Platform使用者介面來設定選件，該選件只需要在HTML頁面標頭中參照小型指令碼。
...
>[!NOTE]
>
>**在Edge Decisioning請求中傳遞內容資料**
>
>您可以在您的Edge Decisioning請求中傳遞內容資料（例如裝置型別、位置或使用者偏好設定），以建立動態適用性規則並根據即時條件提供個人化優惠。 [進一步瞭解內容資料與Edge Decisioning請求](../../context-data-edge.md)
...
包含選項2中的下列JavaScript程式碼片段：在HTML頁面的[區段中，於](https://experienceleague.adobe.com/docs/experience-platform/edge/fundamentals/installing-the-sdk.html)此頁面`<head>`預先建立的獨立版本。
