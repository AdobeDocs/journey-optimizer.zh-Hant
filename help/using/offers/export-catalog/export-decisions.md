...
下面是可在 **[!UICONTROL 決策對象儲存庫 — 決策]** 資料集（以前稱為Decision Object Repository - Activities）。
...
* 選項篩選器
   ...
   **欄位：** 濾波器
   **標題：** 選項篩選器
   **描述：** 對基於收集限定符（以前稱為「標籤」）的篩選器的引用，該篩選器使用其附加的收集限定符匹配庫存中的選項。 該值是引用的決策規則的URI(@id)。 請參見架構https://ns.adobe.com/experience/decisioning/filter。
   **類型:**&#x200B;字串
